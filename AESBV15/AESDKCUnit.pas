unit AESDKCUnit;

interface

procedure MakeContextsAES_DKC_PAS_17(const Key; KeySize : Cardinal);
procedure EncryptAES_DKC_PAS_17(var Block);
procedure DecryptAES_DKC_PAS_17(var Block);
procedure MakeContextsAES_DKC_PAS_18(const Key; KeySize : Cardinal);
procedure EncryptAES_DKC_PAS_18(var Block);
procedure DecryptAES_DKC_PAS_18(var Block);
procedure MakeContextsAES_DKC_PAS_19(const Key; KeySize : Cardinal);
procedure EncryptAES_DKC_PAS_19(var Block);
procedure DecryptAES_DKC_PAS_19(var Block);
procedure MakeContextsAES_DKC_IA32_1(const Key; KeySize : Cardinal);
procedure EncryptAES_DKC_IA32_1(var Block);
procedure DecryptAES_DKC_IA32_1(var Block);
//procedure MakeContextsAES_DKC_IA32_2(const Key; KeySize : Cardinal);
procedure EncryptAES_DKC_IA32_2(var Block);
procedure DecryptAES_DKC_IA32_2(var Block);
//procedure MakeContextsAES_DKC_IA32_3(const Key; KeySize : Cardinal);
procedure EncryptAES_DKC_IA32_3(var Block);
procedure DecryptAES_DKC_IA32_3(var Block);


implementation

uses
 Math, SysUtils;

const
 MaxAESRounds = 15;

 AESNb128 = 4;      { 128 bit block }

 AESNk128 = 4;      { 128 bit key }
 AESNk192 = 6;      { 192 bit key }
 AESNk256 = 8;      { 256 bit key }

type
  TAESBlock  = array[0..15] of Byte;

  TAESVector = record
    case Byte of
      0 : (dw : Cardinal);
      1 : (bt : array[0..3] of Byte);
    end;

  TAESRoundKeys = packed record
   case Byte of
    0 : (W  : array[0..(MaxAESRounds * 4)] of TAESVector);
    1 : (Rk : array[0..MaxAESRounds] of TAESBlock);
   end;

  TAESContext = packed record
    RoundKeys : TAESRoundKeys;
    Encrypt   : Boolean;
    Rounds    : Cardinal;
    KeySize   : Cardinal;
   end;

 TAESVectors = array[0..3] of TAESVector;
 TAESMixColMatrix = array[0..3, 0..3] of Byte;
 TTempStorage = array[0..15] of Cardinal;//Includes room for alignment by max 12 bytes

const
  AESSBox : array[$00..$FF] of Byte =
   ($63, $7C, $77, $7B, $F2, $6B, $6F, $C5, $30, $01, $67, $2B, $FE, $D7, $AB, $76,
    $CA, $82, $C9, $7D, $FA, $59, $47, $F0, $AD, $D4, $A2, $AF, $9C, $A4, $72, $C0,
    $B7, $FD, $93, $26, $36, $3F, $F7, $CC, $34, $A5, $E5, $F1, $71, $D8, $31, $15,
    $04, $C7, $23, $C3, $18, $96, $05, $9A, $07, $12, $80, $E2, $EB, $27, $B2, $75,
    $09, $83, $2C, $1A, $1B, $6E, $5A, $A0, $52, $3B, $D6, $B3, $29, $E3, $2F, $84,
    $53, $D1, $00, $ED, $20, $FC, $B1, $5B, $6A, $CB, $BE, $39, $4A, $4C, $58, $CF,
    $D0, $EF, $AA, $FB, $43, $4D, $33, $85, $45, $F9, $02, $7F, $50, $3C, $9F, $A8,
    $51, $A3, $40, $8F, $92, $9D, $38, $F5, $BC, $B6, $DA, $21, $10, $FF, $F3, $D2,
    $CD, $0C, $13, $EC, $5F, $97, $44, $17, $C4, $A7, $7E, $3D, $64, $5D, $19, $73,
    $60, $81, $4F, $DC, $22, $2A, $90, $88, $46, $EE, $B8, $14, $DE, $5E, $0B, $DB,
    $E0, $32, $3A, $0A, $49, $06, $24, $5C, $C2, $D3, $AC, $62, $91, $95, $E4, $79,
    $E7, $C8, $37, $6D, $8D, $D5, $4E, $A9, $6C, $56, $F4, $EA, $65, $7A, $AE, $08,
    $BA, $78, $25, $2E, $1C, $A6, $B4, $C6, $E8, $DD, $74, $1F, $4B, $BD, $8B, $8A,
    $70, $3E, $B5, $66, $48, $03, $F6, $0E, $61, $35, $57, $B9, $86, $C1, $1D, $9E,
    $E1, $F8, $98, $11, $69, $D9, $8E, $94, $9B, $1E, $87, $E9, $CE, $55, $28, $DF,
    $8C, $A1, $89, $0D, $BF, $E6, $42, $68, $41, $99, $2D, $0F, $B0, $54, $BB, $16);

  AESInvSBox : array[$00..$FF] of Byte =
   ($52, $09, $6A, $D5, $30, $36, $A5, $38, $BF, $40, $A3, $9E, $81, $F3, $D7, $FB,
    $7C, $E3, $39, $82, $9B, $2F, $FF, $87, $34, $8E, $43, $44, $C4, $DE, $E9, $CB,
    $54, $7B, $94, $32, $A6, $C2, $23, $3D, $EE, $4C, $95, $0B, $42, $FA, $C3, $4E,
    $08, $2E, $A1, $66, $28, $D9, $24, $B2, $76, $5B, $A2, $49, $6D, $8B, $D1, $25,
    $72, $F8, $F6, $64, $86, $68, $98, $16, $D4, $A4, $5C, $CC, $5D, $65, $B6, $92,
    $6C, $70, $48, $50, $FD, $ED, $B9, $DA, $5E, $15, $46, $57, $A7, $8D, $9D, $84,
    $90, $D8, $AB, $00, $8C, $BC, $D3, $0A, $F7, $E4, $58, $05, $B8, $B3, $45, $06,
    $D0, $2C, $1E, $8F, $CA, $3F, $0F, $02, $C1, $AF, $BD, $03, $01, $13, $8A, $6B,
    $3A, $91, $11, $41, $4F, $67, $DC, $EA, $97, $F2, $CF, $CE, $F0, $B4, $E6, $73,
    $96, $AC, $74, $22, $E7, $AD, $35, $85, $E2, $F9, $37, $E8, $1C, $75, $DF, $6E,
    $47, $F1, $1A, $71, $1D, $29, $C5, $89, $6F, $B7, $62, $0E, $AA, $18, $BE, $1B,
    $FC, $56, $3E, $4B, $C6, $D2, $79, $20, $9A, $DB, $C0, $FE, $78, $CD, $5A, $F4,
    $1F, $DD, $A8, $33, $88, $07, $C7, $31, $B1, $12, $10, $59, $27, $80, $EC, $5F,
    $60, $51, $7F, $A9, $19, $B5, $4A, $0D, $2D, $E5, $7A, $9F, $93, $C9, $9C, $EF,
    $A0, $E0, $3B, $4D, $AE, $2A, $F5, $B0, $C8, $EB, $BB, $3C, $83, $53, $99, $61,
    $17, $2B, $04, $7E, $BA, $77, $D6, $26, $E1, $69, $14, $63, $55, $21, $0C, $7D);

  RCon : array[1..MaxAESRounds] of Cardinal =
   ($00000001, $00000002, $00000004, $00000008, $00000010, $00000020, $00000040,
    $00000080, $0000001B, $00000036, $0000006C, $000000D8, $000000AB, $0000004D, $0000009A);

  AES_T0 : array[Byte] of Cardinal =
    ($A56363C6, $847C7CF8, $997777EE, $8D7B7BF6, $0DF2F2FF, $BD6B6BD6, $B16F6FDE, $54C5C591,
     $50303060, $03010102, $A96767CE, $7D2B2B56, $19FEFEE7, $62D7D7B5, $E6ABAB4D, $9A7676EC,
     $45CACA8F, $9D82821F, $40C9C989, $877D7DFA, $15FAFAEF, $EB5959B2, $C947478E, $0BF0F0FB,
     $ECADAD41, $67D4D4B3, $FDA2A25F, $EAAFAF45, $BF9C9C23, $F7A4A453, $967272E4, $5BC0C09B,
     $C2B7B775, $1CFDFDE1, $AE93933D, $6A26264C, $5A36366C, $413F3F7E, $02F7F7F5, $4FCCCC83,
     $5C343468, $F4A5A551, $34E5E5D1, $08F1F1F9, $937171E2, $73D8D8AB, $53313162, $3F15152A,
     $0C040408, $52C7C795, $65232346, $5EC3C39D, $28181830, $A1969637, $0F05050A, $B59A9A2F,
     $0907070E, $36121224, $9B80801B, $3DE2E2DF, $26EBEBCD, $6927274E, $CDB2B27F, $9F7575EA,
     $1B090912, $9E83831D, $742C2C58, $2E1A1A34, $2D1B1B36, $B26E6EDC, $EE5A5AB4, $FBA0A05B,
     $F65252A4, $4D3B3B76, $61D6D6B7, $CEB3B37D, $7B292952, $3EE3E3DD, $712F2F5E, $97848413,
     $F55353A6, $68D1D1B9, $00000000, $2CEDEDC1, $60202040, $1FFCFCE3, $C8B1B179, $ED5B5BB6,
     $BE6A6AD4, $46CBCB8D, $D9BEBE67, $4B393972, $DE4A4A94, $D44C4C98, $E85858B0, $4ACFCF85,
     $6BD0D0BB, $2AEFEFC5, $E5AAAA4F, $16FBFBED, $C5434386, $D74D4D9A, $55333366, $94858511,
     $CF45458A, $10F9F9E9, $06020204, $817F7FFE, $F05050A0, $443C3C78, $BA9F9F25, $E3A8A84B,
     $F35151A2, $FEA3A35D, $C0404080, $8A8F8F05, $AD92923F, $BC9D9D21, $48383870, $04F5F5F1,
     $DFBCBC63, $C1B6B677, $75DADAAF, $63212142, $30101020, $1AFFFFE5, $0EF3F3FD, $6DD2D2BF,
     $4CCDCD81, $140C0C18, $35131326, $2FECECC3, $E15F5FBE, $A2979735, $CC444488, $3917172E,
     $57C4C493, $F2A7A755, $827E7EFC, $473D3D7A, $AC6464C8, $E75D5DBA, $2B191932, $957373E6,
     $A06060C0, $98818119, $D14F4F9E, $7FDCDCA3, $66222244, $7E2A2A54, $AB90903B, $8388880B,
     $CA46468C, $29EEEEC7, $D3B8B86B, $3C141428, $79DEDEA7, $E25E5EBC, $1D0B0B16, $76DBDBAD,
     $3BE0E0DB, $56323264, $4E3A3A74, $1E0A0A14, $DB494992, $0A06060C, $6C242448, $E45C5CB8,
     $5DC2C29F, $6ED3D3BD, $EFACAC43, $A66262C4, $A8919139, $A4959531, $37E4E4D3, $8B7979F2,
     $32E7E7D5, $43C8C88B, $5937376E, $B76D6DDA, $8C8D8D01, $64D5D5B1, $D24E4E9C, $E0A9A949,
     $B46C6CD8, $FA5656AC, $07F4F4F3, $25EAEACF, $AF6565CA, $8E7A7AF4, $E9AEAE47, $18080810,
     $D5BABA6F, $887878F0, $6F25254A, $722E2E5C, $241C1C38, $F1A6A657, $C7B4B473, $51C6C697,
     $23E8E8CB, $7CDDDDA1, $9C7474E8, $211F1F3E, $DD4B4B96, $DCBDBD61, $868B8B0D, $858A8A0F,
     $907070E0, $423E3E7C, $C4B5B571, $AA6666CC, $D8484890, $05030306, $01F6F6F7, $120E0E1C,
     $A36161C2, $5F35356A, $F95757AE, $D0B9B969, $91868617, $58C1C199, $271D1D3A, $B99E9E27,
     $38E1E1D9, $13F8F8EB, $B398982B, $33111122, $BB6969D2, $70D9D9A9, $898E8E07, $A7949433,
     $B69B9B2D, $221E1E3C, $92878715, $20E9E9C9, $49CECE87, $FF5555AA, $78282850, $7ADFDFA5,
     $8F8C8C03, $F8A1A159, $80898909, $170D0D1A, $DABFBF65, $31E6E6D7, $C6424284, $B86868D0,
     $C3414182, $B0999929, $772D2D5A, $110F0F1E, $CBB0B07B, $FC5454A8, $D6BBBB6D, $3A16162C);

  AES_T1 : array[Byte] of Cardinal =
    ($6363C6A5, $7C7CF884, $7777EE99, $7B7BF68D, $F2F2FF0D, $6B6BD6BD, $6F6FDEB1, $C5C59154,
     $30306050, $01010203, $6767CEA9, $2B2B567D, $FEFEE719, $D7D7B562, $ABAB4DE6, $7676EC9A,
     $CACA8F45, $82821F9D, $C9C98940, $7D7DFA87, $FAFAEF15, $5959B2EB, $47478EC9, $F0F0FB0B,
     $ADAD41EC, $D4D4B367, $A2A25FFD, $AFAF45EA, $9C9C23BF, $A4A453F7, $7272E496, $C0C09B5B,
     $B7B775C2, $FDFDE11C, $93933DAE, $26264C6A, $36366C5A, $3F3F7E41, $F7F7F502, $CCCC834F,
     $3434685C, $A5A551F4, $E5E5D134, $F1F1F908, $7171E293, $D8D8AB73, $31316253, $15152A3F,
     $0404080C, $C7C79552, $23234665, $C3C39D5E, $18183028, $969637A1, $05050A0F, $9A9A2FB5,
     $07070E09, $12122436, $80801B9B, $E2E2DF3D, $EBEBCD26, $27274E69, $B2B27FCD, $7575EA9F,
     $0909121B, $83831D9E, $2C2C5874, $1A1A342E, $1B1B362D, $6E6EDCB2, $5A5AB4EE, $A0A05BFB,
     $5252A4F6, $3B3B764D, $D6D6B761, $B3B37DCE, $2929527B, $E3E3DD3E, $2F2F5E71, $84841397,
     $5353A6F5, $D1D1B968, $00000000, $EDEDC12C, $20204060, $FCFCE31F, $B1B179C8, $5B5BB6ED,
     $6A6AD4BE, $CBCB8D46, $BEBE67D9, $3939724B, $4A4A94DE, $4C4C98D4, $5858B0E8, $CFCF854A,
     $D0D0BB6B, $EFEFC52A, $AAAA4FE5, $FBFBED16, $434386C5, $4D4D9AD7, $33336655, $85851194,
     $45458ACF, $F9F9E910, $02020406, $7F7FFE81, $5050A0F0, $3C3C7844, $9F9F25BA, $A8A84BE3,
     $5151A2F3, $A3A35DFE, $404080C0, $8F8F058A, $92923FAD, $9D9D21BC, $38387048, $F5F5F104,
     $BCBC63DF, $B6B677C1, $DADAAF75, $21214263, $10102030, $FFFFE51A, $F3F3FD0E, $D2D2BF6D,
     $CDCD814C, $0C0C1814, $13132635, $ECECC32F, $5F5FBEE1, $979735A2, $444488CC, $17172E39,
     $C4C49357, $A7A755F2, $7E7EFC82, $3D3D7A47, $6464C8AC, $5D5DBAE7, $1919322B, $7373E695,
     $6060C0A0, $81811998, $4F4F9ED1, $DCDCA37F, $22224466, $2A2A547E, $90903BAB, $88880B83,
     $46468CCA, $EEEEC729, $B8B86BD3, $1414283C, $DEDEA779, $5E5EBCE2, $0B0B161D, $DBDBAD76,
     $E0E0DB3B, $32326456, $3A3A744E, $0A0A141E, $494992DB, $06060C0A, $2424486C, $5C5CB8E4,
     $C2C29F5D, $D3D3BD6E, $ACAC43EF, $6262C4A6, $919139A8, $959531A4, $E4E4D337, $7979F28B,
     $E7E7D532, $C8C88B43, $37376E59, $6D6DDAB7, $8D8D018C, $D5D5B164, $4E4E9CD2, $A9A949E0,
     $6C6CD8B4, $5656ACFA, $F4F4F307, $EAEACF25, $6565CAAF, $7A7AF48E, $AEAE47E9, $08081018,
     $BABA6FD5, $7878F088, $25254A6F, $2E2E5C72, $1C1C3824, $A6A657F1, $B4B473C7, $C6C69751,
     $E8E8CB23, $DDDDA17C, $7474E89C, $1F1F3E21, $4B4B96DD, $BDBD61DC, $8B8B0D86, $8A8A0F85,
     $7070E090, $3E3E7C42, $B5B571C4, $6666CCAA, $484890D8, $03030605, $F6F6F701, $0E0E1C12,
     $6161C2A3, $35356A5F, $5757AEF9, $B9B969D0, $86861791, $C1C19958, $1D1D3A27, $9E9E27B9,
     $E1E1D938, $F8F8EB13, $98982BB3, $11112233, $6969D2BB, $D9D9A970, $8E8E0789, $949433A7,
     $9B9B2DB6, $1E1E3C22, $87871592, $E9E9C920, $CECE8749, $5555AAFF, $28285078, $DFDFA57A,
     $8C8C038F, $A1A159F8, $89890980, $0D0D1A17, $BFBF65DA, $E6E6D731, $424284C6, $6868D0B8,
     $414182C3, $999929B0, $2D2D5A77, $0F0F1E11, $B0B07BCB, $5454A8FC, $BBBB6DD6, $16162C3A);

  AES_T2 : array[Byte] of Cardinal =
    ($63C6A563, $7CF8847C, $77EE9977, $7BF68D7B, $F2FF0DF2, $6BD6BD6B, $6FDEB16F, $C59154C5,
     $30605030, $01020301, $67CEA967, $2B567D2B, $FEE719FE, $D7B562D7, $AB4DE6AB, $76EC9A76,
     $CA8F45CA, $821F9D82, $C98940C9, $7DFA877D, $FAEF15FA, $59B2EB59, $478EC947, $F0FB0BF0,
     $AD41ECAD, $D4B367D4, $A25FFDA2, $AF45EAAF, $9C23BF9C, $A453F7A4, $72E49672, $C09B5BC0,
     $B775C2B7, $FDE11CFD, $933DAE93, $264C6A26, $366C5A36, $3F7E413F, $F7F502F7, $CC834FCC,
     $34685C34, $A551F4A5, $E5D134E5, $F1F908F1, $71E29371, $D8AB73D8, $31625331, $152A3F15,
     $04080C04, $C79552C7, $23466523, $C39D5EC3, $18302818, $9637A196, $050A0F05, $9A2FB59A,
     $070E0907, $12243612, $801B9B80, $E2DF3DE2, $EBCD26EB, $274E6927, $B27FCDB2, $75EA9F75,
     $09121B09, $831D9E83, $2C58742C, $1A342E1A, $1B362D1B, $6EDCB26E, $5AB4EE5A, $A05BFBA0,
     $52A4F652, $3B764D3B, $D6B761D6, $B37DCEB3, $29527B29, $E3DD3EE3, $2F5E712F, $84139784,
     $53A6F553, $D1B968D1, $00000000, $EDC12CED, $20406020, $FCE31FFC, $B179C8B1, $5BB6ED5B,
     $6AD4BE6A, $CB8D46CB, $BE67D9BE, $39724B39, $4A94DE4A, $4C98D44C, $58B0E858, $CF854ACF,
     $D0BB6BD0, $EFC52AEF, $AA4FE5AA, $FBED16FB, $4386C543, $4D9AD74D, $33665533, $85119485,
     $458ACF45, $F9E910F9, $02040602, $7FFE817F, $50A0F050, $3C78443C, $9F25BA9F, $A84BE3A8,
     $51A2F351, $A35DFEA3, $4080C040, $8F058A8F, $923FAD92, $9D21BC9D, $38704838, $F5F104F5,
     $BC63DFBC, $B677C1B6, $DAAF75DA, $21426321, $10203010, $FFE51AFF, $F3FD0EF3, $D2BF6DD2,
     $CD814CCD, $0C18140C, $13263513, $ECC32FEC, $5FBEE15F, $9735A297, $4488CC44, $172E3917,
     $C49357C4, $A755F2A7, $7EFC827E, $3D7A473D, $64C8AC64, $5DBAE75D, $19322B19, $73E69573,
     $60C0A060, $81199881, $4F9ED14F, $DCA37FDC, $22446622, $2A547E2A, $903BAB90, $880B8388,
     $468CCA46, $EEC729EE, $B86BD3B8, $14283C14, $DEA779DE, $5EBCE25E, $0B161D0B, $DBAD76DB,
     $E0DB3BE0, $32645632, $3A744E3A, $0A141E0A, $4992DB49, $060C0A06, $24486C24, $5CB8E45C,
     $C29F5DC2, $D3BD6ED3, $AC43EFAC, $62C4A662, $9139A891, $9531A495, $E4D337E4, $79F28B79,
     $E7D532E7, $C88B43C8, $376E5937, $6DDAB76D, $8D018C8D, $D5B164D5, $4E9CD24E, $A949E0A9,
     $6CD8B46C, $56ACFA56, $F4F307F4, $EACF25EA, $65CAAF65, $7AF48E7A, $AE47E9AE, $08101808,
     $BA6FD5BA, $78F08878, $254A6F25, $2E5C722E, $1C38241C, $A657F1A6, $B473C7B4, $C69751C6,
     $E8CB23E8, $DDA17CDD, $74E89C74, $1F3E211F, $4B96DD4B, $BD61DCBD, $8B0D868B, $8A0F858A,
     $70E09070, $3E7C423E, $B571C4B5, $66CCAA66, $4890D848, $03060503, $F6F701F6, $0E1C120E,
     $61C2A361, $356A5F35, $57AEF957, $B969D0B9, $86179186, $C19958C1, $1D3A271D, $9E27B99E,
     $E1D938E1, $F8EB13F8, $982BB398, $11223311, $69D2BB69, $D9A970D9, $8E07898E, $9433A794,
     $9B2DB69B, $1E3C221E, $87159287, $E9C920E9, $CE8749CE, $55AAFF55, $28507828, $DFA57ADF,
     $8C038F8C, $A159F8A1, $89098089, $0D1A170D, $BF65DABF, $E6D731E6, $4284C642, $68D0B868,
     $4182C341, $9929B099, $2D5A772D, $0F1E110F, $B07BCBB0, $54A8FC54, $BB6DD6BB, $162C3A16);

  AES_T3 : array[Byte] of Cardinal =
    ($C6A56363, $F8847C7C, $EE997777, $F68D7B7B, $FF0DF2F2, $D6BD6B6B, $DEB16F6F, $9154C5C5,
     $60503030, $02030101, $CEA96767, $567D2B2B, $E719FEFE, $B562D7D7, $4DE6ABAB, $EC9A7676,
     $8F45CACA, $1F9D8282, $8940C9C9, $FA877D7D, $EF15FAFA, $B2EB5959, $8EC94747, $FB0BF0F0,
     $41ECADAD, $B367D4D4, $5FFDA2A2, $45EAAFAF, $23BF9C9C, $53F7A4A4, $E4967272, $9B5BC0C0,
     $75C2B7B7, $E11CFDFD, $3DAE9393, $4C6A2626, $6C5A3636, $7E413F3F, $F502F7F7, $834FCCCC,
     $685C3434, $51F4A5A5, $D134E5E5, $F908F1F1, $E2937171, $AB73D8D8, $62533131, $2A3F1515,
     $080C0404, $9552C7C7, $46652323, $9D5EC3C3, $30281818, $37A19696, $0A0F0505, $2FB59A9A,
     $0E090707, $24361212, $1B9B8080, $DF3DE2E2, $CD26EBEB, $4E692727, $7FCDB2B2, $EA9F7575,
     $121B0909, $1D9E8383, $58742C2C, $342E1A1A, $362D1B1B, $DCB26E6E, $B4EE5A5A, $5BFBA0A0,
     $A4F65252, $764D3B3B, $B761D6D6, $7DCEB3B3, $527B2929, $DD3EE3E3, $5E712F2F, $13978484,
     $A6F55353, $B968D1D1, $00000000, $C12CEDED, $40602020, $E31FFCFC, $79C8B1B1, $B6ED5B5B,
     $D4BE6A6A, $8D46CBCB, $67D9BEBE, $724B3939, $94DE4A4A, $98D44C4C, $B0E85858, $854ACFCF,
     $BB6BD0D0, $C52AEFEF, $4FE5AAAA, $ED16FBFB, $86C54343, $9AD74D4D, $66553333, $11948585,
     $8ACF4545, $E910F9F9, $04060202, $FE817F7F, $A0F05050, $78443C3C, $25BA9F9F, $4BE3A8A8,
     $A2F35151, $5DFEA3A3, $80C04040, $058A8F8F, $3FAD9292, $21BC9D9D, $70483838, $F104F5F5,
     $63DFBCBC, $77C1B6B6, $AF75DADA, $42632121, $20301010, $E51AFFFF, $FD0EF3F3, $BF6DD2D2,
     $814CCDCD, $18140C0C, $26351313, $C32FECEC, $BEE15F5F, $35A29797, $88CC4444, $2E391717,
     $9357C4C4, $55F2A7A7, $FC827E7E, $7A473D3D, $C8AC6464, $BAE75D5D, $322B1919, $E6957373,
     $C0A06060, $19988181, $9ED14F4F, $A37FDCDC, $44662222, $547E2A2A, $3BAB9090, $0B838888,
     $8CCA4646, $C729EEEE, $6BD3B8B8, $283C1414, $A779DEDE, $BCE25E5E, $161D0B0B, $AD76DBDB,
     $DB3BE0E0, $64563232, $744E3A3A, $141E0A0A, $92DB4949, $0C0A0606, $486C2424, $B8E45C5C,
     $9F5DC2C2, $BD6ED3D3, $43EFACAC, $C4A66262, $39A89191, $31A49595, $D337E4E4, $F28B7979,
     $D532E7E7, $8B43C8C8, $6E593737, $DAB76D6D, $018C8D8D, $B164D5D5, $9CD24E4E, $49E0A9A9,
     $D8B46C6C, $ACFA5656, $F307F4F4, $CF25EAEA, $CAAF6565, $F48E7A7A, $47E9AEAE, $10180808,
     $6FD5BABA, $F0887878, $4A6F2525, $5C722E2E, $38241C1C, $57F1A6A6, $73C7B4B4, $9751C6C6,
     $CB23E8E8, $A17CDDDD, $E89C7474, $3E211F1F, $96DD4B4B, $61DCBDBD, $0D868B8B, $0F858A8A,
     $E0907070, $7C423E3E, $71C4B5B5, $CCAA6666, $90D84848, $06050303, $F701F6F6, $1C120E0E,
     $C2A36161, $6A5F3535, $AEF95757, $69D0B9B9, $17918686, $9958C1C1, $3A271D1D, $27B99E9E,
     $D938E1E1, $EB13F8F8, $2BB39898, $22331111, $D2BB6969, $A970D9D9, $07898E8E, $33A79494,
     $2DB69B9B, $3C221E1E, $15928787, $C920E9E9, $8749CECE, $AAFF5555, $50782828, $A57ADFDF,
     $038F8C8C, $59F8A1A1, $09808989, $1A170D0D, $65DABFBF, $D731E6E6, $84C64242, $D0B86868,
     $82C34141, $29B09999, $5A772D2D, $1E110F0F, $7BCBB0B0, $A8FC5454, $6DD6BBBB, $2C3A1616);

  AES_InvT0 : array[Byte] of Cardinal =
    ($00000000, $0B0D090E, $161A121C, $1D171B12, $2C342438, $27392D36, $3A2E3624, $31233F2A,
     $58684870, $5365417E, $4E725A6C, $457F5362, $745C6C48, $7F516546, $62467E54, $694B775A,
     $B0D090E0, $BBDD99EE, $A6CA82FC, $ADC78BF2, $9CE4B4D8, $97E9BDD6, $8AFEA6C4, $81F3AFCA,
     $E8B8D890, $E3B5D19E, $FEA2CA8C, $F5AFC382, $C48CFCA8, $CF81F5A6, $D296EEB4, $D99BE7BA,
     $7BBB3BDB, $70B632D5, $6DA129C7, $66AC20C9, $578F1FE3, $5C8216ED, $41950DFF, $4A9804F1,
     $23D373AB, $28DE7AA5, $35C961B7, $3EC468B9, $0FE75793, $04EA5E9D, $19FD458F, $12F04C81,
     $CB6BAB3B, $C066A235, $DD71B927, $D67CB029, $E75F8F03, $EC52860D, $F1459D1F, $FA489411,
     $9303E34B, $980EEA45, $8519F157, $8E14F859, $BF37C773, $B43ACE7D, $A92DD56F, $A220DC61,
     $F66D76AD, $FD607FA3, $E07764B1, $EB7A6DBF, $DA595295, $D1545B9B, $CC434089, $C74E4987,
     $AE053EDD, $A50837D3, $B81F2CC1, $B31225CF, $82311AE5, $893C13EB, $942B08F9, $9F2601F7,
     $46BDE64D, $4DB0EF43, $50A7F451, $5BAAFD5F, $6A89C275, $6184CB7B, $7C93D069, $779ED967,
     $1ED5AE3D, $15D8A733, $08CFBC21, $03C2B52F, $32E18A05, $39EC830B, $24FB9819, $2FF69117,
     $8DD64D76, $86DB4478, $9BCC5F6A, $90C15664, $A1E2694E, $AAEF6040, $B7F87B52, $BCF5725C,
     $D5BE0506, $DEB30C08, $C3A4171A, $C8A91E14, $F98A213E, $F2872830, $EF903322, $E49D3A2C,
     $3D06DD96, $360BD498, $2B1CCF8A, $2011C684, $1132F9AE, $1A3FF0A0, $0728EBB2, $0C25E2BC,
     $656E95E6, $6E639CE8, $737487FA, $78798EF4, $495AB1DE, $4257B8D0, $5F40A3C2, $544DAACC,
     $F7DAEC41, $FCD7E54F, $E1C0FE5D, $EACDF753, $DBEEC879, $D0E3C177, $CDF4DA65, $C6F9D36B,
     $AFB2A431, $A4BFAD3F, $B9A8B62D, $B2A5BF23, $83868009, $888B8907, $959C9215, $9E919B1B,
     $470A7CA1, $4C0775AF, $51106EBD, $5A1D67B3, $6B3E5899, $60335197, $7D244A85, $7629438B,
     $1F6234D1, $146F3DDF, $097826CD, $02752FC3, $335610E9, $385B19E7, $254C02F5, $2E410BFB,
     $8C61D79A, $876CDE94, $9A7BC586, $9176CC88, $A055F3A2, $AB58FAAC, $B64FE1BE, $BD42E8B0,
     $D4099FEA, $DF0496E4, $C2138DF6, $C91E84F8, $F83DBBD2, $F330B2DC, $EE27A9CE, $E52AA0C0,
     $3CB1477A, $37BC4E74, $2AAB5566, $21A65C68, $10856342, $1B886A4C, $069F715E, $0D927850,
     $64D90F0A, $6FD40604, $72C31D16, $79CE1418, $48ED2B32, $43E0223C, $5EF7392E, $55FA3020,
     $01B79AEC, $0ABA93E2, $17AD88F0, $1CA081FE, $2D83BED4, $268EB7DA, $3B99ACC8, $3094A5C6,
     $59DFD29C, $52D2DB92, $4FC5C080, $44C8C98E, $75EBF6A4, $7EE6FFAA, $63F1E4B8, $68FCEDB6,
     $B1670A0C, $BA6A0302, $A77D1810, $AC70111E, $9D532E34, $965E273A, $8B493C28, $80443526,
     $E90F427C, $E2024B72, $FF155060, $F418596E, $C53B6644, $CE366F4A, $D3217458, $D82C7D56,
     $7A0CA137, $7101A839, $6C16B32B, $671BBA25, $5638850F, $5D358C01, $40229713, $4B2F9E1D,
     $2264E947, $2969E049, $347EFB5B, $3F73F255, $0E50CD7F, $055DC471, $184ADF63, $1347D66D,
     $CADC31D7, $C1D138D9, $DCC623CB, $D7CB2AC5, $E6E815EF, $EDE51CE1, $F0F207F3, $FBFF0EFD,
     $92B479A7, $99B970A9, $84AE6BBB, $8FA362B5, $BE805D9F, $B58D5491, $A89A4F83, $A397468D);

  AES_InvT1 : array[Byte] of Cardinal =
    ($00000000, $0D090E0B, $1A121C16, $171B121D, $3424382C, $392D3627, $2E36243A, $233F2A31,
     $68487058, $65417E53, $725A6C4E, $7F536245, $5C6C4874, $5165467F, $467E5462, $4B775A69,
     $D090E0B0, $DD99EEBB, $CA82FCA6, $C78BF2AD, $E4B4D89C, $E9BDD697, $FEA6C48A, $F3AFCA81,
     $B8D890E8, $B5D19EE3, $A2CA8CFE, $AFC382F5, $8CFCA8C4, $81F5A6CF, $96EEB4D2, $9BE7BAD9,
     $BB3BDB7B, $B632D570, $A129C76D, $AC20C966, $8F1FE357, $8216ED5C, $950DFF41, $9804F14A,
     $D373AB23, $DE7AA528, $C961B735, $C468B93E, $E757930F, $EA5E9D04, $FD458F19, $F04C8112,
     $6BAB3BCB, $66A235C0, $71B927DD, $7CB029D6, $5F8F03E7, $52860DEC, $459D1FF1, $489411FA,
     $03E34B93, $0EEA4598, $19F15785, $14F8598E, $37C773BF, $3ACE7DB4, $2DD56FA9, $20DC61A2,
     $6D76ADF6, $607FA3FD, $7764B1E0, $7A6DBFEB, $595295DA, $545B9BD1, $434089CC, $4E4987C7,
     $053EDDAE, $0837D3A5, $1F2CC1B8, $1225CFB3, $311AE582, $3C13EB89, $2B08F994, $2601F79F,
     $BDE64D46, $B0EF434D, $A7F45150, $AAFD5F5B, $89C2756A, $84CB7B61, $93D0697C, $9ED96777,
     $D5AE3D1E, $D8A73315, $CFBC2108, $C2B52F03, $E18A0532, $EC830B39, $FB981924, $F691172F,
     $D64D768D, $DB447886, $CC5F6A9B, $C1566490, $E2694EA1, $EF6040AA, $F87B52B7, $F5725CBC,
     $BE0506D5, $B30C08DE, $A4171AC3, $A91E14C8, $8A213EF9, $872830F2, $903322EF, $9D3A2CE4,
     $06DD963D, $0BD49836, $1CCF8A2B, $11C68420, $32F9AE11, $3FF0A01A, $28EBB207, $25E2BC0C,
     $6E95E665, $639CE86E, $7487FA73, $798EF478, $5AB1DE49, $57B8D042, $40A3C25F, $4DAACC54,
     $DAEC41F7, $D7E54FFC, $C0FE5DE1, $CDF753EA, $EEC879DB, $E3C177D0, $F4DA65CD, $F9D36BC6,
     $B2A431AF, $BFAD3FA4, $A8B62DB9, $A5BF23B2, $86800983, $8B890788, $9C921595, $919B1B9E,
     $0A7CA147, $0775AF4C, $106EBD51, $1D67B35A, $3E58996B, $33519760, $244A857D, $29438B76,
     $6234D11F, $6F3DDF14, $7826CD09, $752FC302, $5610E933, $5B19E738, $4C02F525, $410BFB2E,
     $61D79A8C, $6CDE9487, $7BC5869A, $76CC8891, $55F3A2A0, $58FAACAB, $4FE1BEB6, $42E8B0BD,
     $099FEAD4, $0496E4DF, $138DF6C2, $1E84F8C9, $3DBBD2F8, $30B2DCF3, $27A9CEEE, $2AA0C0E5,
     $B1477A3C, $BC4E7437, $AB55662A, $A65C6821, $85634210, $886A4C1B, $9F715E06, $9278500D,
     $D90F0A64, $D406046F, $C31D1672, $CE141879, $ED2B3248, $E0223C43, $F7392E5E, $FA302055,
     $B79AEC01, $BA93E20A, $AD88F017, $A081FE1C, $83BED42D, $8EB7DA26, $99ACC83B, $94A5C630,
     $DFD29C59, $D2DB9252, $C5C0804F, $C8C98E44, $EBF6A475, $E6FFAA7E, $F1E4B863, $FCEDB668,
     $670A0CB1, $6A0302BA, $7D1810A7, $70111EAC, $532E349D, $5E273A96, $493C288B, $44352680,
     $0F427CE9, $024B72E2, $155060FF, $18596EF4, $3B6644C5, $366F4ACE, $217458D3, $2C7D56D8,
     $0CA1377A, $01A83971, $16B32B6C, $1BBA2567, $38850F56, $358C015D, $22971340, $2F9E1D4B,
     $64E94722, $69E04929, $7EFB5B34, $73F2553F, $50CD7F0E, $5DC47105, $4ADF6318, $47D66D13,
     $DC31D7CA, $D138D9C1, $C623CBDC, $CB2AC5D7, $E815EFE6, $E51CE1ED, $F207F3F0, $FF0EFDFB,
     $B479A792, $B970A999, $AE6BBB84, $A362B58F, $805D9FBE, $8D5491B5, $9A4F83A8, $97468DA3);

  AES_InvT2 : array[Byte] of Cardinal =
    ($00000000, $090E0B0D, $121C161A, $1B121D17, $24382C34, $2D362739, $36243A2E, $3F2A3123,
     $48705868, $417E5365, $5A6C4E72, $5362457F, $6C48745C, $65467F51, $7E546246, $775A694B,
     $90E0B0D0, $99EEBBDD, $82FCA6CA, $8BF2ADC7, $B4D89CE4, $BDD697E9, $A6C48AFE, $AFCA81F3,
     $D890E8B8, $D19EE3B5, $CA8CFEA2, $C382F5AF, $FCA8C48C, $F5A6CF81, $EEB4D296, $E7BAD99B,
     $3BDB7BBB, $32D570B6, $29C76DA1, $20C966AC, $1FE3578F, $16ED5C82, $0DFF4195, $04F14A98,
     $73AB23D3, $7AA528DE, $61B735C9, $68B93EC4, $57930FE7, $5E9D04EA, $458F19FD, $4C8112F0,
     $AB3BCB6B, $A235C066, $B927DD71, $B029D67C, $8F03E75F, $860DEC52, $9D1FF145, $9411FA48,
     $E34B9303, $EA45980E, $F1578519, $F8598E14, $C773BF37, $CE7DB43A, $D56FA92D, $DC61A220,
     $76ADF66D, $7FA3FD60, $64B1E077, $6DBFEB7A, $5295DA59, $5B9BD154, $4089CC43, $4987C74E,
     $3EDDAE05, $37D3A508, $2CC1B81F, $25CFB312, $1AE58231, $13EB893C, $08F9942B, $01F79F26,
     $E64D46BD, $EF434DB0, $F45150A7, $FD5F5BAA, $C2756A89, $CB7B6184, $D0697C93, $D967779E,
     $AE3D1ED5, $A73315D8, $BC2108CF, $B52F03C2, $8A0532E1, $830B39EC, $981924FB, $91172FF6,
     $4D768DD6, $447886DB, $5F6A9BCC, $566490C1, $694EA1E2, $6040AAEF, $7B52B7F8, $725CBCF5,
     $0506D5BE, $0C08DEB3, $171AC3A4, $1E14C8A9, $213EF98A, $2830F287, $3322EF90, $3A2CE49D,
     $DD963D06, $D498360B, $CF8A2B1C, $C6842011, $F9AE1132, $F0A01A3F, $EBB20728, $E2BC0C25,
     $95E6656E, $9CE86E63, $87FA7374, $8EF47879, $B1DE495A, $B8D04257, $A3C25F40, $AACC544D,
     $EC41F7DA, $E54FFCD7, $FE5DE1C0, $F753EACD, $C879DBEE, $C177D0E3, $DA65CDF4, $D36BC6F9,
     $A431AFB2, $AD3FA4BF, $B62DB9A8, $BF23B2A5, $80098386, $8907888B, $9215959C, $9B1B9E91,
     $7CA1470A, $75AF4C07, $6EBD5110, $67B35A1D, $58996B3E, $51976033, $4A857D24, $438B7629,
     $34D11F62, $3DDF146F, $26CD0978, $2FC30275, $10E93356, $19E7385B, $02F5254C, $0BFB2E41,
     $D79A8C61, $DE94876C, $C5869A7B, $CC889176, $F3A2A055, $FAACAB58, $E1BEB64F, $E8B0BD42,
     $9FEAD409, $96E4DF04, $8DF6C213, $84F8C91E, $BBD2F83D, $B2DCF330, $A9CEEE27, $A0C0E52A,
     $477A3CB1, $4E7437BC, $55662AAB, $5C6821A6, $63421085, $6A4C1B88, $715E069F, $78500D92,
     $0F0A64D9, $06046FD4, $1D1672C3, $141879CE, $2B3248ED, $223C43E0, $392E5EF7, $302055FA,
     $9AEC01B7, $93E20ABA, $88F017AD, $81FE1CA0, $BED42D83, $B7DA268E, $ACC83B99, $A5C63094,
     $D29C59DF, $DB9252D2, $C0804FC5, $C98E44C8, $F6A475EB, $FFAA7EE6, $E4B863F1, $EDB668FC,
     $0A0CB167, $0302BA6A, $1810A77D, $111EAC70, $2E349D53, $273A965E, $3C288B49, $35268044,
     $427CE90F, $4B72E202, $5060FF15, $596EF418, $6644C53B, $6F4ACE36, $7458D321, $7D56D82C,
     $A1377A0C, $A8397101, $B32B6C16, $BA25671B, $850F5638, $8C015D35, $97134022, $9E1D4B2F,
     $E9472264, $E0492969, $FB5B347E, $F2553F73, $CD7F0E50, $C471055D, $DF63184A, $D66D1347,
     $31D7CADC, $38D9C1D1, $23CBDCC6, $2AC5D7CB, $15EFE6E8, $1CE1EDE5, $07F3F0F2, $0EFDFBFF,
     $79A792B4, $70A999B9, $6BBB84AE, $62B58FA3, $5D9FBE80, $5491B58D, $4F83A89A, $468DA397);

  AES_InvT3 : array[Byte] of Cardinal =
    ($00000000, $0E0B0D09, $1C161A12, $121D171B, $382C3424, $3627392D, $243A2E36, $2A31233F,
     $70586848, $7E536541, $6C4E725A, $62457F53, $48745C6C, $467F5165, $5462467E, $5A694B77,
     $E0B0D090, $EEBBDD99, $FCA6CA82, $F2ADC78B, $D89CE4B4, $D697E9BD, $C48AFEA6, $CA81F3AF,
     $90E8B8D8, $9EE3B5D1, $8CFEA2CA, $82F5AFC3, $A8C48CFC, $A6CF81F5, $B4D296EE, $BAD99BE7,
     $DB7BBB3B, $D570B632, $C76DA129, $C966AC20, $E3578F1F, $ED5C8216, $FF41950D, $F14A9804,
     $AB23D373, $A528DE7A, $B735C961, $B93EC468, $930FE757, $9D04EA5E, $8F19FD45, $8112F04C,
     $3BCB6BAB, $35C066A2, $27DD71B9, $29D67CB0, $03E75F8F, $0DEC5286, $1FF1459D, $11FA4894,
     $4B9303E3, $45980EEA, $578519F1, $598E14F8, $73BF37C7, $7DB43ACE, $6FA92DD5, $61A220DC,
     $ADF66D76, $A3FD607F, $B1E07764, $BFEB7A6D, $95DA5952, $9BD1545B, $89CC4340, $87C74E49,
     $DDAE053E, $D3A50837, $C1B81F2C, $CFB31225, $E582311A, $EB893C13, $F9942B08, $F79F2601,
     $4D46BDE6, $434DB0EF, $5150A7F4, $5F5BAAFD, $756A89C2, $7B6184CB, $697C93D0, $67779ED9,
     $3D1ED5AE, $3315D8A7, $2108CFBC, $2F03C2B5, $0532E18A, $0B39EC83, $1924FB98, $172FF691,
     $768DD64D, $7886DB44, $6A9BCC5F, $6490C156, $4EA1E269, $40AAEF60, $52B7F87B, $5CBCF572,
     $06D5BE05, $08DEB30C, $1AC3A417, $14C8A91E, $3EF98A21, $30F28728, $22EF9033, $2CE49D3A,
     $963D06DD, $98360BD4, $8A2B1CCF, $842011C6, $AE1132F9, $A01A3FF0, $B20728EB, $BC0C25E2,
     $E6656E95, $E86E639C, $FA737487, $F478798E, $DE495AB1, $D04257B8, $C25F40A3, $CC544DAA,
     $41F7DAEC, $4FFCD7E5, $5DE1C0FE, $53EACDF7, $79DBEEC8, $77D0E3C1, $65CDF4DA, $6BC6F9D3,
     $31AFB2A4, $3FA4BFAD, $2DB9A8B6, $23B2A5BF, $09838680, $07888B89, $15959C92, $1B9E919B,
     $A1470A7C, $AF4C0775, $BD51106E, $B35A1D67, $996B3E58, $97603351, $857D244A, $8B762943,
     $D11F6234, $DF146F3D, $CD097826, $C302752F, $E9335610, $E7385B19, $F5254C02, $FB2E410B,
     $9A8C61D7, $94876CDE, $869A7BC5, $889176CC, $A2A055F3, $ACAB58FA, $BEB64FE1, $B0BD42E8,
     $EAD4099F, $E4DF0496, $F6C2138D, $F8C91E84, $D2F83DBB, $DCF330B2, $CEEE27A9, $C0E52AA0,
     $7A3CB147, $7437BC4E, $662AAB55, $6821A65C, $42108563, $4C1B886A, $5E069F71, $500D9278,
     $0A64D90F, $046FD406, $1672C31D, $1879CE14, $3248ED2B, $3C43E022, $2E5EF739, $2055FA30,
     $EC01B79A, $E20ABA93, $F017AD88, $FE1CA081, $D42D83BE, $DA268EB7, $C83B99AC, $C63094A5,
     $9C59DFD2, $9252D2DB, $804FC5C0, $8E44C8C9, $A475EBF6, $AA7EE6FF, $B863F1E4, $B668FCED,
     $0CB1670A, $02BA6A03, $10A77D18, $1EAC7011, $349D532E, $3A965E27, $288B493C, $26804435,
     $7CE90F42, $72E2024B, $60FF1550, $6EF41859, $44C53B66, $4ACE366F, $58D32174, $56D82C7D,
     $377A0CA1, $397101A8, $2B6C16B3, $25671BBA, $0F563885, $015D358C, $13402297, $1D4B2F9E,
     $472264E9, $492969E0, $5B347EFB, $553F73F2, $7F0E50CD, $71055DC4, $63184ADF, $6D1347D6,
     $D7CADC31, $D9C1D138, $CBDCC623, $C5D7CB2A, $EFE6E815, $E1EDE51C, $F3F0F207, $FDFBFF0E,
     $A792B479, $A999B970, $BB84AE6B, $B58FA362, $9FBE805D, $91B58D54, $83A89A4F, $8DA39746);

  AES_InvT4 : array[Byte] of Cardinal =
   ($50a7f451, $5365417e, $c3a4171a, $965e273a, $cb6bab3b, $f1459d1f, $ab58faac, $9303e34b,
    $55fa3020, $f66d76ad, $9176cc88, $254c02f5, $fcd7e54f, $d7cb2ac5, $80443526, $8fa362b5,
    $495ab1de, $671bba25, $980eea45, $e1c0fe5d, $02752fc3, $12f04c81, $a397468d, $c6f9d36b,
    $e75f8f03, $959c9215, $eb7a6dbf, $da595295, $2d83bed4, $d3217458, $2969e049, $44c8c98e,
    $6a89c275, $78798ef4, $6b3e5899, $dd71b927, $b64fe1be, $17ad88f0, $66ac20c9, $b43ace7d,
    $184adf63, $82311ae5, $60335197, $457f5362, $e07764b1, $84ae6bbb, $1ca081fe, $942b08f9,
    $58684870, $19fd458f, $876cde94, $b7f87b52, $23d373ab, $e2024b72, $578f1fe3, $2aab5566,
    $0728ebb2, $03c2b52f, $9a7bc586, $a50837d3, $f2872830, $b2a5bf23, $ba6a0302, $5c8216ed,
    $2b1ccf8a, $92b479a7, $f0f207f3, $a1e2694e, $cdf4da65, $d5be0506, $1f6234d1, $8afea6c4,
    $9d532e34, $a055f3a2, $32e18a05, $75ebf6a4, $39ec830b, $aaef6040, $069f715e, $51106ebd,
    $f98a213e, $3d06dd96, $ae053edd, $46bde64d, $b58d5491, $055dc471, $6fd40604, $ff155060,
    $24fb9819, $97e9bdd6, $cc434089, $779ed967, $bd42e8b0, $888b8907, $385b19e7, $dbeec879,
    $470a7ca1, $e90f427c, $c91e84f8, $00000000, $83868009, $48ed2b32, $ac70111e, $4e725a6c,
    $fbff0efd, $5638850f, $1ed5ae3d, $27392d36, $64d90f0a, $21a65c68, $d1545b9b, $3a2e3624,
    $b1670a0c, $0fe75793, $d296eeb4, $9e919b1b, $4fc5c080, $a220dc61, $694b775a, $161a121c,
    $0aba93e2, $e52aa0c0, $43e0223c, $1d171b12, $0b0d090e, $adc78bf2, $b9a8b62d, $c8a91e14,
    $8519f157, $4c0775af, $bbdd99ee, $fd607fa3, $9f2601f7, $bcf5725c, $c53b6644, $347efb5b,
    $7629438b, $dcc623cb, $68fcedb6, $63f1e4b8, $cadc31d7, $10856342, $40229713, $2011c684,
    $7d244a85, $f83dbbd2, $1132f9ae, $6da129c7, $4b2f9e1d, $f330b2dc, $ec52860d, $d0e3c177,
    $6c16b32b, $99b970a9, $fa489411, $2264e947, $c48cfca8, $1a3ff0a0, $d82c7d56, $ef903322,
    $c74e4987, $c1d138d9, $fea2ca8c, $360bd498, $cf81f5a6, $28de7aa5, $268eb7da, $a4bfad3f,
    $e49d3a2c, $0d927850, $9bcc5f6a, $62467e54, $c2138df6, $e8b8d890, $5ef7392e, $f5afc382,
    $be805d9f, $7c93d069, $a92dd56f, $b31225cf, $3b99acc8, $a77d1810, $6e639ce8, $7bbb3bdb,
    $097826cd, $f418596e, $01b79aec, $a89a4f83, $656e95e6, $7ee6ffaa, $08cfbc21, $e6e815ef,
    $d99be7ba, $ce366f4a, $d4099fea, $d67cb029, $afb2a431, $31233f2a, $3094a5c6, $c066a235,
    $37bc4e74, $a6ca82fc, $b0d090e0, $15d8a733, $4a9804f1, $f7daec41, $0e50cd7f, $2ff69117,
    $8dd64d76, $4db0ef43, $544daacc, $df0496e4, $e3b5d19e, $1b886a4c, $b81f2cc1, $7f516546,
    $04ea5e9d, $5d358c01, $737487fa, $2e410bfb, $5a1d67b3, $52d2db92, $335610e9, $1347d66d,
    $8c61d79a, $7a0ca137, $8e14f859, $893c13eb, $ee27a9ce, $35c961b7, $ede51ce1, $3cb1477a,
    $59dfd29c, $3f73f255, $79ce1418, $bf37c773, $eacdf753, $5baafd5f, $146f3ddf, $86db4478,
    $81f3afca, $3ec468b9, $2c342438, $5f40a3c2, $72c31d16, $0c25e2bc, $8b493c28, $41950dff,
    $7101a839, $deb30c08, $9ce4b4d8, $90c15664, $6184cb7b, $70b632d5, $745c6c48, $4257b8d0);

  AES_InvT5 : array[Byte] of Cardinal =
   ($a7f45150, $65417e53, $a4171ac3, $5e273a96, $6bab3bcb, $459d1ff1, $58faacab, $03e34b93,
    $fa302055, $6d76adf6, $76cc8891, $4c02f525, $d7e54ffc, $cb2ac5d7, $44352680, $a362b58f,
    $5ab1de49, $1bba2567, $0eea4598, $c0fe5de1, $752fc302, $f04c8112, $97468da3, $f9d36bc6,
    $5f8f03e7, $9c921595, $7a6dbfeb, $595295da, $83bed42d, $217458d3, $69e04929, $c8c98e44,
    $89c2756a, $798ef478, $3e58996b, $71b927dd, $4fe1beb6, $ad88f017, $ac20c966, $3ace7db4,
    $4adf6318, $311ae582, $33519760, $7f536245, $7764b1e0, $ae6bbb84, $a081fe1c, $2b08f994,
    $68487058, $fd458f19, $6cde9487, $f87b52b7, $d373ab23, $024b72e2, $8f1fe357, $ab55662a,
    $28ebb207, $c2b52f03, $7bc5869a, $0837d3a5, $872830f2, $a5bf23b2, $6a0302ba, $8216ed5c,
    $1ccf8a2b, $b479a792, $f207f3f0, $e2694ea1, $f4da65cd, $be0506d5, $6234d11f, $fea6c48a,
    $532e349d, $55f3a2a0, $e18a0532, $ebf6a475, $ec830b39, $ef6040aa, $9f715e06, $106ebd51,
    $8a213ef9, $06dd963d, $053eddae, $bde64d46, $8d5491b5, $5dc47105, $d406046f, $155060ff,
    $fb981924, $e9bdd697, $434089cc, $9ed96777, $42e8b0bd, $8b890788, $5b19e738, $eec879db,
    $0a7ca147, $0f427ce9, $1e84f8c9, $00000000, $86800983, $ed2b3248, $70111eac, $725a6c4e,
    $ff0efdfb, $38850f56, $d5ae3d1e, $392d3627, $d90f0a64, $a65c6821, $545b9bd1, $2e36243a,
    $670a0cb1, $e757930f, $96eeb4d2, $919b1b9e, $c5c0804f, $20dc61a2, $4b775a69, $1a121c16,
    $ba93e20a, $2aa0c0e5, $e0223c43, $171b121d, $0d090e0b, $c78bf2ad, $a8b62db9, $a91e14c8,
    $19f15785, $0775af4c, $dd99eebb, $607fa3fd, $2601f79f, $f5725cbc, $3b6644c5, $7efb5b34,
    $29438b76, $c623cbdc, $fcedb668, $f1e4b863, $dc31d7ca, $85634210, $22971340, $11c68420,
    $244a857d, $3dbbd2f8, $32f9ae11, $a129c76d, $2f9e1d4b, $30b2dcf3, $52860dec, $e3c177d0,
    $16b32b6c, $b970a999, $489411fa, $64e94722, $8cfca8c4, $3ff0a01a, $2c7d56d8, $903322ef,
    $4e4987c7, $d138d9c1, $a2ca8cfe, $0bd49836, $81f5a6cf, $de7aa528, $8eb7da26, $bfad3fa4,
    $9d3a2ce4, $9278500d, $cc5f6a9b, $467e5462, $138df6c2, $b8d890e8, $f7392e5e, $afc382f5,
    $805d9fbe, $93d0697c, $2dd56fa9, $1225cfb3, $99acc83b, $7d1810a7, $639ce86e, $bb3bdb7b,
    $7826cd09, $18596ef4, $b79aec01, $9a4f83a8, $6e95e665, $e6ffaa7e, $cfbc2108, $e815efe6,
    $9be7bad9, $366f4ace, $099fead4, $7cb029d6, $b2a431af, $233f2a31, $94a5c630, $66a235c0,
    $bc4e7437, $ca82fca6, $d090e0b0, $d8a73315, $9804f14a, $daec41f7, $50cd7f0e, $f691172f,
    $d64d768d, $b0ef434d, $4daacc54, $0496e4df, $b5d19ee3, $886a4c1b, $1f2cc1b8, $5165467f,
    $ea5e9d04, $358c015d, $7487fa73, $410bfb2e, $1d67b35a, $d2db9252, $5610e933, $47d66d13,
    $61d79a8c, $0ca1377a, $14f8598e, $3c13eb89, $27a9ceee, $c961b735, $e51ce1ed, $b1477a3c,
    $dfd29c59, $73f2553f, $ce141879, $37c773bf, $cdf753ea, $aafd5f5b, $6f3ddf14, $db447886,
    $f3afca81, $c468b93e, $3424382c, $40a3c25f, $c31d1672, $25e2bc0c, $493c288b, $950dff41,
    $01a83971, $b30c08de, $e4b4d89c, $c1566490, $84cb7b61, $b632d570, $5c6c4874, $57b8d042);

  AES_InvT6 : array[Byte] of Cardinal =
   ($f45150a7, $417e5365, $171ac3a4, $273a965e, $ab3bcb6b, $9d1ff145, $faacab58, $e34b9303,
    $302055fa, $76adf66d, $cc889176, $02f5254c, $e54ffcd7, $2ac5d7cb, $35268044, $62b58fa3,
    $b1de495a, $ba25671b, $ea45980e, $fe5de1c0, $2fc30275, $4c8112f0, $468da397, $d36bc6f9,
    $8f03e75f, $9215959c, $6dbfeb7a, $5295da59, $bed42d83, $7458d321, $e0492969, $c98e44c8,
    $c2756a89, $8ef47879, $58996b3e, $b927dd71, $e1beb64f, $88f017ad, $20c966ac, $ce7db43a,
    $df63184a, $1ae58231, $51976033, $5362457f, $64b1e077, $6bbb84ae, $81fe1ca0, $08f9942b,
    $48705868, $458f19fd, $de94876c, $7b52b7f8, $73ab23d3, $4b72e202, $1fe3578f, $55662aab,
    $ebb20728, $b52f03c2, $c5869a7b, $37d3a508, $2830f287, $bf23b2a5, $0302ba6a, $16ed5c82,
    $cf8a2b1c, $79a792b4, $07f3f0f2, $694ea1e2, $da65cdf4, $0506d5be, $34d11f62, $a6c48afe,
    $2e349d53, $f3a2a055, $8a0532e1, $f6a475eb, $830b39ec, $6040aaef, $715e069f, $6ebd5110,
    $213ef98a, $dd963d06, $3eddae05, $e64d46bd, $5491b58d, $c471055d, $06046fd4, $5060ff15,
    $981924fb, $bdd697e9, $4089cc43, $d967779e, $e8b0bd42, $8907888b, $19e7385b, $c879dbee,
    $7ca1470a, $427ce90f, $84f8c91e, $00000000, $80098386, $2b3248ed, $111eac70, $5a6c4e72,
    $0efdfbff, $850f5638, $ae3d1ed5, $2d362739, $0f0a64d9, $5c6821a6, $5b9bd154, $36243a2e,
    $0a0cb167, $57930fe7, $eeb4d296, $9b1b9e91, $c0804fc5, $dc61a220, $775a694b, $121c161a,
    $93e20aba, $a0c0e52a, $223c43e0, $1b121d17, $090e0b0d, $8bf2adc7, $b62db9a8, $1e14c8a9,
    $f1578519, $75af4c07, $99eebbdd, $7fa3fd60, $01f79f26, $725cbcf5, $6644c53b, $fb5b347e,
    $438b7629, $23cbdcc6, $edb668fc, $e4b863f1, $31d7cadc, $63421085, $97134022, $c6842011,
    $4a857d24, $bbd2f83d, $f9ae1132, $29c76da1, $9e1d4b2f, $b2dcf330, $860dec52, $c177d0e3,
    $b32b6c16, $70a999b9, $9411fa48, $e9472264, $fca8c48c, $f0a01a3f, $7d56d82c, $3322ef90,
    $4987c74e, $38d9c1d1, $ca8cfea2, $d498360b, $f5a6cf81, $7aa528de, $b7da268e, $ad3fa4bf,
    $3a2ce49d, $78500d92, $5f6a9bcc, $7e546246, $8df6c213, $d890e8b8, $392e5ef7, $c382f5af,
    $5d9fbe80, $d0697c93, $d56fa92d, $25cfb312, $acc83b99, $1810a77d, $9ce86e63, $3bdb7bbb,
    $26cd0978, $596ef418, $9aec01b7, $4f83a89a, $95e6656e, $ffaa7ee6, $bc2108cf, $15efe6e8,
    $e7bad99b, $6f4ace36, $9fead409, $b029d67c, $a431afb2, $3f2a3123, $a5c63094, $a235c066,
    $4e7437bc, $82fca6ca, $90e0b0d0, $a73315d8, $04f14a98, $ec41f7da, $cd7f0e50, $91172ff6,
    $4d768dd6, $ef434db0, $aacc544d, $96e4df04, $d19ee3b5, $6a4c1b88, $2cc1b81f, $65467f51,
    $5e9d04ea, $8c015d35, $87fa7374, $0bfb2e41, $67b35a1d, $db9252d2, $10e93356, $d66d1347,
    $d79a8c61, $a1377a0c, $f8598e14, $13eb893c, $a9ceee27, $61b735c9, $1ce1ede5, $477a3cb1,
    $d29c59df, $f2553f73, $141879ce, $c773bf37, $f753eacd, $fd5f5baa, $3ddf146f, $447886db,
    $afca81f3, $68b93ec4, $24382c34, $a3c25f40, $1d1672c3, $e2bc0c25, $3c288b49, $0dff4195,
    $a8397101, $0c08deb3, $b4d89ce4, $566490c1, $cb7b6184, $32d570b6, $6c48745c, $b8d04257);

  AES_InvT7 : array[Byte] of Cardinal =
   ($5150a7f4, $7e536541, $1ac3a417, $3a965e27, $3bcb6bab, $1ff1459d, $acab58fa, $4b9303e3,
    $2055fa30, $adf66d76, $889176cc, $f5254c02, $4ffcd7e5, $c5d7cb2a, $26804435, $b58fa362,
    $de495ab1, $25671bba, $45980eea, $5de1c0fe, $c302752f, $8112f04c, $8da39746, $6bc6f9d3,
    $03e75f8f, $15959c92, $bfeb7a6d, $95da5952, $d42d83be, $58d32174, $492969e0, $8e44c8c9,
    $756a89c2, $f478798e, $996b3e58, $27dd71b9, $beb64fe1, $f017ad88, $c966ac20, $7db43ace,
    $63184adf, $e582311a, $97603351, $62457f53, $b1e07764, $bb84ae6b, $fe1ca081, $f9942b08,
    $70586848, $8f19fd45, $94876cde, $52b7f87b, $ab23d373, $72e2024b, $e3578f1f, $662aab55,
    $b20728eb, $2f03c2b5, $869a7bc5, $d3a50837, $30f28728, $23b2a5bf, $02ba6a03, $ed5c8216,
    $8a2b1ccf, $a792b479, $f3f0f207, $4ea1e269, $65cdf4da, $06d5be05, $d11f6234, $c48afea6,
    $349d532e, $a2a055f3, $0532e18a, $a475ebf6, $0b39ec83, $40aaef60, $5e069f71, $bd51106e,
    $3ef98a21, $963d06dd, $ddae053e, $4d46bde6, $91b58d54, $71055dc4, $046fd406, $60ff1550,
    $1924fb98, $d697e9bd, $89cc4340, $67779ed9, $b0bd42e8, $07888b89, $e7385b19, $79dbeec8,
    $a1470a7c, $7ce90f42, $f8c91e84, $00000000, $09838680, $3248ed2b, $1eac7011, $6c4e725a,
    $fdfbff0e, $0f563885, $3d1ed5ae, $3627392d, $0a64d90f, $6821a65c, $9bd1545b, $243a2e36,
    $0cb1670a, $930fe757, $b4d296ee, $1b9e919b, $804fc5c0, $61a220dc, $5a694b77, $1c161a12,
    $e20aba93, $c0e52aa0, $3c43e022, $121d171b, $0e0b0d09, $f2adc78b, $2db9a8b6, $14c8a91e,
    $578519f1, $af4c0775, $eebbdd99, $a3fd607f, $f79f2601, $5cbcf572, $44c53b66, $5b347efb,
    $8b762943, $cbdcc623, $b668fced, $b863f1e4, $d7cadc31, $42108563, $13402297, $842011c6,
    $857d244a, $d2f83dbb, $ae1132f9, $c76da129, $1d4b2f9e, $dcf330b2, $0dec5286, $77d0e3c1,
    $2b6c16b3, $a999b970, $11fa4894, $472264e9, $a8c48cfc, $a01a3ff0, $56d82c7d, $22ef9033,
    $87c74e49, $d9c1d138, $8cfea2ca, $98360bd4, $a6cf81f5, $a528de7a, $da268eb7, $3fa4bfad,
    $2ce49d3a, $500d9278, $6a9bcc5f, $5462467e, $f6c2138d, $90e8b8d8, $2e5ef739, $82f5afc3,
    $9fbe805d, $697c93d0, $6fa92dd5, $cfb31225, $c83b99ac, $10a77d18, $e86e639c, $db7bbb3b,
    $cd097826, $6ef41859, $ec01b79a, $83a89a4f, $e6656e95, $aa7ee6ff, $2108cfbc, $efe6e815,
    $bad99be7, $4ace366f, $ead4099f, $29d67cb0, $31afb2a4, $2a31233f, $c63094a5, $35c066a2,
    $7437bc4e, $fca6ca82, $e0b0d090, $3315d8a7, $f14a9804, $41f7daec, $7f0e50cd, $172ff691,
    $768dd64d, $434db0ef, $cc544daa, $e4df0496, $9ee3b5d1, $4c1b886a, $c1b81f2c, $467f5165,
    $9d04ea5e, $015d358c, $fa737487, $fb2e410b, $b35a1d67, $9252d2db, $e9335610, $6d1347d6,
    $9a8c61d7, $377a0ca1, $598e14f8, $eb893c13, $ceee27a9, $b735c961, $e1ede51c, $7a3cb147,
    $9c59dfd2, $553f73f2, $1879ce14, $73bf37c7, $53eacdf7, $5f5baafd, $df146f3d, $7886db44,
    $ca81f3af, $b93ec468, $382c3424, $c25f40a3, $1672c31d, $bc0c25e2, $288b493c, $ff41950d,
    $397101a8, $08deb30c, $d89ce4b4, $6490c156, $7b6184cb, $d570b632, $48745c6c, $d04257b8);

 Alogtable : array[0..255] of Byte = (
    1,   3,   5,  15,  17,  51,  85, 255,  26,  46, 114, 150, 161, 248,  19,  53,
   95, 225,  56,  72, 216, 115, 149, 164, 247,   2,   6,  10,  30,  34, 102, 170,
  229,  52,  92, 228,  55,  89, 235,  38, 106, 190, 217, 112, 144, 171, 230,  49,
   83, 245,   4,  12,  20,  60,  68, 204,  79, 209, 104, 184, 211, 110, 178, 205,
   76, 212, 103, 169, 224,  59,  77, 215,  98, 166, 241,   8,  24,  40, 120, 136,
  131, 158, 185, 208, 107, 189, 220, 127, 129, 152, 179, 206,  73, 219, 118, 154,
  181, 196,  87, 249,  16,  48,  80, 240,  11,  29,  39, 105, 187, 214,  97, 163,
  254,  25,  43, 125, 135, 146, 173, 236,  47, 113, 147, 174, 233,  32,  96, 160,
  251,  22,  58,  78, 210, 109, 183, 194,  93, 231,  50,  86, 250,  21,  63,  65,
  195,  94, 226,  61,  71, 201,  64, 192,  91, 237,  44, 116, 156, 191, 218, 117,
  159, 186, 213, 100, 172, 239,  42, 126, 130, 157, 188, 223, 122, 142, 137, 128,
  155, 182, 193,  88, 232,  35, 101, 175, 234,  37, 111, 177, 200,  67, 197,  84,
  252,  31,  33,  99, 165, 244,   7,   9,  27,  45, 119, 153, 176, 203,  70, 202,
   69, 207,  74, 222, 121, 139, 134, 145, 168, 227,  62,  66, 198,  81, 243,  14,
   18,  54,  90, 238,  41, 123, 141, 140, 143, 138, 133, 148, 167, 242,  13,  23,
   57,  75, 221, 124, 132, 151, 162, 253,  28,  36, 108, 180, 199,  82, 246,   1);

 Logtable : array[0..255] of Byte = (
    0,   0,  25,   1,  50,   2,  26, 198,  75, 199,  27, 104,  51, 238, 223,   3,
  100,   4, 224,  14,  52, 141, 129, 239,  76, 113,   8, 200, 248, 105,  28, 193,
  125, 194,  29, 181, 249, 185,  39, 106,  77, 228, 166, 114, 154, 201,   9, 120,
  101,  47, 138,   5,  33,  15, 225,  36,  18, 240, 130,  69,  53, 147, 218, 142,
  150, 143, 219, 189,  54, 208, 206, 148,  19,  92, 210, 241,  64,  70, 131,  56,
  102, 221, 253,  48, 191,   6, 139,  98, 179,  37, 226, 152,  34, 136, 145,  16,
  126, 110,  72, 195, 163, 182,  30,  66,  58, 107,  40,  84, 250, 133,  61, 186,
   43, 121,  10,  21, 155, 159,  94, 202,  78, 212, 172, 229, 243, 115, 167,  87,
  175,  88, 168,  80, 244, 234, 214, 116,  79, 174, 233, 213, 231, 230, 173, 232,
   44, 215, 117, 122, 235,  22,  11, 245,  89, 203,  95, 176, 156, 169,  81, 160,
  127,  12, 246, 111,  23, 196,  73, 236, 216,  67,  31,  45, 164, 118, 123, 183,
  204, 187,  62,  90, 251,  96, 177, 134,  59,  82, 161, 108, 170,  85,  41, 157,
  151, 178, 135, 144,  97, 190, 220, 252, 188, 149, 207, 205,  55,  63,  91, 209,
   83,  57, 132,  60,  65, 162, 109,  71,  20,  42, 158,  93,  86, 242, 211, 171,
   68,  17, 146, 217,  35,  32,  46, 137, 180, 124, 184,  38, 119, 153, 227, 165,
  103,  74, 237, 222, 197,  49, 254,  24,  13,  99, 140, 128, 192, 247, 112,   7);

function AESSubVector(v : TAESVector) : TAESVector;
  { S-Box substitution }
begin
  Result.bt[0] := AESSBox[v.bt[0]];
  Result.bt[1] := AESSBox[v.bt[1]];
  Result.bt[2] := AESSBox[v.bt[2]];
  Result.bt[3] := AESSBox[v.bt[3]];
end;

function AESRotateVector(v : TAESVector; Count : Byte) : TAESVector;
  { rotate vector (count bytes) to the right, e.g. }
  { |3 2 1 0| -> |0 3 2 1| for Count = 1 }
var
  i : Byte;
begin
  i := Count mod 4;
  Result.bt[0] := v.bt[i];
  Result.bt[1] := v.bt[(i+1) mod 4];
  Result.bt[2] := v.bt[(i+2) mod 4];
  Result.bt[3] := v.bt[(i+3) mod 4];
end;

var
 pDKCContext : ^TAESContext;
 AlignedContext : array of Byte;

//Author:            Dennis Kjaer Christensen
//Date:              10/8 2004
//Instructionset(s): IA32

procedure MakeContextsAES_DKC_PAS_17(const Key; KeySize : Cardinal);
var
 I, TAESContextSize, J, ContextOffSet : Integer;
 Nk : Byte;
 Temp : TAESVector;
 Sk : Longint;
 ContextAddress, ContextAddressAlignment : Cardinal;

begin
 TAESContextSize := SizeOf(TAESContext);
 SetLength(AlignedContext, TAESContextSize+15);//Make room for alignment
 ContextOffSet := -1;
 repeat
  Inc(ContextOffset);
  ContextAddress := Cardinal(@AlignedContext[ContextOffset]);
  ContextAddressAlignment := ContextAddress and $0000000F;
 until(ContextAddressAlignment = 0);
 for J := 0 to TAESContextSize+15-1 do
  begin
   AlignedContext[J] := 0;
  end;
 pDKCContext := @AlignedContext[ContextOffSet]; //Align context
 //Sk := Min(KeySize, SizeOf(DKCContext.RoundKeys.Rk));
 Sk := KeySize;
 Move(Key, pDKCContext.RoundKeys.Rk, Sk * 4);
 case KeySize of
  4 : Nk := 4;
  6 : Nk := 6;
  8 : NK := 8;
 else
  raise Exception.Create('Invalid Key size');
 end;
 pDKCContext.Rounds := 6 + Nk;
 pDKCContext.KeySize := KeySize;
 //Expand key into round keys
 for i := Nk to (4 * (pDKCContext.Rounds + 1)) do
  begin
   temp := pDKCContext.RoundKeys.W[i-1];
   if (Nk in [AESNk128, AESNk192]) then
    begin
     if (i mod Nk) = 0 then
      Temp.dw := AESSubVector(AESRotateVector(Temp, 1)).dw xor RCon[i div Nk];
     pDKCContext.RoundKeys.W[i].dw := pDKCContext.RoundKeys.W[i - Nk].dw xor temp.dw;
    end
   else  //Nk = AESNk256
    begin
     if (I mod Nk) = 0 then
      Temp.dw := AESSubVector(AESRotateVector(Temp, 1)).dw xor RCon[i div Nk]
     else if (I mod Nk) = 4 then
      Temp := AESSubVector(Temp);
     pDKCContext.RoundKeys.W[i].dw := pDKCContext.RoundKeys.W[i - Nk].dw xor Temp.dw;
    end;
  end;
end;

//Author:            Dennis Kjaer Christensen
//Date:              10/8 2004
//Instructionset(s): IA32

procedure EncryptAES_DKC_PAS_17(var Block);
var
 I, IMax : Integer;
 Icardinal : Cardinal;
 pBlock : ^TAESBlock;
 P1, P2, P3 : ^Cardinal;
 C0, C1, C2, C3 : Cardinal;
 E5 : array[0..3] of TAESVector;
 E6 : TAESVectors;

begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[0];
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
 Icardinal := 3;
 for I := 1 to IMax do
  begin
   Inc(Icardinal);
   E6[0].DW := AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
               AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
               AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
               AES_T3[TAESVectors(pBlock^)[3].bt[3]];
   E6[0].dw := E6[0].DW xor pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   E6[1].dw := AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
               AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
               AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
               AES_T3[TAESVectors(pBlock^)[0].bt[3]];
   E6[1].dw := E6[1].dw xor pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   E6[2].dw := AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
               AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
               AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
               AES_T3[TAESVectors(pBlock^)[1].bt[3]];
   E6[2].dw := E6[2].dw xor pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   E6[3].dw := AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
               AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
               AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
               AES_T3[TAESVectors(pBlock^)[2].bt[3]];
   E6[3].dw := E6[3].dw xor pDKCContext.RoundKeys.W[Icardinal].dw;
   //**********************************************
   Inc(Icardinal);
   TAESVectors(pBlock^)[0].dw := AES_T0[E6[0].bt[0]] xor
                                 AES_T1[E6[1].bt[1]] xor
                                 AES_T2[E6[2].bt[2]] xor
                                 AES_T3[E6[3].bt[3]];
   TAESVectors(pBlock^)[0].dw := TAESVectors(pBlock^)[0].dw xor
                                 pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   TAESVectors(pBlock^)[1].dw := AES_T0[E6[1].bt[0]] xor
                                 AES_T1[E6[2].bt[1]] xor
                                 AES_T2[E6[3].bt[2]] xor
                                 AES_T3[E6[0].bt[3]];
   TAESVectors(pBlock^)[1].dw := TAESVectors(pBlock^)[1].dw xor
                                 pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   TAESVectors(pBlock^)[2].dw := AES_T0[E6[2].bt[0]] xor
                                 AES_T1[E6[3].bt[1]] xor
                                 AES_T2[E6[0].bt[2]] xor
                                 AES_T3[E6[1].bt[3]];
   TAESVectors(pBlock^)[2].dw := TAESVectors(pBlock^)[2].dw xor
                                 pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   TAESVectors(pBlock^)[3].dw := AES_T0[E6[3].bt[0]] xor
                                 AES_T1[E6[0].bt[1]] xor
                                 AES_T2[E6[1].bt[2]] xor
                                 AES_T3[E6[2].bt[3]];
   TAESVectors(pBlock^)[3].dw := TAESVectors(pBlock^)[3].dw xor
                                 pDKCContext.RoundKeys.W[Icardinal].dw;
  end;
 //Do last round****************
 Inc(Icardinal);
 C0 := AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
       AES_T3[TAESVectors(pBlock^)[3].bt[3]];
 C0 := C0 xor pDKCContext.RoundKeys.W[Icardinal].dw;
 Inc(Icardinal);
 C1 := AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
       AES_T3[TAESVectors(pBlock^)[0].bt[3]];
 C1 := C1 xor pDKCContext.RoundKeys.W[Icardinal].dw;
 Inc(Icardinal);
 C2 := AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
       AES_T3[TAESVectors(pBlock^)[1].bt[3]];
 C2 := C2 xor pDKCContext.RoundKeys.W[Icardinal].dw;
 Inc(Icardinal);
 C3 := AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
       AES_T3[TAESVectors(pBlock^)[2].bt[3]];
 C3 := C3 xor pDKCContext.RoundKeys.W[Icardinal].dw;
 TAESVectors(pBlock^)[0].dw := C0;
 TAESVectors(pBlock^)[1].dw := C1;
 TAESVectors(pBlock^)[2].dw := C2;
 TAESVectors(pBlock^)[3].dw := C3;
 //****************************
 E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
 E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
 E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
 E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
 E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
 E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
 E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
 E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
 E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
 E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
 E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
 E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
 E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
 E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
 E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
 E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
 P1 := @E5;
 P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
 P3 := @pBlock^;
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
end;

//Author:            Dennis Kjaer Christensen
//Date:              10/8 2004
//Instructionset(s): IA32

procedure DecryptAES_DKC_PAS_17(var Block);
var
 I : Integer;
 pBlock : ^TAESBlock;
 P1, P2 : ^Cardinal;
 C1 : Cardinal;
 E1, E2, E3, E4 : TAESVector;
 Temp1, Temp2, Temp3  : Byte;

begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[TAESVectors(pBlock^)[0].bt[0]];
 TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[TAESVectors(pBlock^)[1].bt[0]];
 TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[TAESVectors(pBlock^)[2].bt[0]];
 TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[TAESVectors(pBlock^)[3].bt[0]];
 Temp1 := AESInvSBox[TAESVectors(pBlock^)[3].bt[1]];
 TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[TAESVectors(pBlock^)[2].bt[1]];
 TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[TAESVectors(pBlock^)[1].bt[1]];
 TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[TAESVectors(pBlock^)[0].bt[1]];
 TAESVectors(pBlock^)[0].bt[1] := Temp1;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[2].bt[2]];
 TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[TAESVectors(pBlock^)[0].bt[2]];
 TAESVectors(pBlock^)[0].bt[2] := Temp2;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[3].bt[2]];
 TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[TAESVectors(pBlock^)[1].bt[2]];
 TAESVectors(pBlock^)[1].bt[2] := Temp2;
 Temp3 := AESInvSBox[TAESVectors(pBlock^)[0].bt[3]];
 TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[TAESVectors(pBlock^)[1].bt[3]];
 TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[TAESVectors(pBlock^)[2].bt[3]];
 TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[TAESVectors(pBlock^)[3].bt[3]];
 TAESVectors(pBlock^)[3].bt[3] := Temp3;
 for i := (pDKCContext.Rounds - 1) downto 1 do
  begin
   P1 := @pBlock^;
   P2 := @pDKCContext.RoundKeys.Rk[i];
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   E1.dw := AES_InvT0[TAESVectors(pBlock^)[0].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[0].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[0].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[0].bt[3]];
   E2.dw := AES_InvT0[TAESVectors(pBlock^)[1].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[1].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[1].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[1].bt[3]];
   E3.dw := AES_InvT0[TAESVectors(pBlock^)[2].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[2].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[2].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[2].bt[3]];
   E4.dw := AES_InvT0[TAESVectors(pBlock^)[3].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[3].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[3].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[3].bt[3]];
   TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[E1.bt[0]];
   TAESVectors(pBlock^)[0].bt[1] := AESInvSBox[E4.bt[1]];
   TAESVectors(pBlock^)[0].bt[2] := AESInvSBox[E3.bt[2]];
   TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[E2.bt[3]];
   TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[E2.bt[0]];
   TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[E1.bt[1]];
   TAESVectors(pBlock^)[1].bt[2] := AESInvSBox[E4.bt[2]];
   TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[E3.bt[3]];
   TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[E3.bt[0]];
   TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[E2.bt[1]];
   TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[E1.bt[2]];
   TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[E4.bt[3]];
   TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[E4.bt[0]];
   TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[E3.bt[1]];
   TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[E2.bt[2]];
   TAESVectors(pBlock^)[3].bt[3] := AESInvSBox[E1.bt[3]];
  end;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[0];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
end;

//Author:            Dennis Kjaer Christensen
//Date:              29/9 2004
//Instructionset(s): IA32

procedure MakeContextsAES_DKC_PAS_18(const Key; KeySize : Cardinal);
var
 I, TAESContextSize, J, ContextOffSet : Integer;
 Nk : Byte;
 Temp : TAESVector;
 Sk : Longint;
 ContextAddress, ContextAddressAlignment : Cardinal;

begin
 TAESContextSize := SizeOf(TAESContext);
 SetLength(AlignedContext, TAESContextSize+15);//Make room for alignment
 ContextOffSet := -1;
 repeat
  Inc(ContextOffset);
  ContextAddress := Cardinal(@AlignedContext[ContextOffset]);
  ContextAddressAlignment := ContextAddress and $0000000F;
 until(ContextAddressAlignment = 0);
 for J := 0 to TAESContextSize+15-1 do
  begin
   AlignedContext[J] := 0;
  end;
 pDKCContext := @AlignedContext[ContextOffSet]; //Align context
 //Sk := Min(KeySize, SizeOf(DKCContext.RoundKeys.Rk));
 Sk := KeySize;
 Move(Key, pDKCContext.RoundKeys.Rk, Sk * 4);
 case KeySize of
  4 : Nk := 4;
  6 : Nk := 6;
  8 : NK := 8;
 else
  raise Exception.Create('Invalid Key size');
 end;
 pDKCContext.Rounds := 6 + Nk;
 pDKCContext.KeySize := KeySize;
 //Expand key into round keys
 for i := Nk to (4 * (pDKCContext.Rounds + 1)) do
  begin
   temp := pDKCContext.RoundKeys.W[i-1];
   if (Nk in [AESNk128, AESNk192]) then
    begin
     if (i mod Nk) = 0 then
      Temp.dw := AESSubVector(AESRotateVector(Temp, 1)).dw xor RCon[i div Nk];
     pDKCContext.RoundKeys.W[i].dw := pDKCContext.RoundKeys.W[i - Nk].dw xor temp.dw;
    end
   else  //Nk = AESNk256
    begin
     if (I mod Nk) = 0 then
      Temp.dw := AESSubVector(AESRotateVector(Temp, 1)).dw xor RCon[i div Nk]
     else if (I mod Nk) = 4 then
      Temp := AESSubVector(Temp);
     pDKCContext.RoundKeys.W[i].dw := pDKCContext.RoundKeys.W[i - Nk].dw xor Temp.dw;
    end;
  end;
end;

//Author:            Dennis Kjaer Christensen
//Date:              29/9 2004
//Instructionset(s): IA32

procedure EncryptAES_DKC_PAS_18(var Block);
var
 I, IMax : Integer;
 Icardinal : Cardinal;
 pBlock : ^TAESBlock;
 P1, P2, P3 : ^Cardinal;
 C0, C1, C2, C3 : Cardinal;
 E5 : array[0..3] of TAESVector;
 E6 : TAESVectors;

begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[0];
 //Add round key
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
 Icardinal := 3;
 for I := 1 to IMax do
  begin
   Inc(Icardinal);
   E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   //**********************************************
   Inc(Icardinal);
   TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
                                  AES_T1[E6[1].bt[1]] xor
                                  AES_T2[E6[2].bt[2]] xor
                                  AES_T3[E6[3].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
                                  AES_T1[E6[2].bt[1]] xor
                                  AES_T2[E6[3].bt[2]] xor
                                  AES_T3[E6[0].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
                                  AES_T1[E6[3].bt[1]] xor
                                  AES_T2[E6[0].bt[2]] xor
                                  AES_T3[E6[1].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   Inc(Icardinal);
   TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
                                  AES_T1[E6[0].bt[1]] xor
                                  AES_T2[E6[1].bt[2]] xor
                                  AES_T3[E6[2].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
  end;
 //Do last round****************
 Inc(Icardinal);
 C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 Inc(Icardinal);
 C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 Inc(Icardinal);
 C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 Inc(Icardinal);
 C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 TAESVectors(pBlock^)[0].dw := C0;
 TAESVectors(pBlock^)[1].dw := C1;
 TAESVectors(pBlock^)[2].dw := C2;
 TAESVectors(pBlock^)[3].dw := C3;
 //****************************
 E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
 E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
 E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
 E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
 E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
 E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
 E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
 E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
 E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
 E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
 E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
 E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
 E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
 E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
 E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
 E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
 P1 := @E5;
 P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
 P3 := @pBlock^;
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
end;

//Author:            Dennis Kjaer Christensen
//Date:              29/9 2004
//Instructionset(s): IA32

procedure DecryptAES_DKC_PAS_18(var Block);
var
 I : Integer;
 pBlock : ^TAESBlock;
 P1, P2 : ^Cardinal;
 C1 : Cardinal;
 E1, E2, E3, E4 : TAESVector;
 Temp1, Temp2, Temp3  : Byte;

begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[TAESVectors(pBlock^)[0].bt[0]];
 TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[TAESVectors(pBlock^)[1].bt[0]];
 TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[TAESVectors(pBlock^)[2].bt[0]];
 TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[TAESVectors(pBlock^)[3].bt[0]];
 Temp1 := AESInvSBox[TAESVectors(pBlock^)[3].bt[1]];
 TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[TAESVectors(pBlock^)[2].bt[1]];
 TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[TAESVectors(pBlock^)[1].bt[1]];
 TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[TAESVectors(pBlock^)[0].bt[1]];
 TAESVectors(pBlock^)[0].bt[1] := Temp1;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[2].bt[2]];
 TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[TAESVectors(pBlock^)[0].bt[2]];
 TAESVectors(pBlock^)[0].bt[2] := Temp2;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[3].bt[2]];
 TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[TAESVectors(pBlock^)[1].bt[2]];
 TAESVectors(pBlock^)[1].bt[2] := Temp2;
 Temp3 := AESInvSBox[TAESVectors(pBlock^)[0].bt[3]];
 TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[TAESVectors(pBlock^)[1].bt[3]];
 TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[TAESVectors(pBlock^)[2].bt[3]];
 TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[TAESVectors(pBlock^)[3].bt[3]];
 TAESVectors(pBlock^)[3].bt[3] := Temp3;
 for i := (pDKCContext.Rounds - 1) downto 1 do
  begin
   P1 := @pBlock^;
   P2 := @pDKCContext.RoundKeys.Rk[i];
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   E1.dw := AES_InvT0[TAESVectors(pBlock^)[0].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[0].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[0].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[0].bt[3]];
   E2.dw := AES_InvT0[TAESVectors(pBlock^)[1].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[1].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[1].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[1].bt[3]];
   E3.dw := AES_InvT0[TAESVectors(pBlock^)[2].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[2].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[2].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[2].bt[3]];
   E4.dw := AES_InvT0[TAESVectors(pBlock^)[3].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[3].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[3].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[3].bt[3]];
   TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[E1.bt[0]];
   TAESVectors(pBlock^)[0].bt[1] := AESInvSBox[E4.bt[1]];
   TAESVectors(pBlock^)[0].bt[2] := AESInvSBox[E3.bt[2]];
   TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[E2.bt[3]];
   TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[E2.bt[0]];
   TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[E1.bt[1]];
   TAESVectors(pBlock^)[1].bt[2] := AESInvSBox[E4.bt[2]];
   TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[E3.bt[3]];
   TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[E3.bt[0]];
   TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[E2.bt[1]];
   TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[E1.bt[2]];
   TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[E4.bt[3]];
   TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[E4.bt[0]];
   TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[E3.bt[1]];
   TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[E2.bt[2]];
   TAESVectors(pBlock^)[3].bt[3] := AESInvSBox[E1.bt[3]];
  end;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[0];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
end;

type

 TBlock = array[0..3, 0..3] of Byte;//4*4 bytes = 16 bytes = 128 bit block

function MulGF2m(A, B : Byte) : Byte;
begin
 // Multiply two elements of GF(2^m). Needed for MixColumn and InvMixColumn
 if (A <> 0) and (B <> 0) then
  Result := Alogtable[(Logtable[A] + Logtable[B]) mod 255]
 else
  Result := 0;
end;

procedure InverseMixColumns(var A : TAESBlock; BC : Integer);
var
 B : TAESBlock;
 I, J : Integer;

begin
 for I := 0 to 15 do
  begin
   B[I] := MulGF2m($E, A[I])
          xor MulGF2m($B, A[(I+1) mod 4])
          xor MulGF2m($D, A[(I+2) mod 4])
          xor MulGF2m($9, A[(I+3) mod 4])
  end;
 for J := 0 to 15 do
  A[J] := B[J];
end;

//Author:            Dennis Kjaer Christensen
//Date:              29/9 2004
//Instructionset(s): IA32

var
 pDKCContext1 : ^TAESContext;
 pDKCContext2 : ^TAESContext;
 AlignedContext1 : array of Byte;
 AlignedContext2 : array of Byte;

procedure MakeContextsAES_DKC_PAS_19(const Key; KeySize : Cardinal);
var
 I, TAESContextSize, J, ContextOffSet1, ContextOffSet2 : Integer;
 Nk : Byte;
 Temp : TAESVector;
 Sk : Longint;
 ContextAddress1, ContextAddressAlignment1 : Cardinal;
 ContextAddress2, ContextAddressAlignment2 : Cardinal;

begin
 TAESContextSize := SizeOf(TAESContext);
 SetLength(AlignedContext1, TAESContextSize+15);//Make room for alignment
 ContextOffSet1 := -1;
 repeat
  Inc(ContextOffset1);
  ContextAddress1 := Cardinal(@AlignedContext1[ContextOffset1]);
  ContextAddressAlignment1 := ContextAddress1 and $0000000F;
 until(ContextAddressAlignment1 = 0);
 for J := 0 to TAESContextSize+15-1 do
  begin
   AlignedContext1[J] := 0;
  end;
 pDKCContext1 := @AlignedContext1[ContextOffSet1]; //Align context
 //*****************
 SetLength(AlignedContext2, TAESContextSize+15);//Make room for alignment
 ContextOffSet2 := -1;
 repeat
  Inc(ContextOffset2);
  ContextAddress2 := Cardinal(@AlignedContext2[ContextOffset2]);
  ContextAddressAlignment2 := ContextAddress2 and $0000000F;
 until(ContextAddressAlignment2 = 0);
 for J := 0 to TAESContextSize+15-1 do
  begin
   AlignedContext2[J] := 0;
  end;
 pDKCContext2 := @AlignedContext2[ContextOffSet2]; //Align context
 //*****************
 //Sk := Min(KeySize, SizeOf(DKCContext.RoundKeys.Rk));
 Sk := KeySize;
 Move(Key, pDKCContext1.RoundKeys.Rk, Sk * 4);
 case KeySize of
  4 : Nk := 4;
  6 : Nk := 6;
  8 : NK := 8;
 else
  raise Exception.Create('Invalid Key size');
 end;
 pDKCContext1.Rounds := 6 + Nk;
 pDKCContext1.KeySize := KeySize;
 //Expand key into round keys
 for i := Nk to (4 * (pDKCContext1.Rounds + 1)) do
  begin
   temp := pDKCContext1.RoundKeys.W[i-1];
   if (Nk in [AESNk128, AESNk192]) then
    begin
     if (i mod Nk) = 0 then
      Temp.dw := AESSubVector(AESRotateVector(Temp, 1)).dw xor RCon[i div Nk];
     pDKCContext1.RoundKeys.W[i].dw := pDKCContext1.RoundKeys.W[i - Nk].dw xor temp.dw;
    end
   else  //Nk = AESNk256
    begin
     if (I mod Nk) = 0 then
      Temp.dw := AESSubVector(AESRotateVector(Temp, 1)).dw xor RCon[i div Nk]
     else if (I mod Nk) = 4 then
      Temp := AESSubVector(Temp);
     pDKCContext1.RoundKeys.W[i].dw := pDKCContext1.RoundKeys.W[i - Nk].dw xor Temp.dw;
    end;
  end;
 Move(AlignedContext1[ContextOffSet1], AlignedContext2[ContextOffSet2], TAESContextSize);
 //Apply InvMixCollums to all roundkeys except the first and the last
 for J := 1 to (pDKCContext2.Rounds-1) do
  begin
   InverseMixColumns(pDKCContext2.RoundKeys.Rk[J], J);
  end;
end;

//Author:            Dennis Kjaer Christensen
//Date:              2/10 2004
//Instructionset(s): IA32

procedure EncryptAES_DKC_PAS_19(var Block);
var
 pBlock : ^TAESBlock;
 P1, P2, P3 : ^Cardinal;
 C0, C1, C2, C3 : Cardinal;
 E5 : array[0..3] of TAESVector;
 E6 : TAESVectors;
 I, IMax : Integer;
 Icardinal : Cardinal;
 
begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext1.RoundKeys.Rk[0];
 //Add round key
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 IMax := (pDKCContext1.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
 Icardinal := 4;
 for I := 1 to IMax do
  begin
   //Inc(Icardinal);
   E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].dw and $ff] xor
                AES_T1[TAESVectors(pBlock^)[1].dw shr 8 and $ff] xor
                AES_T2[TAESVectors(pBlock^)[2].dw shr 16 and $ff] xor
                AES_T3[TAESVectors(pBlock^)[3].dw shr 24 and $ff]) xor
                pDKCContext1.RoundKeys.W[Icardinal].dw;
   //Inc(Icardinal);
   E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].dw and $ff] xor
                AES_T1[TAESVectors(pBlock^)[2].dw shr 8 and $ff] xor
                AES_T2[TAESVectors(pBlock^)[3].dw shr 16 and $ff] xor
                AES_T3[TAESVectors(pBlock^)[0].dw shr 24 and $ff]) xor
                pDKCContext1.RoundKeys.W[Icardinal+1].dw;
   //Inc(Icardinal);
   E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].dw and $ff] xor
                AES_T1[TAESVectors(pBlock^)[3].dw shr 8 and $ff] xor
                AES_T2[TAESVectors(pBlock^)[0].dw shr 16 and $ff] xor
                AES_T3[TAESVectors(pBlock^)[1].dw shr 24 and $ff]) xor
                pDKCContext1.RoundKeys.W[Icardinal+2].dw;
   //Inc(Icardinal);
   E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].dw and $ff] xor
                AES_T1[TAESVectors(pBlock^)[0].dw shr 8 and $ff] xor
                AES_T2[TAESVectors(pBlock^)[1].dw shr 16 and $ff] xor
                AES_T3[TAESVectors(pBlock^)[2].dw shr 24 and $ff]) xor
                pDKCContext1.RoundKeys.W[Icardinal+3].dw;
   //**********************************************
   //Inc(Icardinal);
   TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].dw and $ff] xor
                                  AES_T1[E6[1].dw shr 8 and $ff] xor
                                  AES_T2[E6[2].dw shr 16 and $ff] xor
                                  AES_T3[E6[3].dw shr 24 and $ff]) xor
                                  pDKCContext1.RoundKeys.W[Icardinal+4].dw;
   //Inc(Icardinal);
   TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].dw and $ff] xor
                                  AES_T1[E6[2].dw shr 8 and $ff] xor
                                  AES_T2[E6[3].dw shr 16 and $ff] xor
                                  AES_T3[E6[0].dw shr 24 and $ff]) xor
                                  pDKCContext1.RoundKeys.W[Icardinal+5].dw;
   //Inc(Icardinal);
   TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].dw and $ff] xor
                                  AES_T1[E6[3].dw shr 8 and $ff] xor
                                  AES_T2[E6[0].dw shr 16 and $ff] xor
                                  AES_T3[E6[1].dw shr 24 and $ff]) xor
                                  pDKCContext1.RoundKeys.W[Icardinal+6].dw;
   //Inc(Icardinal);
   TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].dw and $ff] xor
                                  AES_T1[E6[0].dw shr 8 and $ff] xor
                                  AES_T2[E6[1].dw shr 16 and $ff] xor
                                  AES_T3[E6[2].dw shr 24 and $ff]) xor
                                  pDKCContext1.RoundKeys.W[Icardinal+7].dw;
   Inc(Icardinal,8)
  end;
 //Do last round****************
 //Inc(Icardinal);
 C0 := (AES_T0[TAESVectors(pBlock^)[0].dw and $ff] xor
        AES_T1[TAESVectors(pBlock^)[1].dw shr 8 and $ff] xor
        AES_T2[TAESVectors(pBlock^)[2].dw shr 16 and $ff] xor
        AES_T3[TAESVectors(pBlock^)[3].dw shr 24 and $ff]) xor
        pDKCContext1.RoundKeys.W[Icardinal].dw;
 Inc(Icardinal);
 C1 := (AES_T0[TAESVectors(pBlock^)[1].dw and $ff] xor
        AES_T1[TAESVectors(pBlock^)[2].dw shr 8 and $ff] xor
        AES_T2[TAESVectors(pBlock^)[3].dw shr 16 and $ff] xor
        AES_T3[TAESVectors(pBlock^)[0].dw shr 24 and $ff]) xor
        pDKCContext1.RoundKeys.W[Icardinal].dw;
 Inc(Icardinal);
 C2 := (AES_T0[TAESVectors(pBlock^)[2].dw and $ff] xor
        AES_T1[TAESVectors(pBlock^)[3].dw shr 8 and $ff] xor
        AES_T2[TAESVectors(pBlock^)[0].dw shr 16 and $ff] xor
        AES_T3[TAESVectors(pBlock^)[1].dw shr 24 and $ff]) xor
        pDKCContext1.RoundKeys.W[Icardinal].dw;
 Inc(Icardinal);
 C3 := (AES_T0[TAESVectors(pBlock^)[3].dw and $ff] xor
        AES_T1[TAESVectors(pBlock^)[0].dw shr 8 and $ff] xor
        AES_T2[TAESVectors(pBlock^)[1].dw shr 16 and $ff] xor
        AES_T3[TAESVectors(pBlock^)[2].dw shr 24 and $ff]) xor
        pDKCContext1.RoundKeys.W[Icardinal].dw;
 //TAESVectors(pBlock^)[0].dw := C0;
 //TAESVectors(pBlock^)[1].dw := C1;
 //TAESVectors(pBlock^)[2].dw := C2;
 //TAESVectors(pBlock^)[3].dw := C3;
 //****************************
 E5[0].bt[0] := AESSBox[C0 and $ff];
 E5[0].bt[1] := AESSBox[C1 shr 8 and $ff];
 E5[0].bt[2] := AESSBox[C2 shr 16 and $ff];
 E5[0].bt[3] := AESSBox[C3 shr 24 and $ff];
 E5[1].bt[0] := AESSBox[C1 and $ff];
 E5[1].bt[1] := AESSBox[C2 shr 8 and $ff];
 E5[1].bt[2] := AESSBox[C3 shr 16 and $ff];
 E5[1].bt[3] := AESSBox[C0 shr 24 and $ff];
 E5[2].bt[0] := AESSBox[C2 and $ff];
 E5[2].bt[1] := AESSBox[C3 shr 8 and $ff];
 E5[2].bt[2] := AESSBox[C0 shr 16 and $ff];
 E5[2].bt[3] := AESSBox[C1 shr 24 and $ff];
 E5[3].bt[0] := AESSBox[C3 and $ff];
 E5[3].bt[1] := AESSBox[C0 shr 8 and $ff];
 E5[3].bt[2] := AESSBox[C1 shr 16 and $ff];
 E5[3].bt[3] := AESSBox[C2 shr 24 and $ff];
 P1 := @E5;
 P2 := @pDKCContext1.RoundKeys.Rk[pDKCContext1.Rounds];
 P3 := @pBlock^;
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
end;

//Author:            Dennis Kjaer Christensen
//Date:              29/9 2004
//Instructionset(s): IA32

procedure DecryptAES_DKC_PAS_19(var Block);
var
 I : Integer;
 pBlock : ^TAESBlock;
 P1, P2 : ^Cardinal;
 C1 : Cardinal;
 E1, E2, E3, E4 : TAESVector;
 Temp1, Temp2, Temp3  : Byte;

begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext1.RoundKeys.Rk[pDKCContext1.Rounds];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[TAESVectors(pBlock^)[0].bt[0]];
 TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[TAESVectors(pBlock^)[1].bt[0]];
 TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[TAESVectors(pBlock^)[2].bt[0]];
 TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[TAESVectors(pBlock^)[3].bt[0]];
 Temp1 := AESInvSBox[TAESVectors(pBlock^)[3].bt[1]];
 TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[TAESVectors(pBlock^)[2].bt[1]];
 TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[TAESVectors(pBlock^)[1].bt[1]];
 TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[TAESVectors(pBlock^)[0].bt[1]];
 TAESVectors(pBlock^)[0].bt[1] := Temp1;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[2].bt[2]];
 TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[TAESVectors(pBlock^)[0].bt[2]];
 TAESVectors(pBlock^)[0].bt[2] := Temp2;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[3].bt[2]];
 TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[TAESVectors(pBlock^)[1].bt[2]];
 TAESVectors(pBlock^)[1].bt[2] := Temp2;
 Temp3 := AESInvSBox[TAESVectors(pBlock^)[0].bt[3]];
 TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[TAESVectors(pBlock^)[1].bt[3]];
 TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[TAESVectors(pBlock^)[2].bt[3]];
 TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[TAESVectors(pBlock^)[3].bt[3]];
 TAESVectors(pBlock^)[3].bt[3] := Temp3;
 for i := (pDKCContext1.Rounds - 1) downto 1 do
  begin
   P1 := @pBlock^;
   P2 := @pDKCContext1.RoundKeys.Rk[i];
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   E1.dw := AES_InvT0[TAESVectors(pBlock^)[0].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[0].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[0].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[0].bt[3]];
   E2.dw := AES_InvT0[TAESVectors(pBlock^)[1].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[1].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[1].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[1].bt[3]];
   E3.dw := AES_InvT0[TAESVectors(pBlock^)[2].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[2].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[2].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[2].bt[3]];
   E4.dw := AES_InvT0[TAESVectors(pBlock^)[3].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[3].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[3].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[3].bt[3]];
   TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[E1.bt[0]];
   TAESVectors(pBlock^)[0].bt[1] := AESInvSBox[E4.bt[1]];
   TAESVectors(pBlock^)[0].bt[2] := AESInvSBox[E3.bt[2]];
   TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[E2.bt[3]];
   TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[E2.bt[0]];
   TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[E1.bt[1]];
   TAESVectors(pBlock^)[1].bt[2] := AESInvSBox[E4.bt[2]];
   TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[E3.bt[3]];
   TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[E3.bt[0]];
   TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[E2.bt[1]];
   TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[E1.bt[2]];
   TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[E4.bt[3]];
   TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[E4.bt[0]];
   TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[E3.bt[1]];
   TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[E2.bt[2]];
   TAESVectors(pBlock^)[3].bt[3] := AESInvSBox[E1.bt[3]];
  end;
 P1 := @pBlock^;
 P2 := @pDKCContext1.RoundKeys.Rk[0];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
end;

procedure DecryptAES_DKC_PAS_BAD(var Block);
var
 I, IMax, Iint : Integer;
 pBlock : ^TAESBlock;
 P1, P2, P3 : ^Cardinal;
 C0, C1, C2, C3 : Cardinal;
 E5 : array[0..3] of TAESVector;
 E6 : TAESVectors;

begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext2.RoundKeys.Rk[pDKCContext2.Rounds+1];
 //Add round key
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 P1^ := P1^ xor P2^;
 IMax := (pDKCContext2.Rounds - 2) div 2; //Always even number of rounds and odd number of iterations
 Iint := (pDKCContext2.Rounds) * 4 - 5;
 for I := IMax downto 1 do
  begin
   Inc(Iint);
   E6[0].DW := (AES_InvT4[TAESVectors(pBlock^)[0].bt[0]] xor
                AES_InvT5[TAESVectors(pBlock^)[1].bt[1]] xor
                AES_InvT6[TAESVectors(pBlock^)[2].bt[2]] xor
                AES_InvT7[TAESVectors(pBlock^)[3].bt[3]]) xor
                pDKCContext2.RoundKeys.W[Iint].dw;
   Inc(Iint);
   E6[1].dw := (AES_InvT4[TAESVectors(pBlock^)[1].bt[0]] xor
                AES_InvT5[TAESVectors(pBlock^)[2].bt[1]] xor
                AES_InvT6[TAESVectors(pBlock^)[3].bt[2]] xor
                AES_InvT7[TAESVectors(pBlock^)[0].bt[3]]) xor
                pDKCContext2.RoundKeys.W[Iint].dw;
   Inc(Iint);
   E6[2].dw := (AES_InvT4[TAESVectors(pBlock^)[2].bt[0]] xor
                AES_InvT5[TAESVectors(pBlock^)[3].bt[1]] xor
                AES_InvT6[TAESVectors(pBlock^)[0].bt[2]] xor
                AES_InvT7[TAESVectors(pBlock^)[1].bt[3]]) xor
                pDKCContext2.RoundKeys.W[Iint].dw;
   Inc(Iint);
   E6[3].dw := (AES_InvT4[TAESVectors(pBlock^)[3].bt[0]] xor
                AES_InvT5[TAESVectors(pBlock^)[0].bt[1]] xor
                AES_InvT6[TAESVectors(pBlock^)[1].bt[2]] xor
                AES_InvT7[TAESVectors(pBlock^)[2].bt[3]]) xor
                pDKCContext2.RoundKeys.W[Iint].dw;
   Dec(Iint, 8);
   //**********************************************
   Inc(Iint);
   TAESVectors(pBlock^)[0].dw := (AES_InvT4[E6[0].bt[0]] xor
                                  AES_InvT5[E6[1].bt[1]] xor
                                  AES_InvT6[E6[2].bt[2]] xor
                                  AES_InvT7[E6[3].bt[3]]) xor
                                  pDKCContext2.RoundKeys.W[Iint].dw;
   Inc(Iint);
   TAESVectors(pBlock^)[1].dw := (AES_InvT4[E6[1].bt[0]] xor
                                  AES_InvT5[E6[2].bt[1]] xor
                                  AES_InvT6[E6[3].bt[2]] xor
                                  AES_InvT7[E6[0].bt[3]]) xor
                                  pDKCContext2.RoundKeys.W[Iint].dw;
   Inc(Iint);
   TAESVectors(pBlock^)[2].dw := (AES_InvT4[E6[2].bt[0]] xor
                                  AES_InvT5[E6[3].bt[1]] xor
                                  AES_InvT6[E6[0].bt[2]] xor
                                  AES_InvT7[E6[1].bt[3]]) xor
                                  pDKCContext2.RoundKeys.W[Iint].dw;
   Inc(Iint);
   TAESVectors(pBlock^)[3].dw := (AES_InvT4[E6[3].bt[0]] xor
                                  AES_InvT5[E6[0].bt[1]] xor
                                  AES_InvT6[E6[1].bt[2]] xor
                                  AES_InvT7[E6[2].bt[3]]) xor
                                  pDKCContext2.RoundKeys.W[Iint].dw;
   Dec(Iint, 8);
  end;
 //Do last round****************
 Inc(Iint);
 C0 := (AES_InvT4[TAESVectors(pBlock^)[0].bt[0]] xor
        AES_InvT5[TAESVectors(pBlock^)[1].bt[1]] xor
        AES_InvT6[TAESVectors(pBlock^)[2].bt[2]] xor
        AES_InvT7[TAESVectors(pBlock^)[3].bt[3]]) xor
        pDKCContext2.RoundKeys.W[Iint].dw;
 Inc(Iint);
 C1 := (AES_InvT4[TAESVectors(pBlock^)[1].bt[0]] xor
        AES_InvT5[TAESVectors(pBlock^)[2].bt[1]] xor
        AES_InvT6[TAESVectors(pBlock^)[3].bt[2]] xor
        AES_InvT7[TAESVectors(pBlock^)[0].bt[3]]) xor
        pDKCContext2.RoundKeys.W[Iint].dw;
 Inc(Iint);
 C2 := (AES_InvT4[TAESVectors(pBlock^)[2].bt[0]] xor
        AES_InvT5[TAESVectors(pBlock^)[3].bt[1]] xor
        AES_InvT6[TAESVectors(pBlock^)[0].bt[2]] xor
        AES_InvT7[TAESVectors(pBlock^)[1].bt[3]]) xor
        pDKCContext2.RoundKeys.W[Iint].dw;
 Inc(Iint);
 C3 := (AES_InvT4[TAESVectors(pBlock^)[3].bt[0]] xor
        AES_InvT5[TAESVectors(pBlock^)[0].bt[1]] xor
        AES_InvT6[TAESVectors(pBlock^)[1].bt[2]] xor
        AES_InvT7[TAESVectors(pBlock^)[2].bt[3]]) xor
        pDKCContext2.RoundKeys.W[Iint].dw;
 TAESVectors(pBlock^)[0].dw := C0;
 TAESVectors(pBlock^)[1].dw := C1;
 TAESVectors(pBlock^)[2].dw := C2;
 TAESVectors(pBlock^)[3].dw := C3;
 //****************************
 E5[0].bt[0] := AESInvSBox[TAESVectors(pBlock^)[0].bt[0]];
 E5[0].bt[1] := AESInvSBox[TAESVectors(pBlock^)[1].bt[1]];
 E5[0].bt[2] := AESInvSBox[TAESVectors(pBlock^)[2].bt[2]];
 E5[0].bt[3] := AESInvSBox[TAESVectors(pBlock^)[3].bt[3]];
 E5[1].bt[0] := AESInvSBox[TAESVectors(pBlock^)[1].bt[0]];
 E5[1].bt[1] := AESInvSBox[TAESVectors(pBlock^)[2].bt[1]];
 E5[1].bt[2] := AESInvSBox[TAESVectors(pBlock^)[3].bt[2]];
 E5[1].bt[3] := AESInvSBox[TAESVectors(pBlock^)[0].bt[3]];
 E5[2].bt[0] := AESInvSBox[TAESVectors(pBlock^)[2].bt[0]];
 E5[2].bt[1] := AESInvSBox[TAESVectors(pBlock^)[3].bt[1]];
 E5[2].bt[2] := AESInvSBox[TAESVectors(pBlock^)[0].bt[2]];
 E5[2].bt[3] := AESInvSBox[TAESVectors(pBlock^)[1].bt[3]];
 E5[3].bt[0] := AESInvSBox[TAESVectors(pBlock^)[3].bt[0]];
 E5[3].bt[1] := AESInvSBox[TAESVectors(pBlock^)[0].bt[1]];
 E5[3].bt[2] := AESInvSBox[TAESVectors(pBlock^)[1].bt[2]];
 E5[3].bt[3] := AESInvSBox[TAESVectors(pBlock^)[2].bt[3]];
 P1 := @E5;
 P2 := @pDKCContext2.RoundKeys.Rk[0];
 P3 := @pBlock^;
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
 Inc(P1);
 Inc(P2);
 Inc(P3);
 P3^ := P1^ xor P2^;
end;

//Author:            Dennis Kjaer Christensen
//Date:              30/9 2004
//Instructionset(s): IA32

procedure MakeContextsAES_DKC_IA32_1(const Key; KeySize : Cardinal);
var
 I, TAESContextSize, J, ContextOffSet : Integer;
 Nk : Byte;
 Temp : TAESVector;
 Sk : Longint;
 ContextAddress, ContextAddressAlignment : Cardinal;

begin
 TAESContextSize := SizeOf(TAESContext);
 SetLength(AlignedContext, TAESContextSize+15);//Make room for alignment
 ContextOffSet := -1;
 repeat
  Inc(ContextOffset);
  ContextAddress := Cardinal(@AlignedContext[ContextOffset]);
  ContextAddressAlignment := ContextAddress and $0000000F;
 until(ContextAddressAlignment = 0);
 for J := 0 to TAESContextSize+15-1 do
  begin
   AlignedContext[J] := 0;
  end;
 pDKCContext := @AlignedContext[ContextOffSet]; //Align context
 //Sk := Min(KeySize, SizeOf(DKCContext.RoundKeys.Rk));
 Sk := KeySize;
 Move(Key, pDKCContext.RoundKeys.Rk, Sk * 4);
 case KeySize of
  4 : Nk := 4;
  6 : Nk := 6;
  8 : NK := 8;
 else
  raise Exception.Create('Invalid Key size');
 end;
 pDKCContext.Rounds := 6 + Nk;
 pDKCContext.KeySize := KeySize;
 //Expand key into round keys
 for i := Nk to (4 * (pDKCContext.Rounds + 1)) do
  begin
   temp := pDKCContext.RoundKeys.W[i-1];
   if (Nk in [AESNk128, AESNk192]) then
    begin
     if (i mod Nk) = 0 then
      Temp.dw := AESSubVector(AESRotateVector(Temp, 1)).dw xor RCon[i div Nk];
     pDKCContext.RoundKeys.W[i].dw := pDKCContext.RoundKeys.W[i - Nk].dw xor temp.dw;
    end
   else  //Nk = AESNk256
    begin
     if (I mod Nk) = 0 then
      Temp.dw := AESSubVector(AESRotateVector(Temp, 1)).dw xor RCon[i div Nk]
     else if (I mod Nk) = 4 then
      Temp := AESSubVector(Temp);
     pDKCContext.RoundKeys.W[i].dw := pDKCContext.RoundKeys.W[i - Nk].dw xor Temp.dw;
    end;
  end;
end;

//Author:            Dennis Kjaer Christensen
//Date:              30/9 2004
//Instructionset(s): IA32

procedure EncryptAES_DKC_IA32_1a(var Block);
var
 I, IMax : Integer;
 Icardinal : Cardinal;
 pBlock : ^TAESBlock;
 P1, P2, P3 : ^Cardinal;
 C0, C1, C2, C3 : Cardinal;
 E5 : array[0..3] of TAESVector;
 E6 : TAESVectors;

begin
{
00461464 55               push ebp
004608B9 56               push esi
004608BA 57               push edi
004608BB 55               push ebp
004608BC 83C4DC           add esp,-$24
004608BF 8D4C2414         lea ecx,[esp+$14]
004608C3 BED44B4700       mov esi,$00474bd4

}
 pBlock := @Block;
 {
 }
 P1 := @pBlock^;
 {
 004608C8 8BD0             mov edx,eax
 }
 P2 := @pDKCContext.RoundKeys.Rk[0];
 {
 004608CA 8B1E             mov ebx,[esi]
 }
 //Add round key
 P1^ := P1^ xor P2^;
 {
 004608CC 8B3B             mov edi,[ebx]
 004608CE 313A             xor [edx],edi
 }
 Inc(P1);
 {
 004608D0 83C204           add edx,$04
 }
 Inc(P2);
 {
 004608D3 83C304           add ebx,$04
 }
 P1^ := P1^ xor P2^;
 {
 004608D6 8B3B             mov edi,[ebx]
 004608D8 313A             xor [edx],edi
 }
 Inc(P1);
 {
 004608DA 83C204           add edx,$04
 }
 Inc(P2);
 {
 004608DD 83C304           add ebx,$04
 }
 P1^ := P1^ xor P2^;
 {
 004608E0 8B3B             mov edi,[ebx]
 004608E2 313A             xor [edx],edi
 }
 Inc(P1);
 {
 004608E4 83C204           add edx,$04
 }
 Inc(P2);
 {
 004608E7 83C304           add ebx,$04
 }
 P1^ := P1^ xor P2^;
 {
 004608EA 8B1B             mov ebx,[ebx]
 004608EC 311A             xor [edx],ebx
 }
 IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
 {
 004608EE 8B16             mov edx,[esi]
 004608F0 8B9A01010000     mov ebx,[edx+$00000101]
 004608F6 4B               dec ebx
 004608F7 D1EB             shr ebx,1
 }
 Icardinal := 3;
 {
 004608F9 BA03000000       mov edx,$00000003
 }
 for I := 1 to IMax do
 {
 004608FE 85DB             test ebx,ebx
 00460900 0F8EB3010000     jle +$000001b3
 }
  begin
   Inc(Icardinal);
   {
   00460906 42               inc edx
   }
   E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   00460907 0FB638           movzx edi,[eax]
   0046090A 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
   00460911 0FB66805         movzx ebp,[eax+$05]
   00460915 333CAD18B54600   xor edi,[ebp*4+AES_T1]
   0046091C 0FB6680A         movzx ebp,[eax+$0a]
   00460920 333CAD18B94600   xor edi,[ebp*4+AES_T2]
   00460927 0FB6680F         movzx ebp,[eax+$0f]
   0046092B 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
   00460932 8B2E             mov ebp,[esi]
   00460934 337C9500         xor edi,[ebp+edx*4+$00]
   00460938 8939             mov [ecx],edi
   }
   Inc(Icardinal);
   {
   0046093A 42               inc edx
   }
   E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   0046093B 0FB67804         movzx edi,[eax+$04]
   0046093F 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
   00460946 0FB66809         movzx ebp,[eax+$09]
   0046094A 333CAD18B54600   xor edi,[ebp*4+AES_T1]
   00460951 0FB6680E         movzx ebp,[eax+$0e]
   00460955 333CAD18B94600   xor edi,[ebp*4+AES_T2]
   0046095C 0FB66803         movzx ebp,[eax+$03]
   00460960 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
   00460967 8B2E             mov ebp,[esi]
   00460969 337C9500         xor edi,[ebp+edx*4+$00]
   0046096D 897904           mov [ecx+$04],edi
   }
   Inc(Icardinal);
   {
   00460970 42               inc edx
   }
   E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   00460971 0FB67808         movzx edi,[eax+$08]
   00460975 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
   0046097C 0FB6680D         movzx ebp,[eax+$0d]
   00460980 333CAD18B54600   xor edi,[ebp*4+AES_T1]
   00460987 0FB66802         movzx ebp,[eax+$02]
   0046098B 333CAD18B94600   xor edi,[ebp*4+AES_T2]
   00460992 0FB66807         movzx ebp,[eax+$07]
   00460996 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
   0046099D 8B2E             mov ebp,[esi]
   0046099F 337C9500         xor edi,[ebp+edx*4+$00]
   004609A3 897908           mov [ecx+$08],edi
   }
   Inc(Icardinal);
   {
   004609A6 42               inc edx
   }
   E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   004609A7 0FB6780C         movzx edi,[eax+$0c]
   004609AB 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
   004609B2 0FB66801         movzx ebp,[eax+$01]
   004609B6 333CAD18B54600   xor edi,[ebp*4+AES_T1]
   004609BD 0FB66806         movzx ebp,[eax+$06]
   004609C1 333CAD18B94600   xor edi,[ebp*4+AES_T2]
   004609C8 0FB6680B         movzx ebp,[eax+$0b]
   004609CC 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
   004609D3 8B2E             mov ebp,[esi]
   004609D5 337C9500         xor edi,[ebp+edx*4+$00]
   004609D9 89790C           mov [ecx+$0c],edi
   }
   //**********************************************
   Inc(Icardinal);
   {
   004609DC 42               inc edx
   }
   TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
                                  AES_T1[E6[1].bt[1]] xor
                                  AES_T2[E6[2].bt[2]] xor
                                  AES_T3[E6[3].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   004609DD 0FB639           movzx edi,[ecx]
   004609E0 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
   004609E7 0FB66905         movzx ebp,[ecx+$05]
   004609EB 333CAD18B54600   xor edi,[ebp*4+AES_T1]
   004609F2 0FB6690A         movzx ebp,[ecx+$0a]
   004609F6 333CAD18B94600   xor edi,[ebp*4+AES_T2]
   004609FD 0FB6690F         movzx ebp,[ecx+$0f]
   00460A01 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
   00460A08 8B2E             mov ebp,[esi]
   00460A0A 337C9500         xor edi,[ebp+edx*4+$00]
   00460A0E 8938             mov [eax],edi
   }
   Inc(Icardinal);
   {
   00460A10 42               inc edx
   }
   TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
                                  AES_T1[E6[2].bt[1]] xor
                                  AES_T2[E6[3].bt[2]] xor
                                  AES_T3[E6[0].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   00460A11 0FB67904         movzx edi,[ecx+$04]
   00460A15 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
   00460A1C 0FB66909         movzx ebp,[ecx+$09]
   00460A20 333CAD18B54600   xor edi,[ebp*4+AES_T1]
   00460A27 0FB6690E         movzx ebp,[ecx+$0e]
   00460A2B 333CAD18B94600   xor edi,[ebp*4+AES_T2]
   00460A32 0FB66903         movzx ebp,[ecx+$03]
   00460A36 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
   00460A3D 8B2E             mov ebp,[esi]
   00460A3F 337C9500         xor edi,[ebp+edx*4+$00]
   00460A43 897804           mov [eax+$04],edi
   }
   Inc(Icardinal);
   {
   00460A46 42               inc edx
   }
   TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
                                  AES_T1[E6[3].bt[1]] xor
                                  AES_T2[E6[0].bt[2]] xor
                                  AES_T3[E6[1].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   00460A47 0FB67908         movzx edi,[ecx+$08]
   00460A4B 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
   00460A52 0FB6690D         movzx ebp,[ecx+$0d]
   00460A56 333CAD18B54600   xor edi,[ebp*4+AES_T1]
   00460A5D 0FB66902         movzx ebp,[ecx+$02]
   00460A61 333CAD18B94600   xor edi,[ebp*4+AES_T2]
   00460A68 0FB66907         movzx ebp,[ecx+$07]
   00460A6C 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
   00460A73 8B2E             mov ebp,[esi]
   00460A75 337C9500         xor edi,[ebp+edx*4+$00]
   00460A79 897808           mov [eax+$08],edi
   }
   Inc(Icardinal);
   {
   00460A7C 42               inc edx
   }
   TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
                                  AES_T1[E6[0].bt[1]] xor
                                  AES_T2[E6[1].bt[2]] xor
                                  AES_T3[E6[2].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   00460A7D 0FB6790C         movzx edi,[ecx+$0c]
   00460A81 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
   00460A88 0FB66901         movzx ebp,[ecx+$01]
   00460A8C 333CAD18B54600   xor edi,[ebp*4+AES_T1]
   00460A93 0FB66906         movzx ebp,[ecx+$06]
   00460A97 333CAD18B94600   xor edi,[ebp*4+AES_T2]
   00460A9E 0FB6690B         movzx ebp,[ecx+$0b]
   00460AA2 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
   00460AA9 8B2E             mov ebp,[esi]
   00460AAB 337C9500         xor edi,[ebp+edx*4+$00]
   00460AAF 89780C           mov [eax+$0c],edi
   }
  end;
  {
  00460AB2 4B               dec ebx
  00460AB3 0F854DFEFFFF     jnz -$000001b3
  }
 //Do last round****************
 Inc(Icardinal);
 {
 00460AB9 42               inc edx
 }
 C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 {
 00460ABA 33C9             xor ecx,ecx
 00460ABC 8A08             mov cl,[eax]
 00460ABE 8B0C8D18B14600   mov ecx,[ecx*4+AES_T0]
 00460AC5 33DB             xor ebx,ebx
 00460AC7 8A5805           mov bl,[eax+$05]
 00460ACA 330C9D18B54600   xor ecx,[ebx*4+AES_T1]
 00460AD1 33DB             xor ebx,ebx
 00460AD3 8A580A           mov bl,[eax+$0a]
 00460AD6 330C9D18B94600   xor ecx,[ebx*4+AES_T2]
 00460ADD 33DB             xor ebx,ebx
 00460ADF 8A580F           mov bl,[eax+$0f]
 00460AE2 330C9D18BD4600   xor ecx,[ebx*4+AES_T3]
 00460AE9 8B1E             mov ebx,[esi]
 00460AEB 330C93           xor ecx,[ebx+edx*4]
 }
 Inc(Icardinal);
 {
 00460AEE 42               inc edx
 }
 C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 {
 00460AEF 33DB             xor ebx,ebx
 00460AF1 8A5804           mov bl,[eax+$04]
 00460AF4 8B1C9D18B14600   mov ebx,[ebx*4+AES_T0]
 00460AFB 0FB67809         movzx edi,[eax+$09]
 00460AFF 331CBD18B54600   xor ebx,[edi*4+AES_T1]
 00460B06 0FB6780E         movzx edi,[eax+$0e]
 00460B0A 331CBD18B94600   xor ebx,[edi*4+AES_T2]
 00460B11 0FB67803         movzx edi,[eax+$03]
 00460B15 331CBD18BD4600   xor ebx,[edi*4+AES_T3]
 00460B1C 8B3E             mov edi,[esi]
 00460B1E 331C97           xor ebx,[edi+edx*4]
 }
 Inc(Icardinal);
 {
 00460B21 42               inc edx
 }
 C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 {
 00460B22 0FB67808         movzx edi,[eax+$08]
 00460B26 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
 00460B2D 0FB6680D         movzx ebp,[eax+$0d]
 00460B31 333CAD18B54600   xor edi,[ebp*4+AES_T1]
 00460B38 0FB66802         movzx ebp,[eax+$02]
 00460B3C 333CAD18B94600   xor edi,[ebp*4+AES_T2]
 00460B43 0FB66807         movzx ebp,[eax+$07]
 00460B47 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
 00460B4E 8B2E             mov ebp,[esi]
 00460B50 337C9500         xor edi,[ebp+edx*4+$00]
 00460B54 893C24           mov [esp],edi
 }
 Inc(Icardinal);
 {
 00460B57 42               inc edx
 }
 C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 {
 00460B58 0FB6780C         movzx edi,[eax+$0c]
 00460B5C 8B3CBD18B14600   mov edi,[edi*4+AES_T0]
 00460B63 0FB66801         movzx ebp,[eax+$01]
 00460B67 333CAD18B54600   xor edi,[ebp*4+AES_T1]
 00460B6E 0FB66806         movzx ebp,[eax+$06]
 00460B72 333CAD18B94600   xor edi,[ebp*4+AES_T2]
 00460B79 0FB6680B         movzx ebp,[eax+$0b]
 00460B7D 333CAD18BD4600   xor edi,[ebp*4+AES_T3]
 00460B84 8B2E             mov ebp,[esi]
 00460B86 337C9500         xor edi,[ebp+edx*4+$00]
 00460B8A 8BD7             mov edx,edi
 }
 TAESVectors(pBlock^)[0].dw := C0;
 {
 00460B8C 8908             mov [eax],ecx
 }
 TAESVectors(pBlock^)[1].dw := C1;
 {
 00460B8E 895804           mov [eax+$04],ebx
 }
 TAESVectors(pBlock^)[2].dw := C2;
 {
 00460B91 8B0C24           mov ecx,[esp]
 00460B94 894808           mov [eax+$08],ecx
 }
 TAESVectors(pBlock^)[3].dw := C3;
 {
 00460B97 89500C           mov [eax+$0c],edx
 }
 //****************************
 E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
 {
 00460B9A 33D2             xor edx,edx
 00460B9C 8A10             mov dl,[eax]
 00460B9E 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460BA4 88542404         mov [esp+$04],dl
 }
 E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
 {
 00460BA8 33D2             xor edx,edx
 00460BAA 8A5005           mov dl,[eax+$05]
 00460BAD 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460BB3 88542405         mov [esp+$05],dl
 }
 E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
 {
 00460BB7 33D2             xor edx,edx
 00460BB9 8A500A           mov dl,[eax+$0a]
 00460BBC 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460BC2 88542406         mov [esp+$06],dl
 }
 E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
 {
 00460BC6 33D2             xor edx,edx
 00460BC8 8A500F           mov dl,[eax+$0f]
 00460BCB 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460BD1 88542407         mov [esp+$07],dl
 }
 E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
 {
 00460BD5 33D2             xor edx,edx
 00460BD7 8A5004           mov dl,[eax+$04]
 00460BDA 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460BE0 88542408         mov [esp+$08],dl
 }
 E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
 {
 00460BE4 33D2             xor edx,edx
 00460BE6 8A5009           mov dl,[eax+$09]
 00460BE9 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460BEF 88542409         mov [esp+$09],dl
 }
 E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
 {
 00460BF3 33D2             xor edx,edx
 00460BF5 8A500E           mov dl,[eax+$0e]
 00460BF8 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460BFE 8854240A         mov [esp+$0a],dl
 }
 E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
 {
 00460C02 33D2             xor edx,edx
 00460C04 8A5003           mov dl,[eax+$03]
 00460C07 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460C0D 8854240B         mov [esp+$0b],dl
 }
 E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
 {
 00460C11 33D2             xor edx,edx
 00460C13 8A5008           mov dl,[eax+$08]
 00460C16 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460C1C 8854240C         mov [esp+$0c],dl
 }
 E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
 {
 00460C20 33D2             xor edx,edx
 00460C22 8A500D           mov dl,[eax+$0d]
 00460C25 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460C2B 8854240D         mov [esp+$0d],dl
 }
 E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
 {
 00460C2F 33D2             xor edx,edx
 00460C31 8A5002           mov dl,[eax+$02]
 00460C34 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460C3A 8854240E         mov [esp+$0e],dl
 }
 E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
 {
 00460C3E 33D2             xor edx,edx
 00460C40 8A5007           mov dl,[eax+$07]
 00460C43 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460C49 8854240F         mov [esp+$0f],dl
 }
 E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
 {
 00460C4D 33D2             xor edx,edx
 00460C4F 8A500C           mov dl,[eax+$0c]
 00460C52 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460C58 88542410         mov [esp+$10],dl
 }
 E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
 {
 00460C5C 33D2             xor edx,edx
 00460C5E 8A5001           mov dl,[eax+$01]
 00460C61 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460C67 88542411         mov [esp+$11],dl
 }
 E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
 {
 00460C6B 33D2             xor edx,edx
 00460C6D 8A5006           mov dl,[eax+$06]
 00460C70 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460C76 88542412         mov [esp+$12],dl
 }
 E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
 {
 00460C7A 33D2             xor edx,edx
 00460C7C 8A500B           mov dl,[eax+$0b]
 00460C7F 8A92DCAE4600     mov dl,[edx+AESSBox]
 00460C85 88542413         mov [esp+$13],dl
 }
 P1 := @E5;
 {
 00460C89 8D542404         lea edx,[esp+$04]
 }
 P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
 {
 00460C8D 8B0E             mov ecx,[esi]
 00460C8F 8B8901010000     mov ecx,[ecx+$00000101]
 00460C95 03C9             add ecx,ecx
 }
 P3 := @pBlock^;
 {
 00460C97 8B1E             mov ebx,[esi]
 00460C99 8D1CCB           lea ebx,[ebx+ecx*8]
 }
 P3^ := P1^ xor P2^;
 {
 00460C9C 8B0A             mov ecx,[edx]
 00460C9E 330B             xor ecx,[ebx]
 00460CA0 8908             mov [eax],ecx
 }
 Inc(P1);
 {
 00460CA2 83C204           add edx,$04
 }
 Inc(P2);
 {
 00460CA5 83C304           add ebx,$04
 }
 Inc(P3);
 {
 00460CA8 83C004           add eax,$04
 }
 P3^ := P1^ xor P2^;
 {
 00460CAB 8B0A             mov ecx,[edx]
 00460CAD 330B             xor ecx,[ebx]
 00460CAF 8908             mov [eax],ecx
 }
 Inc(P1);
 {
 00460CB1 83C204           add edx,$04
 }
 Inc(P2);
 {
 00460CB4 83C304           add ebx,$04
 }
 Inc(P3);
 {
 00460CB7 83C004           add eax,$04
 }
 P3^ := P1^ xor P2^;
 {
 00460CBA 8B0A             mov ecx,[edx]
 00460CBC 330B             xor ecx,[ebx]
 00460CBE 8908             mov [eax],ecx
 }
 Inc(P1);
 {
 00460CC0 83C204           add edx,$04
 }
 Inc(P2);
 {
 00460CC3 83C304           add ebx,$04
 }
 Inc(P3);
 {
 00460CC6 83C004           add eax,$04
 }
 P3^ := P1^ xor P2^;
 {
 00460CC9 8B12             mov edx,[edx]
 00460CCB 3313             xor edx,[ebx]
 00460CCD 8910             mov [eax],edx
 }
{
00460CCF 83C424           add esp,$24
00460CD2 5D               pop ebp
00460CD3 5F               pop edi
00460CD4 5E               pop esi
00460CD5 5B               pop ebx
ret
}
end;

procedure EncryptAES_DKC_IA32_1b(var Block);
var
 I, IMax : Integer;
 Icardinal : Cardinal;
 pBlock : ^TAESBlock;
 P1, P2, P3 : ^Cardinal;
 C0, C1, C2, C3 : Cardinal;
 E5 : array[0..3] of TAESVector;
 E6 : TAESVectors;

begin
{
push ebp
push esi
push edi
push ebp
add esp,-$24
lea ecx,[esp+$14]
mov esi,$00474bd4
}
 pBlock := @Block;
 {
 }
 P1 := @pBlock^;
 {
 mov edx,eax
 }
 P2 := @pDKCContext.RoundKeys.Rk[0];
 {
 mov ebx,[esi]
 }
 //Add round key
 P1^ := P1^ xor P2^;
 {
 mov edi,[ebx]
 xor [edx],edi
 }
 Inc(P1);
 {
 add edx,$04
 }
 Inc(P2);
 {
 add ebx,$04
 }
 P1^ := P1^ xor P2^;
 {
 mov edi,[ebx]
 xor [edx],edi
 }
 Inc(P1);
 {
 add edx,$04
 }
 Inc(P2);
 {
 add ebx,$04
 }
 P1^ := P1^ xor P2^;
 {
 mov edi,[ebx]
 xor [edx],edi
 }
 Inc(P1);
 {
 add edx,$04
 }
 Inc(P2);
 {
 add ebx,$04
 }
 P1^ := P1^ xor P2^;
 {
 mov ebx,[ebx]
 xor [edx],ebx
 }
 IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
 {
 mov edx,[esi]
 mov ebx,[edx+$00000101]
 dec ebx
 shr ebx,1
 }
 Icardinal := 3;
 {
 mov edx,$00000003
 }
 for I := 1 to IMax do
 {
 test ebx,ebx
 jle +$000001b3
 }
  begin
   Inc(Icardinal);
   {
   inc edx
   }
   E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   movzx edi,[eax]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[eax+$05]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0a]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0f]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [ecx],edi
   }
   Inc(Icardinal);
   {
   inc edx
   }
   E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   movzx edi,[eax+$04]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[eax+$09]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0e]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$03]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [ecx+$04],edi
   }
   Inc(Icardinal);
   {
   inc edx
   }
   E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   movzx edi,[eax+$08]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[eax+$0d]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$02]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$07]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [ecx+$08],edi
   }
   Inc(Icardinal);
   {
   inc edx
   }
   E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
                AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
                AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
                AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
                pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   movzx edi,[eax+$0c]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[eax+$01]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$06]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0b]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [ecx+$0c],edi
   }
   //**********************************************
   Inc(Icardinal);
   {
   inc edx
   }
   TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
                                  AES_T1[E6[1].bt[1]] xor
                                  AES_T2[E6[2].bt[2]] xor
                                  AES_T3[E6[3].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   movzx edi,[ecx]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$05]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0a]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0f]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [eax],edi
   }
   Inc(Icardinal);
   {
   inc edx
   }
   TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
                                  AES_T1[E6[2].bt[1]] xor
                                  AES_T2[E6[3].bt[2]] xor
                                  AES_T3[E6[0].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   movzx edi,[ecx+$04]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$09]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0e]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$03]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [eax+$04],edi
   }
   Inc(Icardinal);
   {
   inc edx
   }
   TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
                                  AES_T1[E6[3].bt[1]] xor
                                  AES_T2[E6[0].bt[2]] xor
                                  AES_T3[E6[1].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   movzx edi,[ecx+$08]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$0d]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$02]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$07]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [eax+$08],edi
   }
   Inc(Icardinal);
   {
   inc edx
   }
   TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
                                  AES_T1[E6[0].bt[1]] xor
                                  AES_T2[E6[1].bt[2]] xor
                                  AES_T3[E6[2].bt[3]]) xor
                                  pDKCContext.RoundKeys.W[Icardinal].dw;
   {
   movzx edi,[ecx+$0c]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$01]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$06]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0b]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [eax+$0c],edi
   }
  end;
  {
  dec ebx
  jnz -$000001b3
  }
 //Do last round****************
 Inc(Icardinal);
 {
 inc edx
 }
 C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 {
 xor ecx,ecx
 mov cl,[eax]
 mov ecx,[ecx*4+AES_T0]
 xor ebx,ebx
 mov bl,[eax+$05]
 xor ecx,[ebx*4+AES_T1]
 xor ebx,ebx
 mov bl,[eax+$0a]
 xor ecx,[ebx*4+AES_T2]
 xor ebx,ebx
 mov bl,[eax+$0f]
 xor ecx,[ebx*4+AES_T3]
 mov ebx,[esi]
 xor ecx,[ebx+edx*4]
 }
 Inc(Icardinal);
 {
 inc edx
 }
 C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 {
 xor ebx,ebx
 mov bl,[eax+$04]
 mov ebx,[ebx*4+AES_T0]
 movzx edi,[eax+$09]
 xor ebx,[edi*4+AES_T1]
 movzx edi,[eax+$0e]
 xor ebx,[edi*4+AES_T2]
 movzx edi,[eax+$03]
 xor ebx,[edi*4+AES_T3]
 mov edi,[esi]
 xor ebx,[edi+edx*4]
 }
 Inc(Icardinal);
 {
 inc edx
 }
 C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 {
 movzx edi,[eax+$08]
 mov edi,[edi*4+AES_T0]
 movzx ebp,[eax+$0d]
 xor edi,[ebp*4+AES_T1]
 movzx ebp,[eax+$02]
 xor edi,[ebp*4+AES_T2]
 movzx ebp,[eax+$07]
 xor edi,[ebp*4+AES_T3]
 mov ebp,[esi]
 xor edi,[ebp+edx*4+$00]
 mov [esp],edi
 }
 Inc(Icardinal);
 {
 inc edx
 }
 C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
        AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
        AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
        AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
        pDKCContext.RoundKeys.W[Icardinal].dw;
 {
 movzx edi,[eax+$0c]
 mov edi,[edi*4+AES_T0]
 movzx ebp,[eax+$01]
 xor edi,[ebp*4+AES_T1]
 movzx ebp,[eax+$06]
 xor edi,[ebp*4+AES_T2]
 movzx ebp,[eax+$0b]
 xor edi,[ebp*4+AES_T3]
 mov ebp,[esi]
 xor edi,[ebp+edx*4+$00]
 mov edx,edi
 }
 TAESVectors(pBlock^)[0].dw := C0;
 {
 mov [eax],ecx
 }
 TAESVectors(pBlock^)[1].dw := C1;
 {
 mov [eax+$04],ebx
 }
 TAESVectors(pBlock^)[2].dw := C2;
 {
 mov ecx,[esp]
 mov [eax+$08],ecx
 }
 TAESVectors(pBlock^)[3].dw := C3;
 {
 mov [eax+$0c],edx
 }
 //****************************
 E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
 {
 xor edx,edx
 mov dl,[eax]
 mov dl,[edx+AESSBox]
 mov [esp+$04],dl
 }
 E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
 {
 xor edx,edx
 mov dl,[eax+$05]
 mov dl,[edx+AESSBox]
 mov [esp+$05],dl
 }
 E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
 {
 xor edx,edx
 mov dl,[eax+$0a]
 mov dl,[edx+AESSBox]
 mov [esp+$06],dl
 }
 E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
 {
 xor edx,edx
 mov dl,[eax+$0f]
 mov dl,[edx+AESSBox]
 mov [esp+$07],dl
 }
 E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
 {
 xor edx,edx
 mov dl,[eax+$04]
 mov dl,[edx+AESSBox]
 mov [esp+$08],dl
 }
 E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
 {
 xor edx,edx
 mov dl,[eax+$09]
 mov dl,[edx+AESSBox]
 mov [esp+$09],dl
 }
 E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
 {
 xor edx,edx
 mov dl,[eax+$0e]
 mov dl,[edx+AESSBox]
 mov [esp+$0a],dl
 }
 E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
 {
 xor edx,edx
 mov dl,[eax+$03]
 mov dl,[edx+AESSBox]
 mov [esp+$0b],dl
 }
 E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
 {
 xor edx,edx
 mov dl,[eax+$08]
 mov dl,[edx+AESSBox]
 mov [esp+$0c],dl
 }
 E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
 {
 xor edx,edx
 mov dl,[eax+$0d]
 mov dl,[edx+AESSBox]
 mov [esp+$0d],dl
 }
 E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
 {
 xor edx,edx
 mov dl,[eax+$02]
 mov dl,[edx+AESSBox]
 mov [esp+$0e],dl
 }
 E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
 {
 xor edx,edx
 mov dl,[eax+$07]
 mov dl,[edx+AESSBox]
 mov [esp+$0f],dl
 }
 E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
 {
 xor edx,edx
 mov dl,[eax+$0c]
 mov dl,[edx+AESSBox]
 mov [esp+$10],dl
 }
 E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
 {
 xor edx,edx
 mov dl,[eax+$01]
 mov dl,[edx+AESSBox]
 mov [esp+$11],dl
 }
 E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
 {
 xor edx,edx
 mov dl,[eax+$06]
 mov dl,[edx+AESSBox]
 mov [esp+$12],dl
 }
 E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
 {
 xor edx,edx
 mov dl,[eax+$0b]
 mov dl,[edx+AESSBox]
 mov [esp+$13],dl
 }
 P1 := @E5;
 {
 lea edx,[esp+$04]
 }
 P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
 {
 mov ecx,[esi]
 mov ecx,[ecx+$00000101]
 add ecx,ecx
 }
 P3 := @pBlock^;
 {
 mov ebx,[esi]
 lea ebx,[ebx+ecx*8]
 }
 P3^ := P1^ xor P2^;
 {
 mov ecx,[edx]
 xor ecx,[ebx]
 mov [eax],ecx
 }
 Inc(P1);
 {
 add edx,$04
 }
 Inc(P2);
 {
 add ebx,$04
 }
 Inc(P3);
 {
 add eax,$04
 }
 P3^ := P1^ xor P2^;
 {
 mov ecx,[edx]
 xor ecx,[ebx]
 mov [eax],ecx
 }
 Inc(P1);
 {
 add edx,$04
 }
 Inc(P2);
 {
 add ebx,$04
 }
 Inc(P3);
 {
 add eax,$04
 }
 P3^ := P1^ xor P2^;
 {
 mov ecx,[edx]
 xor ecx,[ebx]
 mov [eax],ecx
 }
 Inc(P1);
 {
 add edx,$04
 }
 Inc(P2);
 {
 add ebx,$04
 }
 Inc(P3);
 {
 add eax,$04
 }
 P3^ := P1^ xor P2^;
 {
 mov edx,[edx]
 xor edx,[ebx]
 mov [eax],edx
 }
{
add esp,$24
pop ebp
pop edi
pop esi
pop ebx
ret
}
end;

procedure EncryptAES_DKC_IA32_1c(var Block);
asm

push ebx
push esi
push edi
push ebp
add esp,-$24
lea ecx,[esp+$14]
mov esi,$00474bd4

 //pBlock := @Block;


 //P1 := @pBlock^;

 mov edx,eax

 //P2 := @pDKCContext.RoundKeys.Rk[0];

 mov ebx,[esi]

 //Add round key
 //P1^ := P1^ xor P2^;

 mov edi,[ebx]
 xor [edx],edi

 //Inc(P1);

 add edx,$04

 //Inc(P2);

 add ebx,$04

 //P1^ := P1^ xor P2^;

 mov edi,[ebx]
 xor [edx],edi

 //Inc(P1);

 add edx,$04

 //Inc(P2);

 add ebx,$04

 //P1^ := P1^ xor P2^;

 mov edi,[ebx]
 xor [edx],edi

 //Inc(P1);

 add edx,$04

 //Inc(P2);

 add ebx,$04

 //P1^ := P1^ xor P2^;

 mov ebx,[ebx]
 xor [edx],ebx

 //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations

 mov edx,[esi]
 mov ebx,[edx+$00000101]
 dec ebx
 shr ebx,1

 //Icardinal := 3;

 mov edx,$00000003

 //for I := 1 to IMax do

 test ebx,ebx
 jle @LoopEnd

  //begin
   //Inc(Icardinal);
 @LoopStart :
   inc edx

   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;

   movzx edi,[eax]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[eax+$05]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0a]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0f]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [ecx],edi

   //Inc(Icardinal);

   inc edx

   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;

   movzx edi,[eax+$04]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[eax+$09]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0e]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$03]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [ecx+$04],edi

   //Inc(Icardinal);

   inc edx

   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;

   movzx edi,[eax+$08]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[eax+$0d]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$02]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$07]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [ecx+$08],edi

   //Inc(Icardinal);

   inc edx

   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;

   movzx edi,[eax+$0c]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[eax+$01]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$06]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0b]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [ecx+$0c],edi

   //**********************************************
   //Inc(Icardinal);

   inc edx

   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;

   movzx edi,[ecx]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$05]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0a]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0f]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [eax],edi

   //Inc(Icardinal);

   inc edx

   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;

   movzx edi,[ecx+$04]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$09]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0e]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$03]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [eax+$04],edi

   //Inc(Icardinal);

   inc edx

   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;

   movzx edi,[ecx+$08]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$0d]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$02]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$07]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [eax+$08],edi

   //Inc(Icardinal);

   inc edx

   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;

   movzx edi,[ecx+$0c]
   mov edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$01]
   xor edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$06]
   xor edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0b]
   xor edi,[ebp*4+AES_T3]
   mov ebp,[esi]
   xor edi,[ebp+edx*4+$00]
   mov [eax+$0c],edi

  //end;

  dec ebx
  jnz @LoopStart
 @LoopEnd :
 //Do last round****************
 //Inc(Icardinal);

 inc edx

 //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
 //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
 //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
 //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
 //       pDKCContext.RoundKeys.W[Icardinal].dw;

 xor ecx,ecx
 mov cl,[eax]
 mov ecx,[ecx*4+AES_T0]
 xor ebx,ebx
 mov bl,[eax+$05]
 xor ecx,[ebx*4+AES_T1]
 xor ebx,ebx
 mov bl,[eax+$0a]
 xor ecx,[ebx*4+AES_T2]
 xor ebx,ebx
 mov bl,[eax+$0f]
 xor ecx,[ebx*4+AES_T3]
 mov ebx,[esi]
 xor ecx,[ebx+edx*4]

 //Inc(Icardinal);

 inc edx

 //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
 //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
 //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
 //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
 //       pDKCContext.RoundKeys.W[Icardinal].dw;

 xor ebx,ebx
 mov bl,[eax+$04]
 mov ebx,[ebx*4+AES_T0]
 movzx edi,[eax+$09]
 xor ebx,[edi*4+AES_T1]
 movzx edi,[eax+$0e]
 xor ebx,[edi*4+AES_T2]
 movzx edi,[eax+$03]
 xor ebx,[edi*4+AES_T3]
 mov edi,[esi]
 xor ebx,[edi+edx*4]

 //Inc(Icardinal);

 inc edx

 //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
 //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
 //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
 //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
 //       pDKCContext.RoundKeys.W[Icardinal].dw;

 movzx edi,[eax+$08]
 mov edi,[edi*4+AES_T0]
 movzx ebp,[eax+$0d]
 xor edi,[ebp*4+AES_T1]
 movzx ebp,[eax+$02]
 xor edi,[ebp*4+AES_T2]
 movzx ebp,[eax+$07]
 xor edi,[ebp*4+AES_T3]
 mov ebp,[esi]
 xor edi,[ebp+edx*4+$00]
 mov [esp],edi

 //Inc(Icardinal);

 inc edx

 //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
 //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
 //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
 //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
 //       pDKCContext.RoundKeys.W[Icardinal].dw;

 movzx edi,[eax+$0c]
 mov edi,[edi*4+AES_T0]
 movzx ebp,[eax+$01]
 xor edi,[ebp*4+AES_T1]
 movzx ebp,[eax+$06]
 xor edi,[ebp*4+AES_T2]
 movzx ebp,[eax+$0b]
 xor edi,[ebp*4+AES_T3]
 mov ebp,[esi]
 xor edi,[ebp+edx*4+$00]
 mov edx,edi

 //TAESVectors(pBlock^)[0].dw := C0;

 mov [eax],ecx

 //TAESVectors(pBlock^)[1].dw := C1;

 mov [eax+$04],ebx

 //TAESVectors(pBlock^)[2].dw := C2;

 mov ecx,[esp]
 mov [eax+$08],ecx

 //TAESVectors(pBlock^)[3].dw := C3;

 mov [eax+$0c],edx

 //****************************
 //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];

 xor edx,edx
 mov dl,[eax]
 mov dl,[edx+AESSBox]
 mov [esp+$04],dl

 //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];

 xor edx,edx
 mov dl,[eax+$05]
 mov dl,[edx+AESSBox]
 mov [esp+$05],dl

 //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];

 xor edx,edx
 mov dl,[eax+$0a]
 mov dl,[edx+AESSBox]
 mov [esp+$06],dl

 //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];

 xor edx,edx
 mov dl,[eax+$0f]
 mov dl,[edx+AESSBox]
 mov [esp+$07],dl

 //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];

 xor edx,edx
 mov dl,[eax+$04]
 mov dl,[edx+AESSBox]
 mov [esp+$08],dl

 //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];

 xor edx,edx
 mov dl,[eax+$09]
 mov dl,[edx+AESSBox]
 mov [esp+$09],dl

 //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];

 xor edx,edx
 mov dl,[eax+$0e]
 mov dl,[edx+AESSBox]
 mov [esp+$0a],dl

 //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];

 xor edx,edx
 mov dl,[eax+$03]
 mov dl,[edx+AESSBox]
 mov [esp+$0b],dl

 //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];

 xor edx,edx
 mov dl,[eax+$08]
 mov dl,[edx+AESSBox]
 mov [esp+$0c],dl

 //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];

 xor edx,edx
 mov dl,[eax+$0d]
 mov dl,[edx+AESSBox]
 mov [esp+$0d],dl

 //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];

 xor edx,edx
 mov dl,[eax+$02]
 mov dl,[edx+AESSBox]
 mov [esp+$0e],dl

 //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];

 xor edx,edx
 mov dl,[eax+$07]
 mov dl,[edx+AESSBox]
 mov [esp+$0f],dl

 //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];

 xor edx,edx
 mov dl,[eax+$0c]
 mov dl,[edx+AESSBox]
 mov [esp+$10],dl

 //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];

 xor edx,edx
 mov dl,[eax+$01]
 mov dl,[edx+AESSBox]
 mov [esp+$11],dl

 //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];

 xor edx,edx
 mov dl,[eax+$06]
 mov dl,[edx+AESSBox]
 mov [esp+$12],dl

 //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];

 xor edx,edx
 mov dl,[eax+$0b]
 mov dl,[edx+AESSBox]
 mov [esp+$13],dl

 //P1 := @E5;

 lea edx,[esp+$04]

 //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];

 mov ecx,[esi]
 mov ecx,[ecx+$00000101]
 add ecx,ecx

 //P3 := @pBlock^;

 mov ebx,[esi]
 lea ebx,[ebx+ecx*8]

 //P3^ := P1^ xor P2^;

 mov ecx,[edx]
 xor ecx,[ebx]
 mov [eax],ecx

 //Inc(P1);

 add edx,$04

 //Inc(P2);

 add ebx,$04

 //Inc(P3);

 add eax,$04

 //P3^ := P1^ xor P2^;

 mov ecx,[edx]
 xor ecx,[ebx]
 mov [eax],ecx

 //Inc(P1);

 add edx,$04

 //Inc(P2);

 add ebx,$04

 //Inc(P3);

 add eax,$04

 //P3^ := P1^ xor P2^;

 mov ecx,[edx]
 xor ecx,[ebx]
 mov [eax],ecx

 //Inc(P1);

 add edx,$04

 //Inc(P2);

 add ebx,$04

 //Inc(P3);

 add eax,$04

 //P3^ := P1^ xor P2^;

 mov edx,[edx]
 xor edx,[ebx]
 mov [eax],edx


add esp,$24
pop ebp
pop edi
pop esi
pop ebx
//ret

end;

procedure EncryptAES_DKC_IA32_1d(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   add   esp,-$24
   lea   ecx,[esp+$14]
   mov   esi,$00474bd4
   //pBlock := @Block;
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,[esi]
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,[esi]
   mov   ebx,[edx+$00000101]
   dec   ebx
   shr   ebx,1
   //Icardinal := 3;
   mov   edx,$00000003
   //for I := 1 to IMax do
   test  ebx,ebx
   jle   @LoopEnd
   //begin
   //Inc(Icardinal);
 @LoopStart :
   add   edx,1
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$05]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0a]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0f]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   edx,1
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$04]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$09]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0e]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$03]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [ecx+$04],edi
   //Inc(Icardinal);
   add   edx,1
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$08]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$0d]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$02]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$07]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [ecx+$08],edi
   //Inc(Icardinal);
   add   edx,1
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$0c]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$01]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$06]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0b]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [ecx+$0c],edi
   //**********************************************
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[ecx]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$05]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0a]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0f]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [eax],edi
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[ecx+$04]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$09]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0e]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$03]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [eax+$04],edi
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[ecx+$08]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$0d]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$02]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$07]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [eax+$08],edi
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[ecx+$0c]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$01]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$06]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0b]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [eax+$0c],edi
   dec   ebx
   jnz @LoopStart
 @LoopEnd :
   //Do last round****************
   //Inc(Icardinal);
   add   edx,1
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   xor   ecx,ecx
   mov   cl,[eax]
   mov   ecx,[ecx*4+AES_T0]
   xor   ebx,ebx
   mov   bl,[eax+$05]
   xor   ecx,[ebx*4+AES_T1]
   xor   ebx,ebx
   mov   bl,[eax+$0a]
   xor   ecx,[ebx*4+AES_T2]
   xor   ebx,ebx
   mov   bl,[eax+$0f]
   xor   ecx,[ebx*4+AES_T3]
   mov   ebx,[esi]
   xor   ecx,[ebx+edx*4]
   //Inc(Icardinal);
   add   edx,1
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   xor   ebx,ebx
   mov   bl,[eax+$04]
   mov   ebx,[ebx*4+AES_T0]
   movzx edi,[eax+$09]
   xor   ebx,[edi*4+AES_T1]
   movzx edi,[eax+$0e]
   xor   ebx,[edi*4+AES_T2]
   movzx edi,[eax+$03]
   xor   ebx,[edi*4+AES_T3]
   mov   edi,[esi]
   xor   ebx,[edi+edx*4]
   //Inc(Icardinal);
   add   edx,1
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$08]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$0d]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$02]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$07]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [esp],edi
   //Inc(Icardinal);
   add   edx,1
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$0c]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$01]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$06]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0b]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+$04],ebx
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+$08],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+$0c],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   xor   edx,edx
   mov   dl,[eax]
   mov   dl,[edx+AESSBox]
   mov   [esp+$04],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   xor   edx,edx
   mov   dl,[eax+$05]
   mov   dl,[edx+AESSBox]
   mov   [esp+$05],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   xor   edx,edx
   mov   dl,[eax+$0a]
   mov   dl,[edx+AESSBox]
   mov   [esp+$06],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   xor   edx,edx
   mov   dl,[eax+$0f]
   mov   dl,[edx+AESSBox]
   mov   [esp+$07],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   xor   edx,edx
   mov   dl,[eax+$04]
   mov   dl,[edx+AESSBox]
   mov   [esp+$08],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   xor   edx,edx
   mov   dl,[eax+$09]
   mov   dl,[edx+AESSBox]
   mov   [esp+$09],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   xor   edx,edx
   mov   dl,[eax+$0e]
   mov   dl,[edx+AESSBox]
   mov   [esp+$0a],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   xor   edx,edx
   mov   dl,[eax+$03]
   mov   dl,[edx+AESSBox]
   mov   [esp+$0b],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   xor   edx,edx
   mov   dl,[eax+$08]
   mov   dl,[edx+AESSBox]
   mov   [esp+$0c],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   xor   edx,edx
   mov   dl,[eax+$0d]
   mov   dl,[edx+AESSBox]
   mov   [esp+$0d],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   xor   edx,edx
   mov   dl,[eax+$02]
   mov   dl,[edx+AESSBox]
   mov   [esp+$0e],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   xor   edx,edx
   mov   dl,[eax+$07]
   mov   dl,[edx+AESSBox]
   mov   [esp+$0f],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   xor   edx,edx
   mov   dl,[eax+$0c]
   mov   dl,[edx+AESSBox]
   mov   [esp+$10],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   xor   edx,edx
   mov   dl,[eax+$01]
   mov   dl,[edx+AESSBox]
   mov   [esp+$11],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   xor   edx,edx
   mov   dl,[eax+$06]
   mov   dl,[edx+AESSBox]
   mov   [esp+$12],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   xor   edx,edx
   mov   dl,[eax+$0b]
   mov   dl,[edx+AESSBox]
   mov   [esp+$13],dl
   //P1 := @E5;
   lea   edx,[esp+$04]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   mov   ecx,[esi]
   mov   ecx,[ecx+$00000101]
   add   ecx,ecx
   //P3 := @pBlock^;
   mov   ebx,[esi]
   lea   ebx,[ebx+ecx*8]
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //Inc(P3);
   add   eax,$04
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //Inc(P3);
   add   eax,$04
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //Inc(P3);
   add   eax,$04
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[ebx]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;

procedure EncryptAES_DKC_IA32_1e(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   sub   esp,$24
   lea   ecx,[esp+$14]
   mov   esi,$00474bd4
   //pBlock := @Block;
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,[esi]
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,[esi]
   mov   ebx,[edx+$00000101]
   sub   ebx,1
   shr   ebx,1
   //Icardinal := 3;
   mov   edx,3
   //for I := 1 to IMax do
   //test  ebx,ebx
   //jle   @LoopEnd
   //Inc(Icardinal);
 @LoopStart :
   add   edx,1
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$05]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0a]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0f]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   edx,1
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$04]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$09]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0e]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$03]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [ecx+$04],edi
   //Inc(Icardinal);
   add   edx,1
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$08]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$0d]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$02]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$07]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [ecx+$08],edi
   //Inc(Icardinal);
   add   edx,1
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$0c]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$01]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$06]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0b]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [ecx+$0c],edi
   //**********************************************
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[ecx]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$05]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0a]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0f]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [eax],edi
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[ecx+$04]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$09]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0e]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$03]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [eax+$04],edi
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[ecx+$08]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$0d]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$02]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$07]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [eax+$08],edi
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[ecx+$0c]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[ecx+$01]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$06]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0b]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [eax+$0c],edi
   dec   ebx
   jnz @LoopStart
 @LoopEnd :
   //Do last round****************
   //Inc(Icardinal);
   add   edx,1
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   //xor   ecx,ecx
   movzx ecx,[eax]
   mov   ecx,[ecx*4+AES_T0]
   //xor   ebx,ebx
   movzx ebx,[eax+$05]
   xor   ecx,[ebx*4+AES_T1]
   //xor   ebx,ebx
   movzx ebx,[eax+$0a]
   xor   ecx,[ebx*4+AES_T2]
   //xor   ebx,ebx
   movzx ebx,[eax+$0f]
   xor   ecx,[ebx*4+AES_T3]
   mov   ebx,[esi]
   xor   ecx,[ebx+edx*4]
   //Inc(Icardinal);
   add   edx,1
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   //xor   ebx,ebx
   movzx ebx,[eax+$04]
   mov   ebx,[ebx*4+AES_T0]
   movzx edi,[eax+$09]
   xor   ebx,[edi*4+AES_T1]
   movzx edi,[eax+$0e]
   xor   ebx,[edi*4+AES_T2]
   movzx edi,[eax+$03]
   xor   ebx,[edi*4+AES_T3]
   mov   edi,[esi]
   xor   ebx,[edi+edx*4]
   //Inc(Icardinal);
   add   edx,1
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$08]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$0d]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$02]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$07]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   [esp],edi
   //Inc(Icardinal);
   add   edx,1
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edi,[eax+$0c]
   mov   edi,[edi*4+AES_T0]
   movzx ebp,[eax+$01]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$06]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0b]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4+$00]
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+$04],ebx
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+$08],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+$0c],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   //xor   edx,edx
   movzx edx,[eax]
   movzx edx,[edx+AESSBox]
   mov   [esp+$04],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   //xor   edx,edx
   movzx edx,[eax+$05]
   movzx edx,[edx+AESSBox]
   mov   [esp+$05],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   //xor   edx,edx
   movzx edx,[eax+$0a]
   movzx edx,[edx+AESSBox]
   mov   [esp+$06],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   //xor   edx,edx
   movzx edx,[eax+$0f]
   movzx edx,[edx+AESSBox]
   mov   [esp+$07],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   //xor   edx,edx
   movzx edx,[eax+$04]
   movzx edx,[edx+AESSBox]
   mov   [esp+$08],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   //xor   edx,edx
   movzx edx,[eax+$09]
   movzx edx,[edx+AESSBox]
   mov   [esp+$09],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   //xor   edx,edx
   movzx edx,[eax+$0e]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0a],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   //xor   edx,edx
   movzx edx,[eax+$03]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0b],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   //xor   edx,edx
   movzx edx,[eax+$08]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0c],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   //xor   edx,edx
   movzx edx,[eax+$0d]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0d],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   //xor   edx,edx
   movzx edx,[eax+$02]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0e],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   //xor   edx,edx
   movzx edx,[eax+$07]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0f],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   //xor   edx,edx
   movzx edx,[eax+$0c]
   movzx edx,[edx+AESSBox]
   mov   [esp+$10],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   //xor   edx,edx
   movzx edx,[eax+$01]
   movzx edx,[edx+AESSBox]
   mov   [esp+$11],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   //xor   edx,edx
   movzx edx,[eax+$06]
   movzx edx,[edx+AESSBox]
   mov   [esp+$12],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   //xor   edx,edx
   movzx edx,[eax+$0b]
   movzx edx,[edx+AESSBox]
   mov   [esp+$13],dl
   //P1 := @E5;
   lea   edx,[esp+$04]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   mov   ecx,[esi]
   mov   ecx,[ecx+$00000101]
   add   ecx,ecx
   //P3 := @pBlock^;
   mov   ebx,[esi]
   lea   ebx,[ebx+ecx*8]
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //Inc(P3);
   add   eax,$04
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //Inc(P3);
   add   eax,$04
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //Inc(P3);
   add   eax,$04
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[ebx]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;

procedure EncryptAES_DKC_IA32_1f(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   sub   esp,$24
   lea   ecx,[esp+$14]
   mov   esi,$00474bd4
   //pBlock := @Block;
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,[esi]
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,[esi]
   mov   ebx,[edx+$00000101]
   sub   ebx,1
   shr   ebx,1
   //Icardinal := 3;
   mov   edx,3
   //for I := 1 to IMax do
 @LoopStart :
   //Inc(Icardinal);
   add   edx,1
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+$05]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0a]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0f]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   edx,1
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+$04]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+$09]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$0e]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$03]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   [ecx+$04],edi
   //Inc(Icardinal);
   add   edx,1
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+$08]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+$0d]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$02]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$07]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   [ecx+$08],edi
   //Inc(Icardinal);
   add   edx,1
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+$0c]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+$01]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$06]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0b]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   [ecx+$0c],edi
   //**********************************************
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+$05]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0a]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0f]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   [eax],edi
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+$04]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+$09]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$0e]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$03]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   [eax+$04],edi
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+$08]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+$0d]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$02]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$07]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   [eax+$08],edi
   //Inc(Icardinal);
   add   edx,1
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+$0c]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+$01]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+$06]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+$0b]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   [eax+$0c],edi
   dec   ebx
   jnz @LoopStart
 @LoopEnd :
   //Do last round****************
   //Inc(Icardinal);
   add   edx,1
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   ecx,[ebp*4+AES_T0]
   movzx ebp,[eax+$05]
   xor   ecx,[ebp*4+AES_T1]
   movzx ebp,[eax+$0a]
   xor   ecx,[ebp*4+AES_T2]
   movzx ebp,[eax+$0f]
   xor   ecx,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   ecx,[ebp+edx*4]
   //Inc(Icardinal);
   add   edx,1
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+$04]
   mov   ebx,[ebp*4+AES_T0]
   movzx ebp,[eax+$09]
   xor   ebx,[ebp*4+AES_T1]
   movzx ebp,[eax+$0e]
   xor   ebx,[ebp*4+AES_T2]
   movzx ebp,[eax+$03]
   xor   ebx,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   ebx,[ebp+edx*4]
   //Inc(Icardinal);
   add   edx,1
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+$08]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+$0d]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$02]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$07]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   [esp],edi
   //Inc(Icardinal);
   add   edx,1
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+$0c]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+$01]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+$06]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+$0b]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx*4]
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+$04],ebx
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+$08],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+$0c],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   movzx edx,[eax]
   movzx edx,[edx+AESSBox]
   mov   [esp+$04],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   movzx edx,[eax+$05]
   movzx edx,[edx+AESSBox]
   mov   [esp+$05],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   movzx edx,[eax+$0a]
   movzx edx,[edx+AESSBox]
   mov   [esp+$06],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   movzx edx,[eax+$0f]
   movzx edx,[edx+AESSBox]
   mov   [esp+$07],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   movzx edx,[eax+$04]
   movzx edx,[edx+AESSBox]
   mov   [esp+$08],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   movzx edx,[eax+$09]
   movzx edx,[edx+AESSBox]
   mov   [esp+$09],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   movzx edx,[eax+$0e]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0a],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   movzx edx,[eax+$03]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0b],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   movzx edx,[eax+$08]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0c],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   movzx edx,[eax+$0d]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0d],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   movzx edx,[eax+$02]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0e],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   movzx edx,[eax+$07]
   movzx edx,[edx+AESSBox]
   mov   [esp+$0f],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   movzx edx,[eax+$0c]
   movzx edx,[edx+AESSBox]
   mov   [esp+$10],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   movzx edx,[eax+$01]
   movzx edx,[edx+AESSBox]
   mov   [esp+$11],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   movzx edx,[eax+$06]
   movzx edx,[edx+AESSBox]
   mov   [esp+$12],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   movzx edx,[eax+$0b]
   movzx edx,[edx+AESSBox]
   mov   [esp+$13],dl
   //P1 := @E5;
   lea   edx,[esp+$04]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   mov   ecx,[esi]
   mov   ecx,[ecx+$00000101]
   add   ecx,ecx
   //P3 := @pBlock^;
   mov   ebx,[esi]
   lea   ebx,[ebx+ecx*8]
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //Inc(P3);
   add   eax,$04
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //Inc(P3);
   add   eax,$04
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,$04
   //Inc(P2);
   add   ebx,$04
   //Inc(P3);
   add   eax,$04
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[ebx]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;

procedure EncryptAES_DKC_IA32_1g(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   sub   esp,$24
   //pBlock := @Block;
   lea   ecx,[esp+$14]
   mov   esi,$00474bd4
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,[esi]
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,[esi]
   mov   ebx,[edx+$00000101]
   sub   ebx,1
   shr   ebx,1
   //Icardinal := 3;
   mov   edx,12
   //for I := 1 to IMax do
 @LoopStart :
   //Inc(Icardinal);
   add   edx,4
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+5]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+10]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+15]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   edx,4
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+4]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+9]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+14]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+3]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   [ecx+4],edi
   //Inc(Icardinal);
   add   edx,4
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+7]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   [ecx+8],edi
   //Inc(Icardinal);
   add   edx,4
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+11]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   [ecx+12],edi
   //**********************************************
   //Inc(Icardinal);
   add   edx,4
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+5]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+10]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+15]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   [eax],edi
   //Inc(Icardinal);
   add   edx,4
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+4]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+9]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+14]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+3]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   [eax+4],edi
   //Inc(Icardinal);
   add   edx,4
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+7]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   [eax+8],edi
   //Inc(Icardinal);
   add   edx,4
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+11]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   [eax+12],edi
   dec   ebx
   jnz   @LoopStart
 @LoopEnd :
   //Do last round****************
   //Inc(Icardinal);
   add   edx,4
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   ecx,[ebp*4+AES_T0]
   movzx ebp,[eax+5]
   xor   ecx,[ebp*4+AES_T1]
   movzx ebp,[eax+10]
   xor   ecx,[ebp*4+AES_T2]
   movzx ebp,[eax+15]
   xor   ecx,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   ecx,[ebp+edx]
   //Inc(Icardinal);
   add   edx,4
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+4]
   mov   ebx,[ebp*4+AES_T0]
   movzx ebp,[eax+9]
   xor   ebx,[ebp*4+AES_T1]
   movzx ebp,[eax+14]
   xor   ebx,[ebp*4+AES_T2]
   movzx ebp,[eax+3]
   xor   ebx,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   ebx,[ebp+edx]
   //Inc(Icardinal);
   add   edx,4
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+7]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   [esp],edi
   //Inc(Icardinal);
   add   edx,4
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+11]
   xor   edi,[ebp*4+AES_T3]
   mov   ebp,[esi]
   xor   edi,[ebp+edx]
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+4],ebx
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+8],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+12],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   movzx edx,[eax]
   movzx edx,[edx+AESSBox]
   mov   [esp+4],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   movzx edx,[eax+5]
   movzx edx,[edx+AESSBox]
   mov   [esp+5],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   movzx edx,[eax+10]
   movzx edx,[edx+AESSBox]
   mov   [esp+6],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   movzx edx,[eax+15]
   movzx edx,[edx+AESSBox]
   mov   [esp+7],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   movzx edx,[eax+4]
   movzx edx,[edx+AESSBox]
   mov   [esp+8],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   movzx edx,[eax+9]
   movzx edx,[edx+AESSBox]
   mov   [esp+9],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   movzx edx,[eax+14]
   movzx edx,[edx+AESSBox]
   mov   [esp+10],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   movzx edx,[eax+3]
   movzx edx,[edx+AESSBox]
   mov   [esp+11],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   movzx edx,[eax+8]
   movzx edx,[edx+AESSBox]
   mov   [esp+12],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   movzx edx,[eax+13]
   movzx edx,[edx+AESSBox]
   mov   [esp+13],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   movzx edx,[eax+2]
   movzx edx,[edx+AESSBox]
   mov   [esp+14],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   movzx edx,[eax+7]
   movzx edx,[edx+AESSBox]
   mov   [esp+15],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   movzx edx,[eax+12]
   movzx edx,[edx+AESSBox]
   mov   [esp+16],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   movzx edx,[eax+1]
   movzx edx,[edx+AESSBox]
   mov   [esp+17],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   movzx edx,[eax+6]
   movzx edx,[edx+AESSBox]
   mov   [esp+18],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   movzx edx,[eax+11]
   movzx edx,[edx+AESSBox]
   mov   [esp+19],dl
   //P1 := @E5;
   lea   edx,[esp+4]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   mov   ecx,[esi]
   mov   ecx,[ecx+$101]
   add   ecx,ecx
   //P3 := @pBlock^;
   mov   ebx,[esi]
   lea   ebx,[ebx+ecx*8]
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[ebx]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;

procedure EncryptAES_DKC_IA32_1h(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   sub   esp,$24
   //pBlock := @Block;
   lea   ecx,[esp+$14]
   mov   esi,$00474bd4
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,[esi]
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,[esi]
   mov   ebx,[edx+$00000101]
   sub   ebx,1
   shr   ebx,1
   //
   mov   esi,[esi]
   //Icardinal := 3;
   mov   edx,12
   //for I := 1 to IMax do
 @LoopStart :
   //Inc(Icardinal);
   add   edx,4
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+5]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+10]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+15]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   edx,4
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+4]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+9]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+14]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+3]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]
   mov   [ecx+4],edi
   //Inc(Icardinal);
   add   edx,4
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+7]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]
   mov   [ecx+8],edi
   //Inc(Icardinal);
   add   edx,4
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+11]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]
   mov   [ecx+12],edi
   //**********************************************
   //Inc(Icardinal);
   add   edx,4
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+5]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+10]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+15]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]
   mov   [eax],edi
   //Inc(Icardinal);
   add   edx,4
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+4]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+9]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+14]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+3]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]
   mov   [eax+4],edi
   //Inc(Icardinal);
   add   edx,4
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+7]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]
   mov   [eax+8],edi
   //Inc(Icardinal);
   add   edx,4
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+11]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]
   mov   [eax+12],edi
   sub   ebx,1
   jnz   @LoopStart
 @LoopEnd :
   //Do last round****************
   //Inc(Icardinal);
   add   edx,4
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   ecx,[ebp*4+AES_T0]
   movzx ebp,[eax+5]
   xor   ecx,[ebp*4+AES_T1]
   movzx ebp,[eax+10]
   xor   ecx,[ebp*4+AES_T2]
   movzx ebp,[eax+15]
   xor   ecx,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   ecx,[esi+edx]                //ecx = C0
   //Inc(Icardinal);
   add   edx,4
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+4]
   mov   ebx,[ebp*4+AES_T0]
   movzx ebp,[eax+9]
   xor   ebx,[ebp*4+AES_T1]
   movzx ebp,[eax+14]
   xor   ebx,[ebp*4+AES_T2]
   movzx ebp,[eax+3]
   xor   ebx,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   ebx,[esi+edx]                //ebx = C1
   //Inc(Icardinal);
   add   edx,4
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+7]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]
   mov   [esp],edi                   //[esp] = C2
   //Inc(Icardinal);
   add   edx,4
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+11]
   xor   edi,[ebp*4+AES_T3]
   //mov   ebp,[esi]
   xor   edi,[esi+edx]                //edi = C3
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+4],ebx
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+8],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+12],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   movzx edx,[eax]
   movzx edx,[edx+AESSBox]
   mov   [esp+4],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   movzx edx,[eax+5]
   movzx edx,[edx+AESSBox]
   mov   [esp+5],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   movzx edx,[eax+10]
   movzx edx,[edx+AESSBox]
   mov   [esp+6],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   movzx edx,[eax+15]
   movzx edx,[edx+AESSBox]
   mov   [esp+7],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   movzx edx,[eax+4]
   movzx edx,[edx+AESSBox]
   mov   [esp+8],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   movzx edx,[eax+9]
   movzx edx,[edx+AESSBox]
   mov   [esp+9],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   movzx edx,[eax+14]
   movzx edx,[edx+AESSBox]
   mov   [esp+10],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   movzx edx,[eax+3]
   movzx edx,[edx+AESSBox]
   mov   [esp+11],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   movzx edx,[eax+8]
   movzx edx,[edx+AESSBox]
   mov   [esp+12],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   movzx edx,[eax+13]
   movzx edx,[edx+AESSBox]
   mov   [esp+13],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   movzx edx,[eax+2]
   movzx edx,[edx+AESSBox]
   mov   [esp+14],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   movzx edx,[eax+7]
   movzx edx,[edx+AESSBox]
   mov   [esp+15],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   movzx edx,[eax+12]
   movzx edx,[edx+AESSBox]
   mov   [esp+16],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   movzx edx,[eax+1]
   movzx edx,[edx+AESSBox]
   mov   [esp+17],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   movzx edx,[eax+6]
   movzx edx,[edx+AESSBox]
   mov   [esp+18],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   movzx edx,[eax+11]
   movzx edx,[edx+AESSBox]
   mov   [esp+19],dl
   //P1 := @E5;
   lea   edx,[esp+4]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   //mov   ecx,[esi]
   mov   ecx,[esi+$101]
   add   ecx,ecx
   //P3 := @pBlock^;
   //mov   ebx,[esi]
   lea   ebx,[esi+ecx*8]
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[ebx]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;

procedure EncryptAES_DKC_IA32_1i(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   sub   esp,$24
   //pBlock := @Block;
   lea   ecx,[esp+$14]
   //mov   esi, pDKCContext//.RoundKeys.Rk[0]//$00474bd4  //!!!!!!!!!!!!!!!!
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,pDKCContext
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,pDKCContext
   mov   ebx,[edx+$00000101]
   sub   ebx,1
   shr   ebx,1
   mov   esi,pDKCContext
   //mov   [esp-4], esi
   //Icardinal := 3;
   //mov   edx,12
   add   esi,12
   //for I := 1 to IMax do
 @LoopStart :
   //Inc(Icardinal);
   //add   edx,4
   add   esi,4
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+5]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+10]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+15]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+4]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+9]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+14]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+3]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+4],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+7]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+8],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+11]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+12],edi
   //**********************************************
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+5]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+10]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+15]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+4]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+9]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+14]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+3]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+4],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+7]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+8],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+11]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+12],edi
   sub   ebx,1
   jnz   @LoopStart
 @LoopEnd :
   //Do last round****************
   //Inc(Icardinal);
   add   esi,4
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   ecx,[ebp*4+AES_T0]
   movzx ebp,[eax+5]
   xor   ecx,[ebp*4+AES_T1]
   movzx ebp,[eax+10]
   xor   ecx,[ebp*4+AES_T2]
   movzx ebp,[eax+15]
   xor   ecx,[ebp*4+AES_T3]
   xor   ecx,[esi]                //ecx = C0
   //Inc(Icardinal);
   add   esi,4
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+4]
   mov   ebx,[ebp*4+AES_T0]
   movzx ebp,[eax+9]
   xor   ebx,[ebp*4+AES_T1]
   movzx ebp,[eax+14]
   xor   ebx,[ebp*4+AES_T2]
   movzx ebp,[eax+3]
   xor   ebx,[ebp*4+AES_T3]
   xor   ebx,[esi]                //ebx = C1
   //Inc(Icardinal);
   add   esi,4
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+7]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [esp],edi                   //[esp] = C2
   //Inc(Icardinal);
   add   esi,4
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+11]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]                //edi = C3
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+4],ebx
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+8],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+12],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   movzx edx,[eax]
   movzx edx,[edx+AESSBox]
   mov   [esp+4],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   movzx edx,[eax+5]
   movzx edx,[edx+AESSBox]
   mov   [esp+5],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   movzx edx,[eax+10]
   movzx edx,[edx+AESSBox]
   mov   [esp+6],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   movzx edx,[eax+15]
   movzx edx,[edx+AESSBox]
   mov   [esp+7],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   movzx edx,[eax+4]
   movzx edx,[edx+AESSBox]
   mov   [esp+8],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   movzx edx,[eax+9]
   movzx edx,[edx+AESSBox]
   mov   [esp+9],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   movzx edx,[eax+14]
   movzx edx,[edx+AESSBox]
   mov   [esp+10],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   movzx edx,[eax+3]
   movzx edx,[edx+AESSBox]
   mov   [esp+11],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   movzx edx,[eax+8]
   movzx edx,[edx+AESSBox]
   mov   [esp+12],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   movzx edx,[eax+13]
   movzx edx,[edx+AESSBox]
   mov   [esp+13],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   movzx edx,[eax+2]
   movzx edx,[edx+AESSBox]
   mov   [esp+14],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   movzx edx,[eax+7]
   movzx edx,[edx+AESSBox]
   mov   [esp+15],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   movzx edx,[eax+12]
   movzx edx,[edx+AESSBox]
   mov   [esp+16],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   movzx edx,[eax+1]
   movzx edx,[edx+AESSBox]
   mov   [esp+17],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   movzx edx,[eax+6]
   movzx edx,[edx+AESSBox]
   mov   [esp+18],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   movzx edx,[eax+11]
   movzx edx,[edx+AESSBox]
   mov   [esp+19],dl
   //P1 := @E5;
   lea   edx,[esp+4]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   //mov   esi,[esp-4]
   //mov   ecx,[esi+$101]
   //add   ecx,ecx
   //P3 := @pBlock^;
   //lea   ebx,[esi+ecx*8]
   add   esi,4
   mov   ebx,esi
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[ebx]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;

{

procedure EncryptAES_DKC_IA32_1(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   sub   esp,$24
   //pBlock := @Block;
   lea   ecx,[esp+$14]
   mov   esi,$00474bd4
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,[esi]
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,[esi]
   mov   ebx,[edx+$00000101]
   sub   ebx,1
   shr   ebx,1
   mov   esi,[esi]
   //Icardinal := 3;
   add   esi,12
   mov   ebp,eax
   //for I := 1 to IMax do
 @LoopStart :
   //Inc(Icardinal);
   add   esi,4
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   xor   edx,edx
   mov   eax,[ebp]
   movzx edx,al
   mov   edi,[edx*4+AES_T0]
   movzx edx,[ebp+5]
   xor   edi,[edx*4+AES_T1]
   movzx edx,[ebp+10]
   xor   edi,[edx*4+AES_T2]
   movzx edx,[ebp+15]
   xor   edi,[edx*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edx,[ebp+4]
   mov   edi,[edx*4+AES_T0]
   movzx edx,[ebp+9]
   xor   edi,[edx*4+AES_T1]
   movzx edx,[ebp+14]
   xor   edi,[edx*4+AES_T2]
   rol   eax,8
   movzx edx,al
   //movzx edx,[ebp+3]
   xor   edi,[edx*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+4],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edx,[ebp+8]
   mov   edi,[edx*4+AES_T0]
   movzx edx,[ebp+13]
   xor   edi,[edx*4+AES_T1]
   rol   eax,8
   movzx edx,al
   //movzx edx,[ebp+2]
   xor   edi,[edx*4+AES_T2]
   movzx edx,[ebp+7]
   xor   edi,[edx*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+8],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edx,[ebp+12]
   mov   edi,[edx*4+AES_T0]
   rol   eax,8
   movzx edx,al
   //movzx edx,[ebp+1]
   xor   edi,[edx*4+AES_T1]
   movzx edx,[ebp+6]
   xor   edi,[edx*4+AES_T2]
   movzx edx,[ebp+11]
   xor   edi,[edx*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+12],edi
   //**********************************************
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   eax,[ecx]
   movzx edx,al
   //movzx edx,[ecx]
   mov   edi,[edx*4+AES_T0]
   movzx edx,[ecx+5]
   xor   edi,[edx*4+AES_T1]
   movzx edx,[ecx+10]
   xor   edi,[edx*4+AES_T2]
   movzx edx,[ecx+15]
   xor   edi,[edx*4+AES_T3]
   xor   edi,[esi]
   mov   [ebp],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edx,[ecx+4]
   mov   edi,[edx*4+AES_T0]
   movzx edx,[ecx+9]
   xor   edi,[edx*4+AES_T1]
   movzx edx,[ecx+14]
   xor   edi,[edx*4+AES_T2]
   rol   eax,8
   movzx edx,al
   //movzx edx,[ecx+3]
   xor   edi,[edx*4+AES_T3]
   xor   edi,[esi]
   mov   [ebp+4],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edx,[ecx+8]
   mov   edi,[edx*4+AES_T0]
   movzx edx,[ecx+13]
   xor   edi,[edx*4+AES_T1]
   rol   eax,8
   movzx edx,al
   //movzx edx,[ecx+2]
   xor   edi,[edx*4+AES_T2]
   movzx edx,[ecx+7]
   xor   edi,[edx*4+AES_T3]
   xor   edi,[esi]
   mov   [ebp+8],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx edx,[ecx+12]
   mov   edi,[edx*4+AES_T0]
   rol   eax,8
   movzx edx,al
   //movzx edx,[ecx+1]
   xor   edi,[edx*4+AES_T1]
   movzx edx,[ecx+6]
   xor   edi,[edx*4+AES_T2]
   movzx edx,[ecx+11]
   xor   edi,[edx*4+AES_T3]
   xor   edi,[esi]
   mov   [ebp+12],edi
   sub   ebx,1
   jnz   @LoopStart
 @LoopEnd :
   //Do last round****************
   //Inc(Icardinal);
   add   esi,4
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   eax,ebp
   movzx ebp,[eax]
   mov   ecx,[ebp*4+AES_T0]
   movzx ebp,[eax+5]
   xor   ecx,[ebp*4+AES_T1]
   movzx ebp,[eax+10]
   xor   ecx,[ebp*4+AES_T2]
   movzx ebp,[eax+15]
   xor   ecx,[ebp*4+AES_T3]
   xor   ecx,[esi]                //ecx = C0
   //Inc(Icardinal);
   add   esi,4
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+4]
   mov   ebx,[ebp*4+AES_T0]
   movzx ebp,[eax+9]
   xor   ebx,[ebp*4+AES_T1]
   movzx ebp,[eax+14]
   xor   ebx,[ebp*4+AES_T2]
   movzx ebp,[eax+3]
   xor   ebx,[ebp*4+AES_T3]
   xor   ebx,[esi]                //ebx = C1
   //Inc(Icardinal);
   add   esi,4
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+7]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [esp],edi                   //[esp] = C2
   //Inc(Icardinal);
   add   esi,4
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+11]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]                //edi = C3
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+4],ebx
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+8],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+12],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   movzx edx,[eax]
   movzx edx,[edx+AESSBox]
   mov   [esp+4],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   movzx edx,[eax+5]
   movzx edx,[edx+AESSBox]
   mov   [esp+5],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   movzx edx,[eax+10]
   movzx edx,[edx+AESSBox]
   mov   [esp+6],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   movzx edx,[eax+15]
   movzx edx,[edx+AESSBox]
   mov   [esp+7],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   movzx edx,[eax+4]
   movzx edx,[edx+AESSBox]
   mov   [esp+8],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   movzx edx,[eax+9]
   movzx edx,[edx+AESSBox]
   mov   [esp+9],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   movzx edx,[eax+14]
   movzx edx,[edx+AESSBox]
   mov   [esp+10],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   movzx edx,[eax+3]
   movzx edx,[edx+AESSBox]
   mov   [esp+11],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   movzx edx,[eax+8]
   movzx edx,[edx+AESSBox]
   mov   [esp+12],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   movzx edx,[eax+13]
   movzx edx,[edx+AESSBox]
   mov   [esp+13],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   movzx edx,[eax+2]
   movzx edx,[edx+AESSBox]
   mov   [esp+14],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   movzx edx,[eax+7]
   movzx edx,[edx+AESSBox]
   mov   [esp+15],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   movzx edx,[eax+12]
   movzx edx,[edx+AESSBox]
   mov   [esp+16],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   movzx edx,[eax+1]
   movzx edx,[edx+AESSBox]
   mov   [esp+17],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   movzx edx,[eax+6]
   movzx edx,[edx+AESSBox]
   mov   [esp+18],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   movzx edx,[eax+11]
   movzx edx,[edx+AESSBox]
   mov   [esp+19],dl
   //P1 := @E5;
   lea   edx,[esp+4]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   //P3 := @pBlock^;
   add   esi,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[esi]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   esi,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[esi]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   esi,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[esi]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   esi,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[esi]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;
}

procedure EncryptAES_DKC_IA32_1(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   sub   esp,$24
   //pBlock := @Block;
   lea   ecx,[esp+$14]
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,pDKCContext
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,pDKCContext
   mov   ebx,[edx+$00000101]
   sub   ebx,1
   shr   ebx,1
   mov   esi,pDKCContext
   //Icardinal := 3;
   add   esi,12
   //for I := 1 to IMax do
 @LoopStart :
   //Inc(Icardinal);
   add   esi,4
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[eax]
   mov   ebp,[eax]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[eax+5]
   mov   ebp,[eax+5]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[eax+10]
   mov   ebp,[eax+10]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[eax+15]
   mov   ebp,[eax+15]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[eax+4]
   mov   ebp,[eax+4]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[eax+9]
   mov   ebp,[eax+9]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[eax+14]
   mov   ebp,[eax+14]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[eax+3]
   mov   ebp,[eax+3]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+4],edi
   add   esi,4
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[eax+8]
   mov   ebp,[eax+8]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[eax+13]
   mov   ebp,[eax+13]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[eax+2]
   mov   ebp,[eax+2]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[eax+7]
   mov   ebp,[eax+7]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+8],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[eax+12]
   mov   ebp,[eax+12]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[eax+1]
   mov   ebp,[eax+1]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[eax+6]
   mov   ebp,[eax+6]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[eax+11]
   mov   ebp,[eax+11]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+12],edi
   //**********************************************
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[ecx]
   mov   ebp,[ecx]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[ecx+5]
   mov   ebp,[ecx+5]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[ecx+10]
   mov   ebp,[ecx+10]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[ecx+15]
   mov   ebp,[ecx+15]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[ecx+4]
   mov   ebp,[ecx+4]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[ecx+9]
   mov   ebp,[ecx+9]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[ecx+14]
   mov   ebp,[ecx+14]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[ecx+3]
   mov   ebp,[ecx+3]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+4],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[ecx+8]
   mov   ebp,[ecx+8]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[ecx+13]
   mov   ebp,[ecx+13]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[ecx+2]
   mov   ebp,[ecx+2]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[ecx+7]
   mov   ebp,[ecx+7]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+8],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[ecx+12]
   mov   ebp,[ecx+12]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[ecx+1]
   mov   ebp,[ecx+1]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[ecx+6]
   mov   ebp,[ecx+6]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[ecx+11]
   mov   ebp,[ecx+11]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+12],edi
   sub   ebx,1
   jnz   @LoopStart
 @LoopEnd :
   //Do last round****************
   //Inc(Icardinal);
   add   esi,4
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[eax]
   mov   ebp,[eax]
   and   ebp,$ff
   mov   ecx,[ebp*4+AES_T0]
   //movzx ebp,[eax+5]
   mov   ebp,[eax+5]
   and   ebp,$ff
   xor   ecx,[ebp*4+AES_T1]
   //movzx ebp,[eax+10]
   mov   ebp,[eax+10]
   and   ebp,$ff
   xor   ecx,[ebp*4+AES_T2]
   //movzx ebp,[eax+15]
   mov   ebp,[eax+15]
   and   ebp,$ff
   xor   ecx,[ebp*4+AES_T3]
   xor   ecx,[esi]                //ecx = C0
   //Inc(Icardinal);
   add   esi,4
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[eax+4]
   mov   ebp,[eax+4]
   and   ebp,$ff
   mov   ebx,[ebp*4+AES_T0]
   //movzx ebp,[eax+9]
   mov   ebp,[eax+9]
   and   ebp,$ff
   xor   ebx,[ebp*4+AES_T1]
   //movzx ebp,[eax+14]
   mov   ebp,[eax+14]
   and   ebp,$ff
   xor   ebx,[ebp*4+AES_T2]
   //movzx ebp,[eax+3]
   mov   ebp,[eax+3]
   and   ebp,$ff
   xor   ebx,[ebp*4+AES_T3]
   xor   ebx,[esi]                //ebx = C1
   //Inc(Icardinal);
   add   esi,4
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[eax+8]
   mov   ebp,[eax+8]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[eax+13]
   mov   ebp,[eax+13]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[eax+2]
   mov   ebp,[eax+2]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[eax+7]
   mov   ebp,[eax+7]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [esp],edi                   //[esp] = C2
   //Inc(Icardinal);
   add   esi,4
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   //movzx ebp,[eax+12]
   mov   ebp,[eax+12]
   and   ebp,$ff
   mov   edi,[ebp*4+AES_T0]
   //movzx ebp,[eax+1]
   mov   ebp,[eax+1]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T1]
   //movzx ebp,[eax+6]
   mov   ebp,[eax+6]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T2]
   //movzx ebp,[eax+11]
   mov   ebp,[eax+11]
   and   ebp,$ff
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]                //edi = C3
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+4],ebx
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+8],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+12],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   //movzx edx,[eax]
   mov   edx,[eax]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+4],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   //movzx edx,[eax+5]
   mov   edx,[eax+5]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+5],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   //movzx edx,[eax+10]
   mov   edx,[eax+10]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+6],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   //movzx edx,[eax+15]
   mov edx,[eax+15]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+7],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   //movzx edx,[eax+4]
   mov edx,[eax+4]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+8],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   //movzx edx,[eax+9]
   mov   edx,[eax+9]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+9],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   //movzx edx,[eax+14]
   mov edx,[eax+14]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+10],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   //movzx edx,[eax+3]
   mov   edx,[eax+3]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+11],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   //movzx edx,[eax+8]
   mov   edx,[eax+8]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+12],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   //movzx edx,[eax+13]
   mov   edx,[eax+13]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+13],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   //movzx edx,[eax+2]
   mov   edx,[eax+2]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+14],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   //movzx edx,[eax+7]
   mov   edx,[eax+7]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+15],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   //movzx edx,[eax+12]
   mov   edx,[eax+12]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+16],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   //movzx edx,[eax+1]
   mov   edx,[eax+1]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+17],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   //movzx edx,[eax+6]
   mov   edx,[eax+6]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+18],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   //movzx edx,[eax+11]
   mov   edx,[eax+11]
   and   edx,$ff
   movzx edx,[edx+AESSBox]
   mov   [esp+19],dl
   //P1 := @E5;
   lea   edx,[esp+4]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   //P3 := @pBlock^;
   add   esi,4
   mov   ebx,esi
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[ebx]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;

//Author:            Dennis Kjaer Christensen
//Date:              30/9 2004
//Instructionset(s): IA32

procedure DecryptAES_DKC_IA32_1(var Block);
var
 I : Integer;
 pBlock : ^TAESBlock;
 P1, P2 : ^Cardinal;
 C1 : Cardinal;
 E1, E2, E3, E4 : TAESVector;
 Temp1, Temp2, Temp3  : Byte;

begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[TAESVectors(pBlock^)[0].bt[0]];
 TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[TAESVectors(pBlock^)[1].bt[0]];
 TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[TAESVectors(pBlock^)[2].bt[0]];
 TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[TAESVectors(pBlock^)[3].bt[0]];
 Temp1 := AESInvSBox[TAESVectors(pBlock^)[3].bt[1]];
 TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[TAESVectors(pBlock^)[2].bt[1]];
 TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[TAESVectors(pBlock^)[1].bt[1]];
 TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[TAESVectors(pBlock^)[0].bt[1]];
 TAESVectors(pBlock^)[0].bt[1] := Temp1;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[2].bt[2]];
 TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[TAESVectors(pBlock^)[0].bt[2]];
 TAESVectors(pBlock^)[0].bt[2] := Temp2;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[3].bt[2]];
 TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[TAESVectors(pBlock^)[1].bt[2]];
 TAESVectors(pBlock^)[1].bt[2] := Temp2;
 Temp3 := AESInvSBox[TAESVectors(pBlock^)[0].bt[3]];
 TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[TAESVectors(pBlock^)[1].bt[3]];
 TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[TAESVectors(pBlock^)[2].bt[3]];
 TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[TAESVectors(pBlock^)[3].bt[3]];
 TAESVectors(pBlock^)[3].bt[3] := Temp3;
 for i := (pDKCContext.Rounds - 1) downto 1 do
  begin
   P1 := @pBlock^;
   P2 := @pDKCContext.RoundKeys.Rk[i];
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   E1.dw := AES_InvT0[TAESVectors(pBlock^)[0].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[0].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[0].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[0].bt[3]];
   E2.dw := AES_InvT0[TAESVectors(pBlock^)[1].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[1].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[1].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[1].bt[3]];
   E3.dw := AES_InvT0[TAESVectors(pBlock^)[2].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[2].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[2].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[2].bt[3]];
   E4.dw := AES_InvT0[TAESVectors(pBlock^)[3].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[3].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[3].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[3].bt[3]];
   TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[E1.bt[0]];
   TAESVectors(pBlock^)[0].bt[1] := AESInvSBox[E4.bt[1]];
   TAESVectors(pBlock^)[0].bt[2] := AESInvSBox[E3.bt[2]];
   TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[E2.bt[3]];
   TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[E2.bt[0]];
   TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[E1.bt[1]];
   TAESVectors(pBlock^)[1].bt[2] := AESInvSBox[E4.bt[2]];
   TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[E3.bt[3]];
   TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[E3.bt[0]];
   TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[E2.bt[1]];
   TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[E1.bt[2]];
   TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[E4.bt[3]];
   TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[E4.bt[0]];
   TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[E3.bt[1]];
   TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[E2.bt[2]];
   TAESVectors(pBlock^)[3].bt[3] := AESInvSBox[E1.bt[3]];
  end;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[0];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
end;

//Author:            Dennis Kjaer Christensen
//Date:              5/10 2004
//Instructionset(s): IA32

procedure EncryptAES_DKC_IA32_2(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   sub   esp,$24
   //pBlock := @Block;
   lea   ecx,[esp+$14]
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,pDKCContext
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,pDKCContext
   mov   ebx,[edx+$00000101]
   sub   ebx,1
   shr   ebx,1
   mov   esi,pDKCContext
   //Icardinal := 3;
   add   esi,12
   //for I := 1 to IMax do
 @LoopStart :
   //Inc(Icardinal);
   add   esi,4
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+5]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+10]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+15]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+4]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+9]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+14]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+3]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+4],edi
   add   esi,4
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+7]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+8],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+11]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+12],edi
   //**********************************************
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+5]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+10]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+15]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+4]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+9]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+14]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+3]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+4],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+7]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+8],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[ecx+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[ecx+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[ecx+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[ecx+11]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+12],edi
   sub   ebx,1
   jnz   @LoopStart
 @LoopEnd :
   //Do last round****************
   //Inc(Icardinal);
   add   esi,4
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax]
   mov   ecx,[ebp*4+AES_T0]
   movzx ebp,[eax+5]
   xor   ecx,[ebp*4+AES_T1]
   movzx ebp,[eax+10]
   xor   ecx,[ebp*4+AES_T2]
   movzx ebp,[eax+15]
   xor   ecx,[ebp*4+AES_T3]
   xor   ecx,[esi]                //ecx = C0
   //Inc(Icardinal);
   add   esi,4
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+4]
   mov   ebx,[ebp*4+AES_T0]
   movzx ebp,[eax+9]
   xor   ebx,[ebp*4+AES_T1]
   movzx ebp,[eax+14]
   xor   ebx,[ebp*4+AES_T2]
   movzx ebp,[eax+3]
   xor   ebx,[ebp*4+AES_T3]
   xor   ebx,[esi]                //ebx = C1
   //Inc(Icardinal);
   add   esi,4
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+8]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+13]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+2]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+7]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]
   mov   [esp],edi                   //[esp] = C2
   //Inc(Icardinal);
   add   esi,4
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   movzx ebp,[eax+12]
   mov   edi,[ebp*4+AES_T0]
   movzx ebp,[eax+1]
   xor   edi,[ebp*4+AES_T1]
   movzx ebp,[eax+6]
   xor   edi,[ebp*4+AES_T2]
   movzx ebp,[eax+11]
   xor   edi,[ebp*4+AES_T3]
   xor   edi,[esi]                //edi = C3
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+4],ebx
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+8],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+12],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   movzx edx,[eax]
   movzx edx,[edx+AESSBox]
   mov   [esp+4],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   movzx edx,[eax+5]
   movzx edx,[edx+AESSBox]
   mov   [esp+5],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   movzx edx,[eax+10]
   movzx edx,[edx+AESSBox]
   mov   [esp+6],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   movzx edx,[eax+15]
   movzx edx,[edx+AESSBox]
   mov   [esp+7],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   movzx edx,[eax+4]
   movzx edx,[edx+AESSBox]
   mov   [esp+8],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   movzx edx,[eax+9]
   movzx edx,[edx+AESSBox]
   mov   [esp+9],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   movzx edx,[eax+14]
   movzx edx,[edx+AESSBox]
   mov   [esp+10],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   movzx edx,[eax+3]
   movzx edx,[edx+AESSBox]
   mov   [esp+11],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   movzx edx,[eax+8]
   movzx edx,[edx+AESSBox]
   mov   [esp+12],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   movzx edx,[eax+13]
   movzx edx,[edx+AESSBox]
   mov   [esp+13],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   movzx edx,[eax+2]
   movzx edx,[edx+AESSBox]
   mov   [esp+14],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   movzx edx,[eax+7]
   movzx edx,[edx+AESSBox]
   mov   [esp+15],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   movzx edx,[eax+12]
   movzx edx,[edx+AESSBox]
   mov   [esp+16],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   movzx edx,[eax+1]
   movzx edx,[edx+AESSBox]
   mov   [esp+17],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   movzx edx,[eax+6]
   movzx edx,[edx+AESSBox]
   mov   [esp+18],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   movzx edx,[eax+11]
   movzx edx,[edx+AESSBox]
   mov   [esp+19],dl
   //P1 := @E5;
   lea   edx,[esp+4]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   //P3 := @pBlock^;
   add   esi,4
   mov   ebx,esi
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[ebx]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;

//Author:            Dennis Kjaer Christensen
//Date:              5/10 2004
//Instructionset(s): IA32

procedure DecryptAES_DKC_IA32_2(var Block);
var
 I : Integer;
 pBlock : ^TAESBlock;
 P1, P2 : ^Cardinal;
 C1 : Cardinal;
 E1, E2, E3, E4 : TAESVector;
 Temp1, Temp2, Temp3  : Byte;

begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[TAESVectors(pBlock^)[0].bt[0]];
 TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[TAESVectors(pBlock^)[1].bt[0]];
 TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[TAESVectors(pBlock^)[2].bt[0]];
 TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[TAESVectors(pBlock^)[3].bt[0]];
 Temp1 := AESInvSBox[TAESVectors(pBlock^)[3].bt[1]];
 TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[TAESVectors(pBlock^)[2].bt[1]];
 TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[TAESVectors(pBlock^)[1].bt[1]];
 TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[TAESVectors(pBlock^)[0].bt[1]];
 TAESVectors(pBlock^)[0].bt[1] := Temp1;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[2].bt[2]];
 TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[TAESVectors(pBlock^)[0].bt[2]];
 TAESVectors(pBlock^)[0].bt[2] := Temp2;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[3].bt[2]];
 TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[TAESVectors(pBlock^)[1].bt[2]];
 TAESVectors(pBlock^)[1].bt[2] := Temp2;
 Temp3 := AESInvSBox[TAESVectors(pBlock^)[0].bt[3]];
 TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[TAESVectors(pBlock^)[1].bt[3]];
 TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[TAESVectors(pBlock^)[2].bt[3]];
 TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[TAESVectors(pBlock^)[3].bt[3]];
 TAESVectors(pBlock^)[3].bt[3] := Temp3;
 for i := (pDKCContext.Rounds - 1) downto 1 do
  begin
   P1 := @pBlock^;
   P2 := @pDKCContext.RoundKeys.Rk[i];
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   E1.dw := AES_InvT0[TAESVectors(pBlock^)[0].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[0].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[0].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[0].bt[3]];
   E2.dw := AES_InvT0[TAESVectors(pBlock^)[1].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[1].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[1].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[1].bt[3]];
   E3.dw := AES_InvT0[TAESVectors(pBlock^)[2].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[2].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[2].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[2].bt[3]];
   E4.dw := AES_InvT0[TAESVectors(pBlock^)[3].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[3].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[3].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[3].bt[3]];
   TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[E1.bt[0]];
   TAESVectors(pBlock^)[0].bt[1] := AESInvSBox[E4.bt[1]];
   TAESVectors(pBlock^)[0].bt[2] := AESInvSBox[E3.bt[2]];
   TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[E2.bt[3]];
   TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[E2.bt[0]];
   TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[E1.bt[1]];
   TAESVectors(pBlock^)[1].bt[2] := AESInvSBox[E4.bt[2]];
   TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[E3.bt[3]];
   TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[E3.bt[0]];
   TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[E2.bt[1]];
   TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[E1.bt[2]];
   TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[E4.bt[3]];
   TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[E4.bt[0]];
   TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[E3.bt[1]];
   TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[E2.bt[2]];
   TAESVectors(pBlock^)[3].bt[3] := AESInvSBox[E1.bt[3]];
  end;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[0];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
end;

//Author:            Dennis Kjaer Christensen
//Date:              5/10 2004
//Instructionset(s): IA32

procedure EncryptAES_DKC_IA32_3(var Block);
asm
   push  ebx
   push  esi
   push  edi
   push  ebp
   sub   esp,$24
   //pBlock := @Block;
   lea   ecx,[esp+$14]
   //P1 := @pBlock^;
   mov   edx,eax
   //P2 := @pDKCContext.RoundKeys.Rk[0];
   mov   ebx,pDKCContext
   //Add round key
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   edi,[ebx]
   xor   [edx],edi
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //P1^ := P1^ xor P2^;
   mov   ebx,[ebx]
   xor   [edx],ebx
   //IMax := (pDKCContext.Rounds - 1) div 2; //Always even number of rounds and odd number of iterations
   mov   edx,pDKCContext
   mov   ebx,[edx+$00000101]
   sub   ebx,1
   shr   ebx,1
   mov   esi,pDKCContext
   //Icardinal := 3;
   add   esi,12
   //for I := 1 to IMax do
   mov   ebp,ebx
   xor   ebx,ebx
 @LoopStart :
   //Inc(Icardinal);
   add   esi,4
   //E6[0].DW := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[eax]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[eax+5]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[eax+10]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[eax+15]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[1].dw := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[eax+4]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[eax+9]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[eax+14]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[eax+3]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+4],edi
   add   esi,4
   //E6[2].dw := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[eax+8]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[eax+13]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[eax+2]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[eax+7]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+8],edi
   //Inc(Icardinal);
   add   esi,4
   //E6[3].dw := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //             AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //             AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //             AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //             pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[eax+12]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[eax+1]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[eax+6]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[eax+11]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]
   mov   [ecx+12],edi
   //**********************************************
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[0].dw := (AES_T0[E6[0].bt[0]] xor
   //                               AES_T1[E6[1].bt[1]] xor
   //                               AES_T2[E6[2].bt[2]] xor
   //                               AES_T3[E6[3].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[ecx]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[ecx+5]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[ecx+10]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[ecx+15]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]
   mov   [eax],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[1].dw := (AES_T0[E6[1].bt[0]] xor
   //                               AES_T1[E6[2].bt[1]] xor
   //                               AES_T2[E6[3].bt[2]] xor
   //                               AES_T3[E6[0].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[ecx+4]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[ecx+9]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[ecx+14]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[ecx+3]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+4],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[2].dw := (AES_T0[E6[2].bt[0]] xor
   //                               AES_T1[E6[3].bt[1]] xor
   //                               AES_T2[E6[0].bt[2]] xor
   //                               AES_T3[E6[1].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[ecx+8]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[ecx+13]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[ecx+2]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[ecx+7]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+8],edi
   //Inc(Icardinal);
   add   esi,4
   //TAESVectors(pBlock^)[3].dw := (AES_T0[E6[3].bt[0]] xor
   //                               AES_T1[E6[0].bt[1]] xor
   //                               AES_T2[E6[1].bt[2]] xor
   //                               AES_T3[E6[2].bt[3]]) xor
   //                               pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[ecx+12]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[ecx+1]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[ecx+6]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[ecx+11]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]
   mov   [eax+12],edi
   sub   ebp,1
   jnz   @LoopStart
 @LoopEnd :
   xor   ebx,ebx
   //Do last round****************
   //Inc(Icardinal);
   add   esi,4
   //C0 := (AES_T0[TAESVectors(pBlock^)[0].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[1].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[2].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[3].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[eax]
   mov   ecx,[ebx*4+AES_T0]
   mov   bl,[eax+5]
   xor   ecx,[ebx*4+AES_T1]
   mov   bl,[eax+10]
   xor   ecx,[ebx*4+AES_T2]
   mov   bl,[eax+15]
   xor   ecx,[ebx*4+AES_T3]
   xor   ecx,[esi]                //ecx = C0
   //Inc(Icardinal);
   add   esi,4
   //C1 := (AES_T0[TAESVectors(pBlock^)[1].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[2].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[3].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[0].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[eax+4]
   mov   ebp,[ebx*4+AES_T0]
   mov   bl,[eax+9]
   xor   ebp,[ebx*4+AES_T1]
   mov   bl,[eax+14]
   xor   ebp,[ebx*4+AES_T2]
   mov   bl,[eax+3]
   xor   ebp,[ebx*4+AES_T3]
   xor   ebp,[esi]                //ebx = C1
   //Inc(Icardinal);
   add   esi,4
   //C2 := (AES_T0[TAESVectors(pBlock^)[2].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[3].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[0].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[1].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[eax+8]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[eax+13]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[eax+2]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[eax+7]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]
   mov   [esp],edi                   //[esp] = C2
   //Inc(Icardinal);
   add   esi,4
   //C3 := (AES_T0[TAESVectors(pBlock^)[3].bt[0]] xor
   //       AES_T1[TAESVectors(pBlock^)[0].bt[1]] xor
   //       AES_T2[TAESVectors(pBlock^)[1].bt[2]] xor
   //       AES_T3[TAESVectors(pBlock^)[2].bt[3]]) xor
   //       pDKCContext.RoundKeys.W[Icardinal].dw;
   mov   bl,[eax+12]
   mov   edi,[ebx*4+AES_T0]
   mov   bl,[eax+1]
   xor   edi,[ebx*4+AES_T1]
   mov   bl,[eax+6]
   xor   edi,[ebx*4+AES_T2]
   mov   bl,[eax+11]
   xor   edi,[ebx*4+AES_T3]
   xor   edi,[esi]                //edi = C3
   mov   edx,edi
   //TAESVectors(pBlock^)[0].dw := C0;
   mov   [eax],ecx
   //TAESVectors(pBlock^)[1].dw := C1;
   mov   [eax+4],ebp
   //TAESVectors(pBlock^)[2].dw := C2;
   mov   ecx,[esp]
   mov   [eax+8],ecx
   //TAESVectors(pBlock^)[3].dw := C3;
   mov   [eax+12],edx
   //****************************
   //E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].bt[0]];
   movzx edx,[eax]
   movzx edx,[edx+AESSBox]
   mov   [esp+4],dl
   //E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].bt[1]];
   movzx edx,[eax+5]
   movzx edx,[edx+AESSBox]
   mov   [esp+5],dl
   //E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].bt[2]];
   movzx edx,[eax+10]
   movzx edx,[edx+AESSBox]
   mov   [esp+6],dl
   //E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].bt[3]];
   movzx edx,[eax+15]
   movzx edx,[edx+AESSBox]
   mov   [esp+7],dl
   //E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].bt[0]];
   movzx edx,[eax+4]
   movzx edx,[edx+AESSBox]
   mov   [esp+8],dl
   //E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].bt[1]];
   movzx edx,[eax+9]
   movzx edx,[edx+AESSBox]
   mov   [esp+9],dl
   //E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].bt[2]];
   movzx edx,[eax+14]
   movzx edx,[edx+AESSBox]
   mov   [esp+10],dl
   //E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].bt[3]];
   movzx edx,[eax+3]
   movzx edx,[edx+AESSBox]
   mov   [esp+11],dl
   //E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].bt[0]];
   movzx edx,[eax+8]
   movzx edx,[edx+AESSBox]
   mov   [esp+12],dl
   //E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].bt[1]];
   movzx edx,[eax+13]
   movzx edx,[edx+AESSBox]
   mov   [esp+13],dl
   //E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].bt[2]];
   movzx edx,[eax+2]
   movzx edx,[edx+AESSBox]
   mov   [esp+14],dl
   //E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].bt[3]];
   movzx edx,[eax+7]
   movzx edx,[edx+AESSBox]
   mov   [esp+15],dl
   //E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].bt[0]];
   movzx edx,[eax+12]
   movzx edx,[edx+AESSBox]
   mov   [esp+16],dl
   //E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].bt[1]];
   movzx edx,[eax+1]
   movzx edx,[edx+AESSBox]
   mov   [esp+17],dl
   //E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].bt[2]];
   movzx edx,[eax+6]
   movzx edx,[edx+AESSBox]
   mov   [esp+18],dl
   //E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].bt[3]];
   movzx edx,[eax+11]
   movzx edx,[edx+AESSBox]
   mov   [esp+19],dl
   //P1 := @E5;
   lea   edx,[esp+4]
   //P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
   //P3 := @pBlock^;
   add   esi,4
   mov   ebx,esi
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   ecx,[edx]
   xor   ecx,[ebx]
   mov   [eax],ecx
   //Inc(P1);
   add   edx,4
   //Inc(P2);
   add   ebx,4
   //Inc(P3);
   add   eax,4
   //P3^ := P1^ xor P2^;
   mov   edx,[edx]
   xor   edx,[ebx]
   mov   [eax],edx
   add   esp,$24
   pop   ebp
   pop   edi
   pop   esi
   pop   ebx
end;

//Author:            Dennis Kjaer Christensen
//Date:              5/10 2004
//Instructionset(s): IA32

procedure DecryptAES_DKC_IA32_3(var Block);
var
 I : Integer;
 pBlock : ^TAESBlock;
 P1, P2 : ^Cardinal;
 C1 : Cardinal;
 E1, E2, E3, E4 : TAESVector;
 Temp1, Temp2, Temp3  : Byte;

begin
 pBlock := @Block;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[pDKCContext.Rounds];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[TAESVectors(pBlock^)[0].bt[0]];
 TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[TAESVectors(pBlock^)[1].bt[0]];
 TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[TAESVectors(pBlock^)[2].bt[0]];
 TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[TAESVectors(pBlock^)[3].bt[0]];
 Temp1 := AESInvSBox[TAESVectors(pBlock^)[3].bt[1]];
 TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[TAESVectors(pBlock^)[2].bt[1]];
 TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[TAESVectors(pBlock^)[1].bt[1]];
 TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[TAESVectors(pBlock^)[0].bt[1]];
 TAESVectors(pBlock^)[0].bt[1] := Temp1;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[2].bt[2]];
 TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[TAESVectors(pBlock^)[0].bt[2]];
 TAESVectors(pBlock^)[0].bt[2] := Temp2;
 Temp2 := AESInvSBox[TAESVectors(pBlock^)[3].bt[2]];
 TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[TAESVectors(pBlock^)[1].bt[2]];
 TAESVectors(pBlock^)[1].bt[2] := Temp2;
 Temp3 := AESInvSBox[TAESVectors(pBlock^)[0].bt[3]];
 TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[TAESVectors(pBlock^)[1].bt[3]];
 TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[TAESVectors(pBlock^)[2].bt[3]];
 TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[TAESVectors(pBlock^)[3].bt[3]];
 TAESVectors(pBlock^)[3].bt[3] := Temp3;
 for i := (pDKCContext.Rounds - 1) downto 1 do
  begin
   P1 := @pBlock^;
   P2 := @pDKCContext.RoundKeys.Rk[i];
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   Inc(P1);
   Inc(P2);
   C1 := P1^ xor P2^;
   P1^ := C1;
   E1.dw := AES_InvT0[TAESVectors(pBlock^)[0].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[0].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[0].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[0].bt[3]];
   E2.dw := AES_InvT0[TAESVectors(pBlock^)[1].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[1].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[1].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[1].bt[3]];
   E3.dw := AES_InvT0[TAESVectors(pBlock^)[2].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[2].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[2].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[2].bt[3]];
   E4.dw := AES_InvT0[TAESVectors(pBlock^)[3].bt[0]] xor
            AES_InvT1[TAESVectors(pBlock^)[3].bt[1]] xor
            AES_InvT2[TAESVectors(pBlock^)[3].bt[2]] xor
            AES_InvT3[TAESVectors(pBlock^)[3].bt[3]];
   TAESVectors(pBlock^)[0].bt[0] := AESInvSBox[E1.bt[0]];
   TAESVectors(pBlock^)[0].bt[1] := AESInvSBox[E4.bt[1]];
   TAESVectors(pBlock^)[0].bt[2] := AESInvSBox[E3.bt[2]];
   TAESVectors(pBlock^)[0].bt[3] := AESInvSBox[E2.bt[3]];
   TAESVectors(pBlock^)[1].bt[0] := AESInvSBox[E2.bt[0]];
   TAESVectors(pBlock^)[1].bt[1] := AESInvSBox[E1.bt[1]];
   TAESVectors(pBlock^)[1].bt[2] := AESInvSBox[E4.bt[2]];
   TAESVectors(pBlock^)[1].bt[3] := AESInvSBox[E3.bt[3]];
   TAESVectors(pBlock^)[2].bt[0] := AESInvSBox[E3.bt[0]];
   TAESVectors(pBlock^)[2].bt[1] := AESInvSBox[E2.bt[1]];
   TAESVectors(pBlock^)[2].bt[2] := AESInvSBox[E1.bt[2]];
   TAESVectors(pBlock^)[2].bt[3] := AESInvSBox[E4.bt[3]];
   TAESVectors(pBlock^)[3].bt[0] := AESInvSBox[E4.bt[0]];
   TAESVectors(pBlock^)[3].bt[1] := AESInvSBox[E3.bt[1]];
   TAESVectors(pBlock^)[3].bt[2] := AESInvSBox[E2.bt[2]];
   TAESVectors(pBlock^)[3].bt[3] := AESInvSBox[E1.bt[3]];
  end;
 P1 := @pBlock^;
 P2 := @pDKCContext.RoundKeys.Rk[0];
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
 Inc(P1);
 Inc(P2);
 C1 := P1^ xor P2^;
 P1^ := C1;
end;

end.
