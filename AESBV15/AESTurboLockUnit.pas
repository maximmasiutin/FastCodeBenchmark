unit AESTurboLockUnit;

//This unit is an extract of
...
procedure MakeContextsAES_LockBox_PAS_1(const Key; KeySize : Cardinal);
procedure EncryptAES_LockBox_PAS_1(var Block);
procedure DecryptAES_LockBox_PAS_1(var Block);

procedure InitEncryptAES(const Key; KeySize : Longint; var Context : TAESContext; Encrypt : Boolean);
//procedure EncryptAES_TurboLock(const Context : TAESContext; var Block : TAESBlock);
...
type
  ...
  TAESVectors = array[0..(AESNb128 - 1)] of TAESVector;
...
procedure AESRound(const RoundKey : TAESBlock; var State : TAESBlock; Final : Boolean);
...
begin
  for i := 0 to 3 do begin
    if not Final then begin
      e[i].dw := AES_T0[TAESVectors(State)[(i+0) mod 4].bt[0]] xor
                 AES_T1[TAESVectors(State)[(i+1) mod 4].bt[1]] xor
                 AES_T2[TAESVectors(State)[(i+2) mod 4].bt[2]] xor
                 AES_T3[TAESVectors(State)[(i+3) mod 4].bt[3]]
    end else begin
      e[i].bt[0] := AESSBox[TAESVectors(State)[(i+0) mod 4].bt[0]];
      e[i].bt[1] := AESSBox[TAESVectors(State)[(i+1) mod 4].bt[1]];
      e[i].bt[2] := AESSBox[TAESVectors(State)[(i+2) mod 4].bt[2]];
      e[i].bt[3] := AESSBox[TAESVectors(State)[(i+3) mod 4].bt[3]];
    end;
  end;
  XorMemPrim(e, RoundKey, SizeOf(TAESBlock));
  State := TAESBlock(e);
end;

procedure AESInvRound(const RoundKey : TAESBlock; var State : TAESBlock; First : Boolean);
...
var
  ...
  e : TAESVector;
begin
  XorMemPrim(State, RoundKey, SizeOf(TAESBlock));
  for i := 0 to 3 do begin
    if not First then begin
      e.dw := AES_InvT0[TAESVectors(State)[i].bt[0]] xor
              AES_InvT1[TAESVectors(State)[i].bt[1]] xor
              AES_InvT2[TAESVectors(State)[i].bt[2]] xor
              AES_InvT3[TAESVectors(State)[i].bt[3]];
      r[(i+0) mod 4].bt[0] := AESInvSBox[e.bt[0]];
      r[(i+1) mod 4].bt[1] := AESInvSBox[e.bt[1]];
      r[(i+2) mod 4].bt[2] := AESInvSBox[e.bt[2]];
      r[(i+3) mod 4].bt[3] := AESInvSBox[e.bt[3]];
    end else begin
      r[i].bt[0] := AESInvSBox[TAESVectors(State)[(i+0) mod 4].bt[0]];
      r[i].bt[1] := AESInvSBox[TAESVectors(State)[(i+3) mod 4].bt[1]];
      r[i].bt[2] := AESInvSBox[TAESVectors(State)[(i+2) mod 4].bt[2]];
      r[i].bt[3] := AESInvSBox[TAESVectors(State)[(i+1) mod 4].bt[3]];
    end;
  end;
  State := TAESBlock(r);
end;
...
begin
  if Context.Encrypt then begin
    XorMemPrim(Block, Context.RoundKeys.Rk[0], SizeOf(TAESBlock));
    for i := 1 to (Context.Rounds - 1) do
      AESRound(Context.RoundKeys.Rk[i], Block, False);
    AESRound(Context.RoundKeys.Rk[Context.Rounds], Block, True);
  end else begin
    AESInvRound(Context.RoundKeys.Rk[Context.Rounds], Block, True);
    for i := (Context.Rounds - 1) downto 1 do
      AESInvRound(Context.RoundKeys.Rk[i], Block, False);
    XorMemPrim(Block, Context.RoundKeys.Rk[0], SizeOf(TAESBlock));
  end;
end;

procedure InitEncryptAES(const Key; KeySize : Longint;
            var Context : TAESContext; Encrypt : Boolean);
...
begin
  ...
  Sk := Min(KeySize, SizeOf(Context.RoundKeys.Rk));
  Move(Key, Context.RoundKeys.Rk, Sk * 4);
  case KeySize of
   4 : Nk := 4;
   6 : Nk := 6;
   8 : NK := 8;
...
var
 LockBoxEncryptContext : TAESContext;
 LockBoxDecryptContext : TAESContext;

procedure MakeContextsAES_LockBox_PAS_1(const Key; KeySize : Cardinal);
begin
 InitEncryptAES(Key, KeySize, LockBoxEncryptContext, True);
 InitEncryptAES(Key, KeySize, LockBoxDecryptContext, False);
end;

procedure EncryptAES_LockBox_PAS_1(var Block);
begin
 EncryptAES_TurboLock(LockBoxEncryptContext, TAESBlock(Block));
end;

procedure DecryptAES_LockBox_PAS_1(var Block);
begin
 EncryptAES_TurboLock(LockBoxDecryptContext, TAESBlock(Block));
end;
