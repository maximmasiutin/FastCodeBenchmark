...
procedure InitAES_JOH_PAS(const Key; KeySize: cardinal);
procedure EncryptAES_JOH_PAS(var Block);
procedure DecryptAES_JOH_PAS(var Block);
...
const
  RCon : array[0..9] of Integer = ($01,$02,$04,$08,$10,$20,$40,$80,$1B,$36);

type
  AESBlock = packed array[0.. 3] of Integer;
  KeyData  = packed array[0..14] of AESBlock;
...
var
  EncryptContext, DecryptContext : AESContext;
  
{$R-}

procedure AES_Init(const Key; const KeySize: Word);
var
  I, K : Integer;
  PK   : PIntegerArray;
begin
  PK := Pointer(@EncryptContext.Key);
  Move(Key, PK^, 4*KeySize);
  EncryptContext.Rounds := KeySize + 6;
  case KeySize of
    4  : begin
           for I := 0 to 9 do
             begin
               ...
               PK[6] := PK[2] xor PK[5];
               PK[7] := PK[3] xor PK[6];
               Inc(Integer(PK), 4 * SizeOf(Integer));
             end;
         end;
    6  : begin
           for I := 0 to 7 do
             begin
               ...
               Inc(Integer(PK), 6 * SizeOf(Integer));
             end;
         end;
    else begin
           for I := 0 to 6 do
             begin
               ...
               Inc(Integer(PK), 8 * SizeOf(Integer));
             end;
         end;
  end;
end; {AES_Init}

procedure AES_Encrypt(var BO: AESBlock);
var
  I  : Integer;
  PK : PIntegerArray;
  b0, b1, b2, b3 : Integer;
...
begin
  ...
  s3 := BO[3] xor PK[3];
  Inc(Integer(PK), 4 * SizeOf(Integer));
  for I := 1 to EncryptContext.Rounds-1 do
    begin
      b0 := T1[s0 and $ff] xor T2[s1 shr 8 and $ff] xor T3[s2 shr 16 and $ff] xor T4[s3 shr 24];
      b1 := T1[s1 and $ff] xor T2[s2 shr 8 and $ff] xor T3[s3 shr 16 and $ff] xor T4[s0 shr 24];
      b2 := T1[s2 and $ff] xor T2[s3 shr 8 and $ff] xor T3[s0 shr 16 and $ff] xor T4[s1 shr 24];
      b3 := T1[s3 and $ff] xor T2[s0 shr 8 and $ff] xor T3[s1 shr 16 and $ff] xor T4[s2 shr 24];
      s0 := b0 xor PK[0];
      s1 := b1 xor PK[1];
      s2 := b2 xor PK[2];
      s3 := b3 xor PK[3];
...
var
  ...
  s0, s1, s2, s3 : Integer;
begin
  PK := Pointer(@DecryptContext.Key[DecryptContext.Rounds]);
  s0 := BO[0] xor PK[0];
  s1 := BO[1] xor PK[1];
  s2 := BO[2] xor PK[2];
  s3 := BO[3] xor PK[3];
  Dec(Integer(PK), 4 * SizeOf(Integer));
  for I := 1 to DecryptContext.Rounds-1 do
    begin
      b0 := T5[s0 and $ff] xor T6[s3 shr 8 and $ff] xor T7[s2 shr 16 and $ff] xor T8[s1 shr 24];
      b1 := T5[s1 and $ff] xor T6[s0 shr 8 and $ff] xor T7[s3 shr 16 and $ff] xor T8[s2 shr 24];
      b2 := T5[s2 and $ff] xor T6[s1 shr 8 and $ff] xor T7[s0 shr 16 and $ff] xor T8[s3 shr 24];
      b3 := T5[s3 and $ff] xor T6[s2 shr 8 and $ff] xor T7[s1 shr 16 and $ff] xor T8[s0 shr 24];
      s0 := b0 xor PK[0];
      s1 := b1 xor PK[1];
      s2 := b2 xor PK[2];
      s3 := b3 xor PK[3];
...
begin
  ...
  P := PInteger(@DecryptContext.Key[1]);
  for I := 1 to 4*(DecryptContext.Rounds-1) do
    begin         
      P^ := T5[SB[P^        and $ff]] xor
            T6[SB[P^ shr  8 and $ff]] xor
            T7[SB[P^ shr 16 and $ff]] xor
            T8[SB[P^ shr 24]];
      Inc(P);
    end;
end;
...
procedure EncryptAES_JOH_PAS(var Block);
begin
  AES_Encrypt(AESBlock(Block));
end;

procedure DecryptAES_JOH_PAS(var Block);
begin
  AES_Decrypt(AESBlock(Block));
end;
