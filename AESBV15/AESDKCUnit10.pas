...
procedure MakeContextsAES_DKC_PAS_27(const Key; KeySize : Cardinal);
procedure EncryptAES_DKC_PAS_27(var Block);
procedure DecryptAES_DKC_PAS_27(var Block);
...
procedure LogRoundKeys(pDKCContext2 : pTAESContext);
var
 I1, I2, IMax : Integer;
 S : AnsiString;
 J, K : Cardinal;
...
begin
 //IMax := pDKCContext2.Rounds;
 IMax := MaxAESRounds;
 for I1 := 0 to IMax-1 do
  begin
   S := '';
   for I2 := 0 to 3 do
    begin
     K := 4*(I1)+I2;
     J := pDKCContext2.RoundKeys.W[K].dw;
     S := S + ';' + IntToStr(J);
    end;
   MainForm.ReportRichEdit.Lines.Add(S);
  end;
end;

procedure MakeDecryptContext1(pDKCContext2 : pTAESContext);
var
 J : Integer;
begin
...
begin
 ...
 for I := 1 to pDKCContext1.Rounds-1 do
  begin
   K0 := (AES_T0[TAESVectors(pBlock^)[0].dw and $ff] xor
                ...
                P2^;
   Inc(P2);
   K1 := (AES_T0[TAESVectors(pBlock^)[1].dw and $ff] xor
                ...
                P2^;
   Inc(P2);
   K2 := (AES_T0[TAESVectors(pBlock^)[2].dw and $ff] xor
                ...
                P2^;
   Inc(P2);
   K3 := (AES_T0[TAESVectors(pBlock^)[3].dw and $ff] xor
                ...
                P2^;
   Inc(P2);
   TAESVectors(pBlock^)[0].dw := K0;
   TAESVectors(pBlock^)[1].dw := K1;
   TAESVectors(pBlock^)[2].dw := K2;
   TAESVectors(pBlock^)[3].dw := K3;
  end;
 E5[0].bt[0] := AESSBox[TAESVectors(pBlock^)[0].DW and $ff];
 E5[0].bt[1] := AESSBox[TAESVectors(pBlock^)[1].DW shr 8 and $ff];
 E5[0].bt[2] := AESSBox[TAESVectors(pBlock^)[2].DW shr 16 and $ff];
 E5[0].bt[3] := AESSBox[TAESVectors(pBlock^)[3].DW shr 24 and $ff];
 E5[1].bt[0] := AESSBox[TAESVectors(pBlock^)[1].DW and $ff];
 E5[1].bt[1] := AESSBox[TAESVectors(pBlock^)[2].DW shr 8 and $ff];
 E5[1].bt[2] := AESSBox[TAESVectors(pBlock^)[3].DW shr 16 and $ff];
 E5[1].bt[3] := AESSBox[TAESVectors(pBlock^)[0].DW shr 24 and $ff];
 E5[2].bt[0] := AESSBox[TAESVectors(pBlock^)[2].DW and $ff];
 E5[2].bt[1] := AESSBox[TAESVectors(pBlock^)[3].DW shr 8 and $ff];
 E5[2].bt[2] := AESSBox[TAESVectors(pBlock^)[0].DW shr 16 and $ff];
 E5[2].bt[3] := AESSBox[TAESVectors(pBlock^)[1].DW shr 24 and $ff];
 E5[3].bt[0] := AESSBox[TAESVectors(pBlock^)[3].DW and $ff];
 E5[3].bt[1] := AESSBox[TAESVectors(pBlock^)[0].DW shr 8 and $ff];
 E5[3].bt[2] := AESSBox[TAESVectors(pBlock^)[1].DW shr 16 and $ff];
 E5[3].bt[3] := AESSBox[TAESVectors(pBlock^)[2].DW shr 24 and $ff];
 P3 := @pBlock^;
 P3^ := E5[0].dw xor P2^;
 Inc(P2);
 Inc(P3);
...
var
 ...
 P1, P2, P3 : ^Cardinal;
 I : Integer;
 S0, S1, S2, S3, C0, C1, C2, C3 : Cardinal;
...
begin
 ...
 S3 := P1^ xor P2^;
 Dec(P2,4);
 for I := pDKCContext2.Rounds-1 downto 1 do
  begin
   Dec(P2,3);
   TAESVectors(pBlock^)[0].dw := (AES_InvT4[S0 and $ff] xor
                                  AES_InvT5[S3 shr 8 and $ff] xor
                                  AES_InvT6[S2 shr 16 and $ff] xor
                                  AES_InvT7[S1 shr 24]) xor P2^;
   Inc(P2);
   TAESVectors(pBlock^)[1].dw := (AES_InvT4[S1 and $ff] xor
                                  AES_InvT5[S0 shr 8 and $ff] xor
                                  AES_InvT6[S3 shr 16 and $ff] xor
                                  AES_InvT7[S2 shr 24]) xor P2^;
   Inc(P2);
   TAESVectors(pBlock^)[2].dw := (AES_InvT4[S2 and $ff] xor
                                  AES_InvT5[S1 shr 8 and $ff] xor
                                  AES_InvT6[S0 shr 16 and $ff] xor
                                  AES_InvT7[S3 shr 24]) xor P2^;
   Inc(P2);
   TAESVectors(pBlock^)[3].dw := (AES_InvT4[S3 and $ff] xor
                                  AES_InvT5[S2 shr 8 and $ff] xor
                                  AES_InvT6[S1 shr 16 and $ff] xor
                                  AES_InvT7[S0 shr 24]) xor P2^;
   Dec(P2,4);
   S3 := TAESVectors(pBlock^)[3].dw;
   S2 := TAESVectors(pBlock^)[2].dw;
   S1 := TAESVectors(pBlock^)[1].dw;
   S0 := TAESVectors(pBlock^)[0].dw;
  end;
 C0 := (AESInvSBox[s3 and $ff]) xor
       (AESInvSBox[s2 shr 8 and $ff]) shl  8 xor
       (AESInvSBox[s1 shr 16 and $ff]) shl 16 xor
       (AESInvSBox[s0 shr 24]) shl 24;
 C1 := (AESInvSBox[s2 and $ff]) xor
       (AESInvSBox[s1 shr  8 and $ff]) shl  8 xor
       (AESInvSBox[s0 shr 16 and $ff]) shl 16 xor
       (AESInvSBox[s3 shr 24]) shl 24;
 C2 := (AESInvSBox[s1 and $ff]) xor
       (AESInvSBox[s0 shr  8 and $ff]) shl  8 xor
       (AESInvSBox[s3 shr 16 and $ff]) shl 16 xor
       (AESInvSBox[s2 shr 24]) shl 24;
 C3 := (AESInvSBox[s0 and $ff]) xor
       (AESInvSBox[s3 shr  8 and $ff]) shl  8 xor
       (AESInvSBox[s2 shr 16 and $ff]) shl 16 xor
       (AESInvSBox[s1 shr 24]) shl 24;
 P2 := @pDKCContext2.RoundKeys.W[3];
 P3 := @pBlock^;
 Inc(P3,3);
 P3^ := C0 xor P2^;
 Dec(P3);
