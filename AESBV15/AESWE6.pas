...
(*************************************************************************
 ...
 DESCRIPTION     :  AES encrypt/eecrypt functions
...
[*note]
This source code is a special edition for the FastCode AES project on
Dennis Kjaer Christensen's page http://dennishomepage.gugs-cats.dk
Fast code interface assumes Delphi 4 or newer, the full source code
is/will be availables at http://home.netsurf.de/wolfgang.ehrhardt.
**************************************************************************)
...
procedure MakeContextsAES_WE_PAS_6(const Key; KeySize: cardinal);
  {-Initialize unit global contexts, keysize = keybits div 32}

procedure EncryptAES_WE_PAS_6(var Block);
  {-encrypt one block with encryption context}

procedure DecryptAES_WE_PAS_6(var Block);
...
begin
  ...
  if keybits=128 then begin
    for i:=0 to 9 do begin
      ...
      pK^[4] := (cardinal(SBox[(temp shr  8) and $ff])       ) xor
    ...
    for i:=0 to 7 do begin
      ...
      pK^[ 6] := (cardinal(SBox[(temp shr  8) and $ff])       ) xor
                 (cardinal(SBox[(temp shr 16) and $ff]) shl  8) xor
                 (cardinal(SBox[(temp shr 24)        ]) shl 16) xor
                 (cardinal(SBox[(temp       ) and $ff]) shl 24) xor
                 pK^[0] xor RCon[i];
    ...
    for i:=0 to 6 do begin
      ...
      pK^[ 8] := (cardinal(SBox[(temp shr  8) and $ff])       ) xor
                 (cardinal(SBox[(temp shr 16) and $ff]) shl  8) xor
                 (cardinal(SBox[(temp shr 24)        ]) shl 16) xor
                 (cardinal(SBox[(temp       ) and $ff]) shl 24) xor
                 pK^[0] xor RCon[i];
      ...
      pK^[12] := (cardinal(SBox[(temp       ) and $ff])       ) xor
...
var
  ...
  t0,t1,t2,t3: cardinal;    {TAESBlock t as separate variables}
begin
  {Setup key pointer}
  pK := PAWk(@ctx.RK);
  ...
  inc(cardinal(pK), (ctx.rounds shl 2)*sizeof(cardinal));

  {Uses InvSbox and shl, needs type cast cardinal() for}
  {16 bit compilers: here Sbox is byte, Te4 is cardinal}
  TWA4(BO)[0] := (cardinal(SBox[t0        and $ff])        xor
                  cardinal(SBox[t1 shr  8 and $ff]) shl  8 xor
                  cardinal(SBox[t2 shr 16 and $ff]) shl 16 xor
                  cardinal(SBox[t3 shr 24])         shl 24    ) xor pK^[0];
  TWA4(BO)[1] := (cardinal(SBox[t1        and $ff])        xor
                  cardinal(SBox[t2 shr  8 and $ff]) shl  8 xor
                  cardinal(SBox[t3 shr 16 and $ff]) shl 16 xor
                  cardinal(SBox[t0 shr 24])         shl 24    ) xor pK^[1];
  TWA4(BO)[2] := (cardinal(SBox[t2        and $ff])        xor
                  cardinal(SBox[t3 shr  8 and $ff]) shl  8 xor
                  cardinal(SBox[t0 shr 16 and $ff]) shl 16 xor
                  cardinal(SBox[t1 shr 24])         shl 24    ) xor pK^[2];
  TWA4(BO)[3] := (cardinal(SBox[t3        and $ff])        xor
                  cardinal(SBox[t0 shr  8 and $ff]) shl  8 xor
                  cardinal(SBox[t1 shr 16 and $ff]) shl 16 xor
                  cardinal(SBox[t2 shr 24])         shl 24    ) xor pK^[3];
...
begin
  ...
  TWA4(BO)[0] := (cardinal(InvSBox[t0        and $ff])        xor
                  cardinal(InvSBox[t3 shr  8 and $ff]) shl  8 xor
                  cardinal(InvSBox[t2 shr 16 and $ff]) shl 16 xor
                  cardinal(InvSBox[t1 shr 24        ]) shl 24    ) xor pK^[0];
  TWA4(BO)[1] := (cardinal(InvSBox[t1        and $ff])        xor
                  cardinal(InvSBox[t0 shr  8 and $ff]) shl  8 xor
                  cardinal(InvSBox[t3 shr 16 and $ff]) shl 16 xor
                  cardinal(InvSBox[t2 shr 24        ]) shl 24    ) xor pK^[1];
  TWA4(BO)[2] := (cardinal(InvSBox[t2 and $ff       ])        xor
                  cardinal(InvSBox[t1 shr  8 and $ff]) shl  8 xor
                  cardinal(InvSBox[t0 shr 16 and $ff]) shl 16 xor
                  cardinal(InvSBox[t3 shr 24        ]) shl 24    ) xor pK^[2];
  TWA4(BO)[3] := (cardinal(InvSBox[t3 and $ff       ])        xor
                  cardinal(InvSBox[t2 shr  8 and $ff]) shl  8 xor
                  cardinal(InvSBox[t1 shr 16 and $ff]) shl 16 xor
                  cardinal(InvSBox[t0 shr 24        ]) shl 24    ) xor pK^[3];
...
var
  ctx: array[boolean] of TAESContext;  {true=enc, false=dec}
...
procedure EncryptAES_WE_PAS_6(var Block);
  {-encrypt one block with encryption context}
begin
  AES_Encrypt(ctx[true], TAESBlock(Block), TAESBlock(Block));
end;
...
procedure DecryptAES_WE_PAS_6(var Block);
  {-decrypt one block with decryption context}
begin
  AES_Decrypt(ctx[false], TAESBlock(Block), TAESBlock(Block));
end;
