...
var
 pDKCContext1 : pTAESContext;
 pDKCContext2 : pTAESContext;
 AlignedContext1 : array of Byte;
 AlignedContext2 : array of Byte;

procedure MakeDecryptContext2(var pDKCContext2 : pTAESContext);
var
 J : Integer;
 p : ^Cardinal;
 X : Cardinal;
...
begin
 ...
 P := @(pDKCContext2.RoundKeys.W[4]);
 for J := 1 to 4*(pDKCContext2.Rounds-1) do
  begin
   X  := P^;
   p^ := AES_InvT7[AESSBox[X shr 24]] xor
         AES_InvT6[AESSBox[X shr 16 and $ff]] xor
         AES_InvT5[AESSBox[X shr 8 and $ff]] xor
         AES_InvT4[AESSBox[X and $ff]];
   Inc(P);
  end;
end;
...
procedure MakeContextsAES_DKC_IA32_2(const Key; KeySize : Cardinal);
var
 I, J, TAESContextSize, ContextOffSet1, ContextOffSet2 : Integer;
 Nk : Byte;
 Temp : TAESVector;
 Sk : Longint;
...
begin
 ...
 MakeDecryptContext2(pDKCContext2);
end;
...
asm
   ...
   mov   ebp,[pDKCContext1]
   //pDKCContext1.Rounds-1
   mov   edi,[ebp+$00000101]
   dec   edi
   mov   [esp+$10],edi
   //S0 := P1^ xor P2^;
   mov   edx,[esi]
   ...
   add   ebp,$04
   //Dec(P1,3);
   sub   dword ptr [esp],$0c
   //for I := 1 to pDKCContext1.Rounds-1 do
@LoopStart :
   //B0 := (AES_T0[S0 and $ff] xor
   //       AES_T1[S1 shr 8 and $ff] xor
   //       AES_T2[S2 shr 16 and $ff] xor
   //       AES_T3[S3 shr 24]) xor P2^;
   movzx eax,dl
   mov   edi,[eax*4+AES_T0]
   movzx eax,ch
   xor   edi,[eax*4+AES_T1]
   mov   eax,ebx
   shr   eax,$10
   and   eax,$000000ff
   xor   edi,[eax*4+AES_T2]
   mov   eax,esi
   shr   eax,$18
   xor   edi,[eax*4+AES_T3]
   xor   edi,[ebp]
   mov   [esp+$04],edi
   //Inc(P2);
   add   ebp,$04
   //B1 := (AES_T0[S1 and $ff] xor
   //       AES_T1[S2 shr 8 and $ff] xor
   //       AES_T2[S3 shr 16 and $ff] xor
   //       AES_T3[S0 shr 24]) xor P2^;
   movzx eax,cl
   mov   edi,[eax*4+AES_T0]
   movzx eax,bh
   xor   edi,[eax*4+AES_T1]
   ...
   //Inc(P2);
   add   ebp,$04
   //B2 := (AES_T0[S2 and $ff] xor
   //       AES_T1[S3 shr 8 and $ff] xor
   //       AES_T2[S0 shr 16 and $ff] xor
   //       AES_T3[S1 shr 24]) xor P2^;
   movzx eax,bl
   mov   edi,[eax*4+AES_T0]
   mov   eax,esi
   movzx eax,ah
   ...
   //Inc(P2);
   add   ebp,$04
   //B3 := (AES_T0[S3 and $ff] xor
   //       AES_T1[S0 shr 8 and $ff] xor
   //       AES_T2[S1 shr 16 and $ff] xor
   //       AES_T3[S2 shr 24]) xor P2^;
   and   esi,$000000ff
   mov   esi,[esi*4+AES_T0]
   movzx edx,dh
   xor   esi,[edx*4+AES_T1]
   shr   ecx,$10
   and   ecx,$000000ff
   xor   esi,[ecx*4+AES_T2]
   shr   ebx,$18
   xor   esi,[ebx*4+AES_T3]
   xor   esi,[ebp]
   //Inc(P2);
   add   ebp,$04
   //S0 := B0;
   ...
   jnz   @LoopStart
 @LoopEnd :
   //P1^ := ((AESSBox[s0 and $ff]) xor
   //        (AESSBox[s1 shr 8 and $ff]) shl  8 xor
   //        (AESSBox[s2 shr 16 and $ff]) shl 16 xor
   //        (AESSBox[s3 shr 24]) shl 24) xor P2^;
   movzx eax,dl
   movzx edi,[eax+AESSBox]
   mov   eax,ecx
   movzx eax,ah
   movzx eax,[eax+AESSBox]
   shl   eax,$08
   xor   edi,eax
   mov   eax,ebx
   shr   eax,$10
   ...
   //P1^ := ((AESSBox[s1 and $ff]) xor
   //        (AESSBox[s2 shr  8 and $ff]) shl  8 xor
   //        (AESSBox[s3 shr 16 and $ff]) shl 16 xor
   //        (AESSBox[s0 shr 24]) shl 24) xor P2^;
   //mov   edi,ecx
   //and   edi,$000000ff
   movzx edi,cl
   movzx edi,[edi+AESSBox]
   mov   eax,ebx
   movzx eax,bh
   movzx eax,[eax+AESSBox]
   shl   eax,$08
   ...
   //Inc(P1);
   add   dword ptr [esp],$04
   //P1^ := ((AESSBox[s2 and $ff]) xor
   //        (AESSBox[s3 shr  8 and $ff]) shl  8 xor
   //        (AESSBox[s0 shr 16 and $ff]) shl 16 xor
   //        (AESSBox[s1 shr 24]) shl 24) xor P2^;
   //mov   edi,ebx
   //and   edi,$000000ff
   movzx edi,bl
   movzx edi,[edi+AESSBox]
   ...
   //Inc(P1);
   add   dword ptr [esp],$04
   //P1^ := ((AESSBox[s3 and $ff]) xor
   //        (AESSBox[s0 shr  8 and $ff]) shl  8 xor
   //        (AESSBox[s1 shr 16 and $ff]) shl 16 xor
   //        (AESSBox[s2 shr 24]) shl 24) xor P2^;
   and   esi,$000000ff
   movzx esi,[esi+AESSBox]
   movzx edx,dh
...
procedure DecryptAES_DKC_IA32_2a(var Block);
var
 pBlock : ^TAESBlock;
 P1, P2, P3 : ^Cardinal;
 I : Integer;
 S0, S1, S2, S3, B0, B1, B2, B3 : Cardinal;

begin
{
00467BC8 53               push ebx
00467BC9 56               push esi
00467BCA 57               push edi
00467BCB 55               push ebp
00467BCC 83C4E8           add esp,-$18
}
 pBlock := @Block;
 {
 00467BCF 890424           mov [esp],eax
 }
 P1 := @pBlock^;
 {
 00467BD2 8B3424           mov esi,[esp]
 }
 P2 := @pDKCContext2.RoundKeys.Rk[pDKCContext2.Rounds];
 {
 00467BD5 A1C0A84800       mov eax,[pDKCContext2]
 00467BDA 8B8001010000     mov eax,[eax+$00000101]
 00467BE0 03C0             add eax,eax
 00467BE2 8B15C0A84800     mov edx,[pDKCContext2]
 00467BE8 8D04C2           lea eax,[edx+eax*8]
 }
 S0 := P1^ xor P2^;
 {
 00467BEB 8B16             mov edx,[esi]
 00467BED 3310             xor edx,[eax]
 }
 Inc(P1);
 {
 00467BEF 83C604           add esi,$04
 }
 Inc(P2);
 {
 00467BF2 83C004           add eax,$04
 }
 S1 := P1^ xor P2^;
 {
 00467BF5 8B0E             mov ecx,[esi]
 00467BF7 3308             xor ecx,[eax]
 }
 Inc(P1);
 {
 00467BF9 83C604           add esi,$04
 }
 Inc(P2);
 {
 00467BFC 83C004           add eax,$04
 }
 S2 := P1^ xor P2^;
 {
 00467BFF 8B1E             mov ebx,[esi]
 00467C01 3318             xor ebx,[eax]
 }
 Inc(P1);
 {
 00467C03 83C604           add esi,$04
 }
 Inc(P2);
 {
 00467C06 83C004           add eax,$04
 }
 S3 := P1^ xor P2^;
 {
 00467C09 8B36             mov esi,[esi]
 00467C0B 3330             xor esi,[eax]
 }
 Dec(P2,4);
 {
 00467C0D 83E810           sub eax,$10
 }
 I := pDKCContext2.Rounds-1;
 {
 00467C10 8B3DC0A84800     mov edi,[pDKCContext2]
 00467C16 8BBF01010000     mov edi,[edi+$00000101]
 00467C1C 4F               dec edi
 00467C1D 897C2408         mov [esp+$08],edi
 }
 repeat
 {
 }
  B0 := AES_InvT4[S0 and $ff] xor
        AES_InvT5[S3 shr 8 and $ff] xor
        AES_InvT6[S2 shr 16 and $ff] xor
        AES_InvT7[S1 shr 24];
  {
  00467C21 8BFA             mov edi,edx
  00467C23 81E7FF000000     and edi,$000000ff
  00467C29 8B3CBD706B4800   mov edi,[edi*4+AES_InvT4]
  00467C30 8BEE             mov ebp,esi
  00467C32 C1ED08           shr ebp,$08
  00467C35 81E5FF000000     and ebp,$000000ff
  00467C3B 333CAD706F4800   xor edi,[ebp*4+AES_InvT5]
  00467C42 8BEB             mov ebp,ebx
  00467C44 C1ED10           shr ebp,$10
  00467C47 81E5FF000000     and ebp,$000000ff
  00467C4D 333CAD70734800   xor edi,[ebp*4+AES_InvT6]
  00467C54 8BE9             mov ebp,ecx
  00467C56 C1ED18           shr ebp,$18
  00467C59 333CAD70774800   xor edi,[ebp*4+AES_InvT7]
  00467C60 897C240C         mov [esp+$0c],edi
  }
  B1 := AES_InvT4[S1 and $ff] xor
        AES_InvT5[S0 shr 8 and $ff] xor
        AES_InvT6[S3 shr 16 and $ff] xor
        AES_InvT7[S2 shr 24];
  {
  00467C64 8BF9             mov edi,ecx
  00467C66 81E7FF000000     and edi,$000000ff
  00467C6C 8B3CBD706B4800   mov edi,[edi*4+AES_InvT4]
  00467C73 8BEA             mov ebp,edx
  00467C75 C1ED08           shr ebp,$08
  00467C78 81E5FF000000     and ebp,$000000ff
  00467C7E 333CAD706F4800   xor edi,[ebp*4+AES_InvT5]
  00467C85 8BEE             mov ebp,esi
  00467C87 C1ED10           shr ebp,$10
  00467C8A 81E5FF000000     and ebp,$000000ff
  00467C90 333CAD70734800   xor edi,[ebp*4+AES_InvT6]
  00467C97 8BEB             mov ebp,ebx
  00467C99 C1ED18           shr ebp,$18
  00467C9C 333CAD70774800   xor edi,[ebp*4+AES_InvT7]
  00467CA3 897C2410         mov [esp+$10],edi
  }
  B2 := AES_InvT4[S2 and $ff] xor
        AES_InvT5[S1 shr 8 and $ff] xor
        AES_InvT6[S0 shr 16 and $ff] xor
        AES_InvT7[S3 shr 24];
  {
  00467CA7 8BFB             mov edi,ebx
  00467CA9 81E7FF000000     and edi,$000000ff
  00467CAF 8B3CBD706B4800   mov edi,[edi*4+AES_InvT4]
  00467CB6 8BE9             mov ebp,ecx
  00467CB8 C1ED08           shr ebp,$08
  00467CBB 81E5FF000000     and ebp,$000000ff
  00467CC1 333CAD706F4800   xor edi,[ebp*4+AES_InvT5]
  00467CC8 8BEA             mov ebp,edx
  00467CCA C1ED10           shr ebp,$10
  00467CCD 81E5FF000000     and ebp,$000000ff
  00467CD3 333CAD70734800   xor edi,[ebp*4+AES_InvT6]
  00467CDA 8BEE             mov ebp,esi
  00467CDC C1ED18           shr ebp,$18
  00467CDF 333CAD70774800   xor edi,[ebp*4+AES_InvT7]
  00467CE6 897C2414         mov [esp+$14],edi
  }
  B3 := AES_InvT4[S3 and $ff] xor
        AES_InvT5[S2 shr 8 and $ff] xor
        AES_InvT6[S1 shr 16 and $ff] xor
        AES_InvT7[S0 shr 24];
  {
  00467CEA 81E6FF000000     and esi,$000000ff
  00467CF0 8B34B5706B4800   mov esi,[esi*4+AES_InvT4]
  00467CF7 C1EB08           shr ebx,$08
  00467CFA 81E3FF000000     and ebx,$000000ff
  00467D00 33349D706F4800   xor esi,[ebx*4+AES_InvT5]
  00467D07 C1E910           shr ecx,$10
  00467D0A 81E1FF000000     and ecx,$000000ff
  00467D10 33348D70734800   xor esi,[ecx*4+AES_InvT6]
  00467D17 C1EA18           shr edx,$18
  00467D1A 33349570774800   xor esi,[edx*4+AES_InvT7]
  00467D21 8BD6             mov edx,esi
  }
  S3 := B3 xor P2^;
  {
  00467D23 8B30             mov esi,[eax]
  00467D25 33F2             xor esi,edx
  }
  Dec(P2);
  {
  00467D27 83E804           sub eax,$04
  }
  S2 := B2 xor P2^;
  {
  00467D2A 8B5C2414         mov ebx,[esp+$14]
  00467D2E 3318             xor ebx,[eax]
  }
  Dec(P2);
  {
  00467D30 83E804           sub eax,$04
  }
  S1 := B1 xor P2^;
  {
  00467D33 8B4C2410         mov ecx,[esp+$10]
  00467D37 3308             xor ecx,[eax]
  }
  Dec(P2);
  {
  00467D39 83E804           sub eax,$04
  }
  S0 := B0 xor P2^;
  {
  00467D3C 8B54240C         mov edx,[esp+$0c]
  00467D40 3310             xor edx,[eax]
  }
  Dec(P2);
  {
  00467D42 83E804           sub eax,$04
  }
  Dec(I);
  {
  00467D45 FF4C2408         dec dword ptr [esp+$08]
  }
 until(I = 0);
 {
 00467D49 837C240800       cmp dword ptr [esp+$08],$00
 00467D4E 0F85CDFEFFFF     jnz -$00000133
 }
 P3 := @pBlock^;
 {
 00467D54 8B3C24           mov edi,[esp]
 00467D57 897C2404         mov [esp+$04],edi
 }
 Inc(P3,3);
 {
 00467D5B 834424040C       add dword ptr [esp+$04],$0c
 }
 P3^ := ((AESInvSBox[s3 and $ff]) xor
         (AESInvSBox[s2 shr 8 and $ff]) shl  8 xor
         (AESInvSBox[s1 shr 16 and $ff]) shl 16 xor
         (AESInvSBox[s0 shr 24]) shl 24) xor P2^;
 {
 00467D60 8BFE             mov edi,esi
 00467D62 81E7FF000000     and edi,$000000ff
 00467D68 0FB6BF345A4800   movzx edi,[edi+AESInvSBox]
 00467D6F 8BEB             mov ebp,ebx
 00467D71 C1ED08           shr ebp,$08
 00467D74 81E5FF000000     and ebp,$000000ff
 00467D7A 0FB6AD345A4800   movzx ebp,[ebp+AESInvSBox]
 00467D81 C1E508           shl ebp,$08
 00467D84 33FD             xor edi,ebp
 00467D86 8BE9             mov ebp,ecx
 00467D88 C1ED10           shr ebp,$10
 00467D8B 81E5FF000000     and ebp,$000000ff
 00467D91 0FB6AD345A4800   movzx ebp,[ebp+AESInvSBox]
 00467D98 C1E510           shl ebp,$10
 00467D9B 33FD             xor edi,ebp
 00467D9D 8BEA             mov ebp,edx
 00467D9F C1ED18           shr ebp,$18
 00467DA2 0FB6AD345A4800   movzx ebp,[ebp+AESInvSBox]
 00467DA9 C1E518           shl ebp,$18
 00467DAC 33FD             xor edi,ebp
 00467DAE 3338             xor edi,[eax]
 00467DB0 8B6C2404         mov ebp,[esp+$04]
 00467DB4 897D00           mov [ebp+$00],edi
 }
 Dec(P2);
 {
 00467DB7 83E804           sub eax,$04
 }
 Dec(P3);
 {
 00467DBA 836C240404       sub dword ptr [esp+$04],$04
 }
 P3^ := ((AESInvSBox[s2 and $ff]) xor
         (AESInvSBox[s1 shr  8 and $ff]) shl  8 xor
         (AESInvSBox[s0 shr 16 and $ff]) shl 16 xor
         (AESInvSBox[s3 shr 24]) shl 24) xor P2^;
 {
 00467DBF 8BFB             mov edi,ebx
 00467DC1 81E7FF000000     and edi,$000000ff
 00467DC7 0FB6BF345A4800   movzx edi,[edi+AESInvSBox]
 00467DCE 8BE9             mov ebp,ecx
 00467DD0 C1ED08           shr ebp,$08
 00467DD3 81E5FF000000     and ebp,$000000ff
 00467DD9 0FB6AD345A4800   movzx ebp,[ebp+AESInvSBox]
 00467DE0 C1E508           shl ebp,$08
 00467DE3 33FD             xor edi,ebp
 00467DE5 8BEA             mov ebp,edx
 00467DE7 C1ED10           shr ebp,$10
 00467DEA 81E5FF000000     and ebp,$000000ff
 00467DF0 0FB6AD345A4800   movzx ebp,[ebp+AESInvSBox]
 00467DF7 C1E510           shl ebp,$10
 00467DFA 33FD             xor edi,ebp
 00467DFC 8BEE             mov ebp,esi
 00467DFE C1ED18           shr ebp,$18
 00467E01 0FB6AD345A4800   movzx ebp,[ebp+AESInvSBox]
 00467E08 C1E518           shl ebp,$18
 00467E0B 33FD             xor edi,ebp
 00467E0D 3338             xor edi,[eax]
 00467E0F 8B6C2404         mov ebp,[esp+$04]
 00467E13 897D00           mov [ebp+$00],edi
 }
 Dec(P2);
 {
 00467E16 83E804           sub eax,$04
 }
 Dec(P3);
 {
 00467E19 836C240404       sub dword ptr [esp+$04],$04
 }
 P3^ := ((AESInvSBox[s1 and $ff]) xor
         (AESInvSBox[s0 shr  8 and $ff]) shl  8 xor
         (AESInvSBox[s3 shr 16 and $ff]) shl 16 xor
         (AESInvSBox[s2 shr 24]) shl 24) xor P2^;
 {
 00467E1E 8BF9             mov edi,ecx
 00467E20 81E7FF000000     and edi,$000000ff
 00467E26 0FB6BF345A4800   movzx edi,[edi+AESInvSBox]
 00467E2D 8BEA             mov ebp,edx
 00467E2F C1ED08           shr ebp,$08
 00467E32 81E5FF000000     and ebp,$000000ff
 00467E38 0FB6AD345A4800   movzx ebp,[ebp+AESInvSBox]
 00467E3F C1E508           shl ebp,$08
 00467E42 33FD             xor edi,ebp
 00467E44 8BEE             mov ebp,esi
 00467E46 C1ED10           shr ebp,$10
 00467E49 81E5FF000000     and ebp,$000000ff
 00467E4F 0FB6AD345A4800   movzx ebp,[ebp+AESInvSBox]
 00467E56 C1E510           shl ebp,$10
 00467E59 33FD             xor edi,ebp
 00467E5B 8BEB             mov ebp,ebx
 00467E5D C1ED18           shr ebp,$18
 00467E60 0FB6AD345A4800   movzx ebp,[ebp+AESInvSBox]
 00467E67 C1E518           shl ebp,$18
 00467E6A 33FD             xor edi,ebp
 00467E6C 3338             xor edi,[eax]
 00467E6E 8B6C2404         mov ebp,[esp+$04]
 00467E72 897D00           mov [ebp+$00],edi
 }
 Dec(P2);
 {
 00467E75 83E804           sub eax,$04
 }
 Dec(P3);
 {
 00467E78 836C240404       sub dword ptr [esp+$04],$04
 }
 P3^ := ((AESInvSBox[s0 and $ff]) xor
         (AESInvSBox[s3 shr  8 and $ff]) shl  8 xor
         (AESInvSBox[s2 shr 16 and $ff]) shl 16 xor
         (AESInvSBox[s1 shr 24]) shl 24) xor P2^;
 {
 00467E7D 81E2FF000000     and edx,$000000ff
 00467E83 0FB692345A4800   movzx edx,[edx+AESInvSBox]
 00467E8A C1EE08           shr esi,$08
 00467E8D 81E6FF000000     and esi,$000000ff
 00467E93 0FB6B6345A4800   movzx esi,[esi+AESInvSBox]
 00467E9A C1E608           shl esi,$08
 00467E9D 33D6             xor edx,esi
 00467E9F C1EB10           shr ebx,$10
 00467EA2 81E3FF000000     and ebx,$000000ff
 00467EA8 0FB69B345A4800   movzx ebx,[ebx+AESInvSBox]
 00467EAF C1E310           shl ebx,$10
 00467EB2 33D3             xor edx,ebx
 00467EB4 C1E918           shr ecx,$18
 00467EB7 0FB689345A4800   movzx ecx,[ecx+AESInvSBox]
 00467EBE C1E118           shl ecx,$18
 00467EC1 33D1             xor edx,ecx
 00467EC3 3310             xor edx,[eax]
 00467EC5 8B442404         mov eax,[esp+$04]
 00467EC9 8910             mov [eax],edx
 }
{
00467ECB 83C418           add esp,$18
00467ECE 5D               pop ebp
00467ECF 5F               pop edi
00467ED0 5E               pop esi
00467ED1 5B               pop ebx
...
procedure DecryptAES_DKC_IA32_2b(var Block);
var
 pBlock : ^TAESBlock;
 P1, P2, P3 : ^Cardinal;
 I : Integer;
...
add esp,-$18
 ...
 P2 := @pDKCContext2.RoundKeys.Rk[pDKCContext2.Rounds];
 {
 mov eax,[pDKCContext2]
 mov eax,[eax+$00000101]
 add eax,eax
 mov edx,[pDKCContext2]
 lea eax,[edx+eax*8]
 }
 S0 := P1^ xor P2^;
 {
 ...
 I := pDKCContext2.Rounds-1;
 {
 mov edi,[pDKCContext2]
 mov edi,[edi+$00000101]
 dec edi
 mov [esp+$08],edi
 }
 repeat
 {
  ...
  mov edi,edx
  and edi,$000000ff
  mov edi,[edi*4+AES_InvT4]
  mov ebp,esi
  shr ebp,$08
  and ebp,$000000ff
  xor edi,[ebp*4+AES_InvT5]
  mov ebp,ebx
  shr ebp,$10
  and ebp,$000000ff
  xor edi,[ebp*4+AES_InvT6]
  mov ebp,ecx
  shr ebp,$18
  xor edi,[ebp*4+AES_InvT7]
  mov [esp+$0c],edi
  }
  B1 := AES_InvT4[S1 and $ff] xor
        AES_InvT5[S0 shr 8 and $ff] xor
  ...
  {
  and esi,$000000ff
  mov esi,[esi*4+AES_InvT4]
  shr ebx,$08
  and ebx,$000000ff
  xor esi,[ebx*4+AES_InvT5]
  shr ecx,$10
  and ecx,$000000ff
  xor esi,[ecx*4+AES_InvT6]
  shr edx,$18
  xor esi,[edx*4+AES_InvT7]
  mov edx,esi
  }
  S3 := B3 xor P2^;
  {
 ...
 mov edi,esi
 and edi,$000000ff
 movzx edi,[edi+AESInvSBox]
 mov ebp,ebx
 shr ebp,$08
 and ebp,$000000ff
 movzx ebp,[ebp+AESInvSBox]
 shl ebp,$08
 xor edi,ebp
 mov ebp,ecx
 shr ebp,$10
 ...
 {
 and edx,$000000ff
 movzx edx,[edx+AESInvSBox]
 shr esi,$08
 and esi,$000000ff
 movzx esi,[esi+AESInvSBox]
 shl esi,$08
 xor edx,esi
 shr ebx,$10
 and ebx,$000000ff
 movzx ebx,[ebx+AESInvSBox]
 shl ebx,$10
 xor edx,ebx
 shr ecx,$18
 movzx ecx,[ecx+AESInvSBox]
 shl ecx,$18
 xor edx,ecx
 xor edx,[eax]
 mov eax,[esp+$04]
...
procedure DecryptAES_DKC_IA32_2c(var Block);
asm
push ebx
push esi
push edi
...
procedure DecryptAES_DKC_IA32_2d(var Block);
asm
   push  ebx
   push  esi
   push  edi
...
procedure DecryptAES_DKC_IA32_2e(var Block);
asm
   push  ebx
   push  esi
   push  edi
...
procedure DecryptAES_DKC_IA32_2f(var Block);
asm
   push  ebx
   push  esi
   push  edi
...
procedure DecryptAES_DKC_IA32_2g(var Block);
asm
   push  ebx
   push  esi
   push  edi
...
procedure DecryptAES_DKC_IA32_2h(var Block);
asm
   push  ebx
   push  esi
   push  edi
   ...
   //mov   eax,eax
   //P2 := @pDKCContext2.RoundKeys.Rk[pDKCContext2.Rounds];
   mov   esi,[pDKCContext2]
   mov   edx,esi
   mov   esi,[esi+$00000101]
   mov   edi,esi
   add   esi,esi
   //mov   edx,[pDKCContext2]
   lea   esi,[edx+esi*8]
 ...
 @LoopStart :
   ...
   //      AES_InvT7[S0 shr 24];
   and   eax,$000000ff
   mov   eax,[eax*4+AES_InvT4]
   //shr   ebx,$08
   //and   ebx,$000000ff
   movzx ebx,bh
   xor   eax,[ebx*4+AES_InvT5]
   shr   ecx,$10
   and   ecx,$000000ff
   xor   eax,[ecx*4+AES_InvT6]
   shr   edx,$18
   xor   eax,[edx*4+AES_InvT7]
   mov   edx,eax
   //S3 := B3 xor P2^;
   mov   eax,[esi]
   xor   eax,edx
...
procedure DecryptAES_DKC_IA32_2i(var Block);
asm
   push  ebx
   push  esi
   push  edi
