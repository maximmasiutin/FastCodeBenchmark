...
(*
  ...
  History:
   ...
   18/01/2005 BucketMem 1.0 First version sent to FastCode MM Challenge using spinlocks instead of
    ...
    19/12/2005 Dennis Kjaer Christensen fixed a bug. Search for "//Added by DKC" to see code changes
    20/12/2005 Dennis Kjaer Christensen fixed a bug. Search for "//Added by DKC" to see code changes
*)
...
asm
  ...
  @NewSheet:
    ...
    call CreateSheet
    test eax, eax
    jz   @OutOfMemory     //Added by DKC
    mov eax, TBucket(esi).CurAddr
    jnz @Linear

  @OutOfMemory:
    mov byte ptr TBucket(esi).SpinLock, 0 //Added by DKC
    xor eax, eax
    pop esi
    ret
end;
