...
(*
  ...
  History:
   ...
   18/01/2005 BucketMem 1.0 First version sent to FastCode MM Challenge using spinlocks instead of
    ...
    19/12/2005 Dennis Kjaer Christensen fixed two bugs and made some optimizations.
    23/1/2006 Dennis Kjaer Christensen made some optimizations.
*)
...
{.$define USECACHEOFFSET}      // give arbitrary offset to large blocks to improve cache associativity
...
procedure OSHeapInit; {$ifdef ver170}inline; {$endif} {$ifdef ver180}inline; {$endif}
begin
 HeapHandle := GetProcessHeap;
end;

function OSHeapAlloc(Size: Cardinal): Pointer; {$ifdef ver170}inline; {$endif} {$ifdef ver180}inline; {$endif}
begin
 Result := HeapAlloc(HeapHandle, 0, Size);
end;

function OSHeapFree(P: Pointer): boolean; {$ifdef ver170}inline; {$endif} {$ifdef ver180}inline; {$endif}
begin
 Result := HeapFree(HeapHandle, 0, P);
end;

function OSVirtualFree(P: Pointer): boolean; {$ifdef ver180}inline; {$endif}
begin
...
begin
  ...
  Result.P := VirtualAlloc(nil, VBlockDescriptor.VMSize, MEM_COMMIT, PAGE_READWRITE);
...
procedure LockVBlocks(VBlockDescriptor: PVBlockDescriptor);{$ifdef ver170}inline; {$endif} {$ifdef ver180}inline; {$endif}
begin
...
procedure ReleaseVBlocks(VBlockDescriptor: PVBlockDescriptor); {$ifdef ver170}inline; {$endif} {$ifdef ver180}inline; {$endif}
begin
...
begin
  if Size > OldSize then begin
    ...
    //else
      //inc(Size, Size div 4);  //25% overallocation
  end;
  ...
  if OldSize < Size then
   ...
   MoveSize := Size;
  MoveSize := (MoveSize + $00000003) and $FFFFFFFC;  // round for Move operation
  FastMove(Pointer(NativeUInt(Pntr) + 4)^, Result^, MoveSize - 4);
  FreeMemBlock(Pntr);
end;
...
var
 SizeX, ReallocSizeDifference, MoveSize : Cardinal;

begin
  if Size > OldSize then
   begin//upsize
    // Anticipate future upsizes if already resized at least EXACT_REALLOC_COUNT times
   ...
   begin //Downsize
    if Pntr.ReallocCount < EXACT_REALLOC_COUNT then begin
      ...
      if (Size > MAX_BUCKET_MEM) and (((Size + $0000FFFF) and $FFFF0000) = OldSize) then
       begin
      ...
      {$else}
      // possibly small downsize... check whether the original block would be fine
      SizeX := ((Size + $0000FFFF) and $FFFF0000);
      if OldSize >= SizeX then
       ReallocSizeDifference := OldSize - SizeX//Downsize <=> SizeX < OldSize
      else
       ReallocSizeDifference := SizeX - OldSize;//Should never get here !!!!!!!!!!!!!!!!!!!!
      if (Size+16 > MAX_BUCKET_MEM) and (ReallocSizeDifference <= Size div 4) then
       begin
        inc(Pntr.ReallocCount);
        Result := Pointer(NativeUInt(Pntr) + 16);
        Exit;
  ...
  FastMove(Pointer(NativeUInt(Pntr) + 16)^, Result^, MoveSize - 16);
...
begin
  ...
  if (Size and $FFFF) + OffSet >= MAXOFFSET then
    OffSet := 0;
  ...
  {$else}
  Size := (Size + $0000FFFF) and $FFFF0000;
