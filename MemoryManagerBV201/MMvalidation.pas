...
begin
  ...
  DoValidate(Validate34, 'Validate34');
  DoValidate(Validate35, 'Validate35');
  DoValidate(Validate36, 'Validate36');
  DoValidate(Validate37, 'Validate37');
  DoValidate(Validate38, 'Validate38');
  // Validate13, 29, 30, 31, 33 are really very nasty by provoking an EOutofMemory
  // We execute it at the end, then give the system 10 seconds to relax...
  DoValidate(Validate13, 'Validate13');
  Sleep(10000);
  DoValidate(Validate29, 'Validate29');
  Sleep(10000);
  DoValidate(Validate30, 'Validate30');
  Sleep(10000);
  DoValidate(Validate31, 'Validate31');
  Sleep(10000);
  DoValidate(Validate33, 'Validate33');
  Sleep(10000);
...
begin
  ...
  DoValidate(Validate32, 'Validate32');
...
var
 Pointers : array[0..250000] of Pointer;
 n : integer;
...
begin
  ...
  begin
    ...
    FillChar(PChar(Integer(LStrings[LIndex]) + LOldSize)^, LNewSize - LOldSize, Byte(LIndex));
  end;
end;
...
var
 Pointers : array[0..150000] of Pointer;
 n : integer;
...
var
 ...
 pMem : Pointer;
 PointerArray : array[1..1000] of Pointer;
 MemoryStatus : TMemoryStatus;
const
 BYTESTOALLOCATEMAX : Cardinal = 2*1024*1024;//2 Mbyte per pointer max
 RUNNOMAX : Cardinal = 1250;
...
begin
 ...
 try
  ...
  repeat
   //Allocate pointer
   AllocSizeFP := AllocSizeFP * STEPSIZE;
   AllocSize := Round(AllocSizeFP);
...
const
 BYTESTOALLOCATEMIN : Cardinal = 1;//1 byte
 BYTESTOALLOCATEMAX : Cardinal = 1*1024*1024;//1 Mbyte
   ...
   begin
    if MemoryStatus.dwAvailVirtual >= BytesToAllocate then
     begin
      ...
      BytesToAllocateFirst := Random(BYTESTOALLOCATEMAX-1)+1;
