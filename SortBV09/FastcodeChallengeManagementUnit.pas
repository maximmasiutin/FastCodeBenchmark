...
uses
  Classes, FastcodeChallengeUnit, Math, ReportWriterUnit, SysUtils, Windows;
...
type
  ...
  public
    ...
    procedure Lock;
    procedure ReportText(const Challenges: array of TFastcodeChallenge; ReportWriter: TReportWriter);
    procedure Unlock;
...
uses
  D5Support, SystemInfoUnit;

const
  FAILURE_COUNT_MAX = 256;

type
  PDummyResultsRec = ^TDummyResultsRec;
  TDummyResultsRec = record
    Challenge: TFastcodeChallenge;
    Results: TSubBenchmarkResultArray;
  end;
...
begin
  ...
  begin
    DoStatusChange(Format('Running validator %d (out of %d) on function %s', [I+1, Challenge.ValidatorCount, Func^.Name]), True);
...
procedure TFastcodeChallengeManager.ReportText(const Challenges: array of TFastcodeChallenge; ReportWriter: TReportWriter);
var
 ...
 procedure ReportResultTableRow(ResultRec: PFunctionResultRec);
 var
   I: Integer;
   {$ifdef ReportSpread}
     Spread: Double;
...
begin
  ReportWriter.SectionStart('Benchmark report');
  try
    ReportTestInfoTable;

    for I := Low(Challenges) to High(Challenges) do
    begin
      if not HasResults(Challenges[I]) then
        Continue;
      ReportWriter.SectionStart(Format(
        'Benchmark report for %s challenge (B&V version %s)',
        [Challenges[I].Name, Challenges[I].Version]));
      try
          ...
          try
            ...
            try
              ...
              for J := 0 to FunctionsDisplayed.Count - 1 do
                ReportResultTableRow(PFunctionResultRec(FunctionsDisplayed[J]));
            finally
              Unlock;
            end;
          finally
