...
procedure RGBToBGRLBGIA32_3(SrcDestBitmap : TBitmap);
...
begin
  ...
  begin
    ...
    begin
    BSwap(pScanLine);
//      pScanLine^ := (pScanLine^ and $0000ff00)+  ((pScanLine^ and $00ff0000) shr 16)+ ((pScanLine^ and $000000ff) shl 16);
      inc(pScanLine, 1);
    end;
    dec(pScanLine, ScanIncr);
  end;
...
//New:               18/12 2004
//Instructionset(s): IA32 (486 and up)
//Note:              A tuned "RGBToBGR_JOH_PAS" function.
...
procedure BSwap( PP:PByteArray ; const PP2 : PByteArray);
asm
@1:     mov      ecx ,[eax]
         bswap    ecx
         ror      ecx , 8
...
begin
   if SrcDestBitmap.PixelFormat <> pf32bit then
     raise Exception.Create('Only pf32bit PixelFormat supported');
   Height := SrcDestBitmap.Height;
   if Height > 0 then  begin
     P1 := SrcDestBitmap.ScanLine[0];
     if Height > 1 then
       begin
         P2  := SrcDestBitmap.ScanLine[Height-1];
         if Integer(P1) > Integer(P2) then P1 := P2;
       end;
     P2 := @P1[SrcDestBitmap.Width * Height * 4];
     BSwap(P1,P2);
   end;
end;
end.
