...
procedure OSHeapInit; {$ifdef ver180}inline; {$endif}
begin
 HeapHandle := GetProcessHeap;
end;

function OSHeapAlloc(Size: Cardinal): Pointer; {$ifdef ver180}inline; {$endif}
begin
 Result := HeapAlloc(HeapHandle, 0, Size);
end;

function OSHeapFree(P: Pointer): boolean; {$ifdef ver180}inline; {$endif}
begin
 Result := HeapFree(HeapHandle, 0, P);
end;

//function OSVirtualAlloc(Size: Cardinal): Pointer; {$ifdef ver180}inline; {$endif}
//begin
...
begin
  ...
  FastMove(Pointer(Integer(Pntr) + 4)^, Result^, Size - 4);
  FreeMemBlock(Pntr);
end;
...
begin
  ...
  FastMove(Pointer(Integer(Pntr) + 16)^, Result^, Size - 16);
